...
const loaderStyles = cva("relative", {
  ...
  defaultVariants: { intent: "current", size: "sm" },
})

type LoaderVariantProps = VariantProps<typeof loaderStyles>

const Bars = ({ className, ...props }: React.SVGProps<SVGSVGElement>) => (
  <svg className={cn("size-4", className)} data-slot="icon" viewBox="0 0 135 140"
       xmlns="http://www.w3.org/2000/svg" fill="currentColor" {...props}>
...
// "spin" : spinner type Apple (12 traits dégradés qui tournent).
  ...
  <svg className={cn("size-4", className)} data-slot="icon" viewBox="0 0 2400 2400" {...props}>
    <g strokeWidth="200" strokeLinecap="round" fill="none">
      ...
      <animateTransform attributeName="transform" attributeType="XML" type="rotate"
        ...
        dur="0.83333s" begin="0.08333s" repeatCount="indefinite" calcMode="discrete" />
    </g>
  </svg>
)

// "ring" : cercle qui tourne (lucide, en remplacement de @intentui IconLoader).
const Ring = ({ className, ...props }: React.SVGProps<SVGSVGElement>) => (
  <Loader2 className={cn("size-4 animate-spin", className)} {...props} />
)

const LOADERS = { bars: Bars, ring: Ring, spin: Spin }
...
function Loader({ className, variant = DEFAULT_SPINNER, intent, size, ...props }: LoaderProps) {
  const LoaderPrimitive = LOADERS[variant in LOADERS ? variant : DEFAULT_SPINNER]
  return (
    <span role="progressbar" aria-label={props["aria-label"] ?? "Chargement…"} data-slot="loader"
