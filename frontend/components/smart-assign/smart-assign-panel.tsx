    ...
    <div className="mt-2 rounded-lg border border-border bg-muted/20 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-muted/30 border-b border-border">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-foreground">Smart Assign</span>
        </div>
        <button
          type="button"
      ...
      <div className="px-3 py-3 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-1">
          <Badge variant="secondary" className="text-[0.6875rem] h-4 px-1.5 bg-muted/60 border-0 text-muted-foreground">
            priority:{issuePriority.toLowerCase()}
          </Badge>
          {issueLabels.length > 0
            ? issueLabels.map((l) => (
                <Badge key={l} variant="secondary" className="text-[0.6875rem] h-4 px-1.5 bg-muted/60 border-0 text-muted-foreground">
                  {l}
                </Badge>
              ))
        ...
        {!loading && !ran && (
          <Button size="sm" className="h-7 w-full gap-1.5 text-sm" onClick={handleAnalyze}>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="rounded-md border border-primary/30 bg-primary/5 p-2.5 flex flex-col gap-2">
              ...
              {top.factors.length > 0 && (
                <div className="flex flex-wrap items-center gap-1">
                  {top.factors.map((f) => (
                    <Badge key={f} variant="outline" className="text-[0.6875rem] h-3.5 px-1 border-primary/20 text-primary/70">{f}</Badge>
                  ))}
                </div>
              )}
            ...
            {rest.length > 0 && (
              <div>
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground w-full py-0.5 transition-colors"
                >
                  ...
                  <div className="flex flex-col gap-1.5 mt-1.5">
                    {rest.map((candidate) => (
                      ...
                      >
                        ...
                        <span className="text-xs font-semibold text-muted-foreground shrink-0 group-hover:text-foreground">{candidate.score}%</span>
                      </button>
                    ))}
                  </div>
                )}
