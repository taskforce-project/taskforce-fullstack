...
type NotifTab = "all" | "mentions" | "alerts" | "assignments"
...
const MOCK_NOTIFICATIONS: Notification[] = [
  {
    ...
    type: "mention",
    read: false,
    actor: { name: "Sophie Martin", initials: "SM", color: "bg-violet-500" },
    project: "Website Redesign",
    projectUrl: "/projects/1",
    issueTitle: "Update hero section copy",
    issueId: "TF-41",
    issueUrl: "/projects/1/issues/41",
    timestamp: "2 min ago",
    comment: "Hey @you can you take a look at the design specs for this one?",
  },
  {
    id: "2",
    type: "assigned",
    read: false,
    actor: { name: "Lucas Dufour", initials: "LD", color: "bg-blue-500" },
    project: "Mobile App",
    projectUrl: "/projects/2",
    issueTitle: "Fix login screen crash on iOS 17",
    issueId: "TF-43",
  ...
  },
  {
    ...
    issueTitle: "Migrate authentication endpoints",
    issueId: "TF-38",
  ...
  },
  {
    ...
    type: "commented",
    read: true,
    actor: { name: "Emma Petit", initials: "EP", color: "bg-emerald-500" },
    project: "Website Redesign",
    projectUrl: "/projects/1",
    ...
    comment: "LGTM! Merging this tomorrow if no other comments.",
  },
  {
    id: "5",
    type: "dueSoon",
    ...
    project: "Mobile App",
    projectUrl: "/projects/2",
    issueTitle: "Sprint review preparation",
    issueId: "TF-51",
  ...
  },
  {
    ...
    type: "mention",
    read: true,
    actor: { name: "Thomas Bernard", initials: "TB", color: "bg-orange-500" },
    project: "API v2",
    projectUrl: "/projects/3",
    issueTitle: "Rate limiting strategy",
    issueId: "TF-22",
    issueUrl: "/projects/3/issues/22",
    timestamp: "Yesterday",
    comment: "Ping @you — what do you think about using sliding window?",
  },
  {
    id: "7",
    type: "statusChanged",
  ...
  },
  {
    ...
    project: "Mobile App",
    projectUrl: "/projects/2",
    issueTitle: "Onboarding flow redesign",
    issueId: "TF-12",
  ...
  },
  {
    ...
    project: "API v2",
    projectUrl: "/projects/3",
    issueTitle: "Write OpenAPI spec for /users endpoints",
    issueId: "TF-55",
...
function getTypeIcon(type: NotifType) {
  switch (type) {
    case "mention":
      return <AtSign className="h-3.5 w-3.5 text-violet-400" />
    case "assigned":
...
function NotificationItem({
  ...
  return (
    <div
      className={cn(
        "group flex items-start gap-3 px-4 py-3 transition-colors hover:bg-muted/40 border-b border-border/50 last:border-0",
    ...
    >
      ...
      <div className="flex-shrink-0 mt-0.5">
        {notif.actor ? (
          <Avatar className="h-8 w-8">
            <AvatarFallback className={cn("text-xs text-white", notif.actor.color)}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          {notif.actor && (
            <span className="text-sm font-medium text-foreground">{notif.actor.name}</span>
          )}
          <span className="text-sm text-muted-foreground">{getTypeLabel(notif.type, t)}</span>
          <Link
            href={notif.issueUrl}
            className="text-sm font-medium text-foreground hover:text-primary truncate max-w-[200px] sm:max-w-xs"
          >
            {notif.issueTitle}
          </Link>
      ...
      {/* Mark read button (visible on hover) */}
      {!notif.read && (
        <button
          onClick={() => onMarkRead(notif.id)}
          title={t("inbox.markRead")}
          className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground"
        >
...
function EmptyState({ tab, t }: { tab: NotifTab; t: (key: string) => string }) {
  const messages: Record<NotifTab, string> = {
    all: t("inbox.empty.all"),
    mentions: t("inbox.empty.mentions"),
    alerts: t("inbox.empty.alerts"),
    assignments: t("inbox.empty.assignments"),
  }
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden [box-shadow:var(--shadow-sm)]">
        ...
        <div className="flex items-center border-b border-border bg-muted/30">
          {TABS.map(({ key, icon: Icon }) => {
            const tabNotifs = notifications.filter(TABS.find((t) => t.key === key)!.filter)
            ...
            return (
              ...
              >
                ...
                {tabUnread > 0 && (
                  <Badge
                    variant="secondary"
                    className="h-5 min-w-5 px-1.5 text-xs bg-primary/15 text-primary border-0"
                  >
                    {tabUnread}
                  </Badge>
                )}
