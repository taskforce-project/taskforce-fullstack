...
import React, { type CSSProperties, useEffect, useId, useRef } from 'react';
...
const useInstanceId = (): string => {
  const id = useId();
  return `shadowoverlay-${id.replaceAll(':', '')}`;
};
    ...
    >
      <div
        style={{
          ...
          filter: animationEnabled ? `url(#${id}) blur(6px)` : 'blur(6px)',
          opacity: 0.95,
        }}
      >
        {animationEnabled && (
