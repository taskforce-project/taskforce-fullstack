-- =====================================================================
-- TaskForce — Seed DE DEMO « Demo Workspace » (PROD, idempotent, re-runnable)
-- =====================================================================
-- But : peupler un workspace de demo realiste rattache au compte de Pierre
--       (le proprietaire reel) pour la soutenance / le post LinkedIn :
--       Smart Assign, board, equipes, KPIs, inbox, billing (plan reel).
--
-- Idempotence : tout le contenu vit dans le workspace slug 'demo'.
--   Re-run = on purge ce workspace (cascade) + l'ancien 'nimbus', puis on
--   reconstruit. Les utilisateurs de demo (users) persistent (upsert par email).
--
-- Les coequipiers sont des comptes DATA-ONLY (keycloak_id 'seed-...', domaine
--   @seed.taskforce.dev) : ils ne peuvent pas se logger et sont EXCLUS des
--   metriques produit (cf. monitoring/vm1/metrics/grafana-metrics.sql).
--
-- IMPORTANT (difference avec dev_seed) : on NE TOUCHE PAS au compte de Pierre.
--   Pas de mutation Stripe/abonnement sur son user (section 17 supprimee) : la
--   page Billing de la demo montre son plan REEL.
--
-- Lancer (sur la VM1) :
--   docker exec -i taskforce-postgres-prod \
--     psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" < prod_demo_seed.sql
--
-- Prerequis : le compte pierre.michel.work@gmail.com doit exister en prod.
-- =====================================================================
...
BEGIN
    -- ----------------------------------------------------------------
    -- 0. Proprietaire = compte REEL de Pierre (resolu par email)
    -- ----------------------------------------------------------------
    SELECT id INTO v_admin FROM users WHERE lower(email) = 'pierre.michel.work@gmail.com';
    IF v_admin IS NULL THEN
        RAISE EXCEPTION 'Compte pierre.michel.work@gmail.com introuvable en prod — connecte-toi une fois avant de semer.';
    END IF;

    -- ----------------------------------------------------------------
    -- 1. Reset des workspaces de demo (cascade : projets, issues, equipes, profils...)
    --    On purge le nouveau slug 'demo' ET l'ancien 'nimbus'. Les notifications
    --    ne portant pas toujours de cascade, on les nettoie explicitement d'abord
    --    (scope = workspaces de demo uniquement, l'inbox reelle de Pierre est intacte).
    -- ----------------------------------------------------------------
    DELETE FROM notifications
     WHERE workspace_id IN (SELECT id FROM workspaces WHERE slug IN ('demo','nimbus','taskforce-demo'));
    DELETE FROM workspaces WHERE slug IN ('demo','nimbus','taskforce-demo');
    -- Donnees non rattachees au workspace (purge re-run dediee)
    ...
    -- 17. (SUPPRIME EN PROD) Le dev_seed mutait ici le compte admin (stripe_customer_id +
    --     abonnement BUSINESS + historique). En prod le proprietaire est le compte REEL de
    --     Pierre : on ne touche NI a son plan NI a son etat Stripe. La page Billing de la demo
    --     reflete donc son abonnement reel. Ne pas reintroduire de mutation sur v_admin ici.
    ...
    RAISE NOTICE 'Seed DEMO PROD complet : workspace "Demo Workspace" (slug demo, id=%) rattache a Pierre — 9 membres + 24 solos (hors equipe), 4 projets, board realiste (KPIs/capacite/burndown), commentaires, worklogs, cycles, notifications, favoris, pages, invitations, demandes enterprise, echeances reparties. AUCUNE mutation du compte/billing de Pierre.', v_ws;
END
$seed$;
