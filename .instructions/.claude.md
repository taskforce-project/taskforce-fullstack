# 🤖 Instructions Claude Sonnet 4.6 - Projet Taskforce

> **Fichier de configuration pour l'IA Claude Sonnet 4.6**  
...
## ⚡ Agent Performance & Analysis Strategy

## Repository Exploration Rules

### Default Mode: Targeted Analysis

Always assume the user wants the smallest valid analysis scope.

Do not perform repository-wide scans unless explicitly requested.

Before searching, identify:

- The feature concerned
- The frontend module concerned
- The backend module concerned

Limit analysis to those modules first.
...
1. Files explicitly provided by the user
2. Files directly referenced by imports
3. Files directly referenced by routes
4. Files directly referenced by dependencies
5. Repository-wide exploration (last resort)

---

### File Read Budget

For standard investigations:

- Maximum initial read: 10 files
- Maximum extended read: 30 files
...
- Which additional files would need inspection

---

### Stop Condition

Stop analysis immediately when:

- The root cause is identified
- The missing dependency is identified
- The broken route is identified
- The compilation issue is identified

Do not continue exploring unrelated modules.

---

### API Investigation Strategy

For frontend/backend consistency audits:
...
Do not inspect repositories, entities, infrastructure, tests or unrelated modules unless required.
...
- \*_/_.ts
- entire backend scan
- entire frontend scan

Repository-wide searches require explicit justification.

---

### Re-read Prevention

If a file was already analyzed during the current conversation:

Do not reopen it unless:

- New information contradicts previous findings
...
Repository-wide exploration allowed.

Architecture Audit must only be triggered when explicitly requested.

Never automatically escalate from Small Audit to Full Audit.
...
Assume architecture is stable unless evidence suggests otherwise.
...
Never continue scanning only to eliminate every uncertainty.

Prefer fast actionable conclusions over exhaustive exploration.

---

### Knowledge Persistence

When discovering architecture, API contracts, module mappings or recurring patterns:
...
instead of rediscovering the same information in future sessions.

Documentation should become the primary source of truth for recurring analysis tasks.
...
Check whether architecture documentation already exists.

Priority:

1. .ai/api-contracts.md
2. .ai/architecture-map.md
3. API_SPECIFICATION.md
4. Existing project documentation
5. Source code exploration

If the answer exists in documentation,
do not rediscover it from source code.
