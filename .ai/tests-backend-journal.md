...
## Packages 0% → 100% (02/07) : 85,1 % — gate 0,82

- **85,1 % ligne hors-brain (5300/6226), 645 tests, 0 échec.** Gate `jacoco:check` BUNDLE LINE **≥ 0,82** vert. Les 4 packages demandés (0 % → **100 %**) :
  - `modules.chat.api` **100 %** : `ChannelControllerWebMvcTest` (list/create canaux, list/send/edit/delete messages + 401 + 400 @NotBlank) + `ChatWebSocketControllerTest` (unit STOMP `@MessageMapping` : diffusion `/topic/channel.{id}`, principal null / non-numérique ignorés). NB `ChannelKind` = DM/GROUP_DM/PROJECT_CHANNEL (pas de GENERAL).
