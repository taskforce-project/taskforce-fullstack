...
- **Progression coverage** : 26,8 % (brut) → **31,7 %** (exclusions) → **34,2 %** (+ProjectService 6/6) → **37,1 %** (+IssueService étendu, 16/16) — **207 tests verts**. Prochaines grosses cibles : controllers `core.api` (716 l), puis services 0 % testables (`UserService` 94, `TeamService` 75, `MemberSkillProfileService` 92, `WorkspaceInvitationService` 107, `DiscussionService` 55, `MemberLeaveService` 47, `PageService`).
