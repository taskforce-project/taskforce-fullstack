...
> - **Live + revue** : `it.ps1 -Test ALL` vert (988). **Start prouvé live contre Linear** (découverte + DCR + URL d'autorisation ; `redirect_uri` localhost accepté). **Revue OWASP Top 10 faite** (conforme) : durcissement **A08/A10** = `authorization_endpoint` (metadata d'un serveur semi-fiable) validé `SsrfGuard` (public http(s)) avant la redirection navigateur - fix `fix/mcp-oauth-authz-ssrf`. Note mineure non bloquante : suppression du `state` sur erreur dans la même transaction (survit jusqu'à l'expiry sur rollback ; single-use + code AS consommé).
> - **Reste** : config prod (`redirect-base`/`app-base` = URLs publiques) ; le tour complet consentement + callback (compte Linear du user) ; déploiement `dev→main`. Caveat DCR : pas universel (fallback : coller un token).
