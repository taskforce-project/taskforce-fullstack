...
> - **Gates** : `tsc --noEmit` vert, `vitest run --coverage` vert (seuils par-fichier OK), build landing vert. **Reste** : appliquer Grafana + seed en SSH (bloqué sur Tailscale offline) ; vérifier `TF_TURNSTILE_*`/`TF_HUMAN_CHALLENGE_*` dans `.env.prod` (VM1) ; dead code `comparison-table.tsx` (chip).
