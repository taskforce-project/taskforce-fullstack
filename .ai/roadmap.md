...
> **▶ MAJ 25/09/2026, 3e passe (livré en prod : produit v0.40.1) - Runner local : délégation jamais bloquée, flag durable, logo Anthropic retiré.** `[backend + frontend + landing + ops]`
> - **Décisions CEO déléguées** (« t'as mon autorisation partout, prends les meilleures décisions ») sur les 3 points ouverts : logo Anthropic, flag du runner en prod, stash obsolète.
> - **Bug trouvé en vérifiant l'état réel de la prod** : flag du runner ALLUMÉ (override hors dépôt du 23/09, conteneur backend recréé le 23/09) → « Claude Code » proposé à TOUS les utilisateurs de prod ; sans runner, une délégation restait `QUEUED` pour toujours (`DeliveryRunner.refresh` ne relit que les runs `RUNNING`, `ClaudeCodeProvider.poll` ne détecte que le runner PERDU après un heartbeat). Smart Assign ne recommande PAS Claude Code (préférences cursor / github-copilot / claude-api / stub, toujours disponibles) : le piège exige un choix manuel.
> - **Correctif** : `delivery.local-runner.claim-timeout-minutes` (15 par défaut, `LocalRunnerSettings`) + `DeliveryAgentProvider.claimTimeout()` (null = jamais ; `ClaudeCodeProvider` le lit de la config) ; `DeliveryRunner.refresh` clôt en échec un run pull encore `QUEUED` au-delà du délai, par UPDATE CONDITIONNEL (`DeliveryRunRepository.expireUnclaimed` : encore `QUEUED` et sans runner, donc un claim concurrent gagne toujours), puis issue → « Blocked » ; message : « No runner picked this task up within 15 min. Start the TaskForce runner on your machine, then delegate the task again. The local runner is in early access. » Un runner qui s'allume avant toute relecture prend encore la tâche (utile). ⚠️ Trou rattrapé avant release en relisant le chemin RÉEL : `DeliveryController.latestRun` n'appelait `refresh` que pour un run `RUNNING`, donc la clôture ne se serait jamais déclenchée (invisible aux tests unitaires du runner) ; le contrôleur relit maintenant aussi les runs `QUEUED`, sur la fiche d'issue ET dans la liste du panneau (`listRuns`), tests `@WebMvcTest` dédiés. Choix écarté : disponibilité par utilisateur (runner en ligne), qui recouvre le WIP « Connect a runner » du CEO.
> - **Flag durable** : `DELIVERY_LOCAL_RUNNER_ENABLED: ${DELIVERY_LOCAL_RUNNER_ENABLED:-false}` dans `docker-compose.prod.yml` (backend) + `true` dans `.env.prod` de la VM1 : reprise à chaque rebuild (l'auto-deploy passe `--env-file .env.prod`), override `~/taskforce-runner-flag.yml` supprimé après vérification.
> - **Logo Anthropic** (conditions d'Anthropic relues dans l'ADR-013 : pas de logo sans permission) : règle `NAME_ONLY` au seul point de rendu de chaque front. App : `frontend/components/ui/brand-logo.tsx` → initiales (Claude Code, Claude API, clé Anthropic des réglages), test `brand-logo.test.tsx`. Site : `landing-page/src/components/site/BrandLogo.tsx` (`logoSrc()` renvoie `null`) → glyphe neutre dans le mur « Works with » et la table de runs du hero, initiales « AC » dans le catalogue d'intégrations. Le backend garde `logoKey = "anthropic"` (clé de marque, commentée). Les autres logos de marque relèvent de leurs propres règles, non vérifiées.
> - **Stash obsolète** (`frontend/tsconfig.json`, créé par moi, contenu déjà dans HEAD) supprimé, patch gardé dans le scratchpad ; le stash du 08/07 (WIP CEO) intact.
> - **Vérifié** : 65 tests backend ciblés verts (dont 4 cas paramétrés + message) puis suite complète `it.ps1 -Test ALL` verte (1311 tests, 0 échec, 1 ignoré) ; flag posé dans `.env.prod` de la VM1 (sauvegarde `.env.prod.bak-20260925-runner-flag`) AVANT la release ; front 36 tests liés verts, eslint propre ; landing lint 0 erreur, build vert, aucun `/logos/anthropic` dans le HTML construit, mur et table rendus avec glyphe 18×18, tuile catalogue « AC ». Compose validé (`config`) : `"true"` si la variable est posée, `"false"` sinon.
> - **Livré (25/09)** : 7 commits → PR #321 vers `dev` (labels `backend|frontend|landing:release:patch`, CI verte dont tests backend, CodeQL 0 alerte) ; release commit-tree → PR #322 vers `main`, `--admin --squash` (`f98bf781`) ; tags `backend-v0.10.2`, `frontend-v0.16.1`, `landing-v1.3.1`, `taskforce-v0.40.1`, `dev` et `main` au même arbre. **Vu en prod** : site en `v0.40.1`, aucun `/logos/anthropic` (home, intégrations) ; VM1 : conteneur backend recréé par l'auto-deploy à 17:08 (après le commit), healthy, `DELIVERY_LOCAL_RUNNER_ENABLED=true` venu de `.env.prod`, override `~/taskforce-runner-flag.yml` supprimé ensuite.

> **▶ MAJ 25/09/2026, 2e passe (livré en prod : landing-v1.3.0, produit v0.40.0) - Runner local sur le site, Enterprise sur un écran, fin de la passe ponctuation.** `[landing]`
> - **Demande CEO** : « je valide tout » (dont l'allègement d'Enterprise) + « le truc de runner, y a un truc à pousser sur le site ? ». Constat : le runner (ADR-013) tourne en prod derrière un flag (activé pour le test du CEO, provisionnement par script ; « Connect a runner » et le plugin sont du WIP non commité), mais le site ne disait nulle part comment la délégation se fait.
> - **Local runner (Beta, accès anticipé)** : section `#local-runner` sur `/product/orchestration` (4 étapes réelles Assign / Claim / Build / Review, tableau « What the agent can touch » repris du README du runner, mode sans dépôt, consigne glissée bornée par le périmètre, prérequis, « TaskForce never sees your Claude credentials »), lien depuis la carte Handoff et depuis Smart Assign, item Beta dans « Next » de `/roadmap`, ligne dans `llms.txt`, phrase dans la FAQ de la home (JSON-LD compris). Conditions d'Anthropic (README du runner) : Claude Code nommé en texte seulement, jamais comme nom de fonctionnalité, sans logo. Cursor / Copilot non mentionnés dans la section (non vérifiés en live).
> - **Enterprise sur un écran** : `Proof.tsx › Trust` en deux colonnes (texte + 2 listes de garanties | vraie vue d'audit) ; les 3 affirmations « attributable / recorded / logged » deviennent la légende de la preuve, la colonne « Monitoring & oversight » est la vue d'audit elle-même. Mesuré à 1440×900 : bloc 1,87 → 0,90 écran, home 12,3 → 11,3 (≈ 10 sur la base de mesure d'hier). Cadre d'audit en 4:3 : 8 lignes de 45 px (35 px en 1024), 5 lignes sous `sm` (le 16:9 tronquait les lignes sur téléphone).
> - **Fin de la passe ponctuation** : la passe du 25/09 avait traité les attributs `lead=` mais pas les paragraphes `<p class="t-lead">` : 38 leads + 2 `support=` de CalloutBand corrigés (virgule, deux-points, point ou parenthèses, aucun mot changé sauf la faute « then let you »), contrôle par script = 0 restant.
> - **Vérifié** : lint 0 erreur (6 warnings préexistants), `tsc` OK, `astro build` vert, classes émises (`max-sm:hidden`, grille `1.1fr`, `scroll-mt-20`), HTML des pages touchées contrôlé, mesures DOM à 1440 / 1024 / mobile (0 débordement, 0 scroll horizontal). Version produit pré-bumpée 0.39.0 → 0.40.0 dans la PR, pour que `dev` et `main` restent alignés.
> - **Livré (25/09)** : 6 commits → PR #319 vers `dev` (label `landing:release:minor`, → `landing-v1.3.0-rc1`), dont l'entrée « Local runner » du groupe Beta de `/changelog` ; branche release commit-tree → PR #320 vers `main`, `--admin --squash` (`9d9e509d`). Tags `landing-v1.3.0` et `taskforce-v0.40.0` créés, backend ignoré, **`dev` et `main` au même arbre** (le pré-bump a évité le désalignement de la 1re passe). **Vu en prod** sur taskforce-project.fr : footer `v0.40.0`, légende d'audit du bloc Enterprise, FAQ, section `#local-runner` (titre, tableau, lien Handoff), roadmap, changelog, lien Smart Assign, `llms.txt`, leads corrigés, sitemap 91 URLs. ⚠️ Les tags locaux périmés `landing-v2/3/4.0.0-rc1` n'existent pas sur le serveur (le calcul de la stable lit les tags du serveur) : vérifier avec `git ls-remote --tags`, pas `git tag -l`.

> **▶ MAJ 25/09/2026 (livré en prod, landing-v1.2.0, produit v0.39.0) - Polish complet du site façon relevanceai.com (design, UI, UX, SEO).** `[landing]`
> - **Demande CEO** : « peaufiner le site complet, s'inspirer de Relevance ». Audit mesuré d'abord (92 pages, rendu à 390 / 768 / 1024 / 1280 / 1440 px) : base saine (0 lien mort, alt OK, pas de scroll horizontal) mais H3 en 9 tailles, 38 tirets de liaison sur la home, logos « Works with » illisibles, home à 14,3 écrans, ~50 metas > 160 car. CEO a validé les 4 emprunts à Relevance + les fusions et doublons proposés AVANT coupe.
> - **Hero scindé + table live + onglets par rôle** : `Hero.tsx` (titre à gauche / promesse + CTA à droite dès xl, coupes H1 maîtrisées, sous-titre sans tiret, logos logo + NOM en `eager`) + nouvelle île `HeroRuns.tsx` (`client:idle`) : onglets Engineering / Product / Operations / Client services (Radix Tabs), table de runs dont la ligne du haut tourne (travail → « Awaiting your approval » → réglé), mention « Illustrative » (règle d'honnêteté de `story.ts`), figée si reduced-motion, statut empilé sous le titre en mobile. Remplace la capture statique. `BrandLogo` prend `loading`.
> - **Home 14,3 → 11,8 écrans** (validé) : Problem + Before/After fusionnés (`BeforeAfter.astro`), Smart Assign dans « What ships today » (carte pleine largeur + vraie capture), bandeau Memory dans « Why it's different », « The direction » dans le bas de « Who it's for » ; doublons retirés (4 puces entreprise de The run, 2e piste d'audit / cellule Verify). `SmartAssign.astro` supprimé, `Problem` / `BrainTeaser` / `WhereThisGoes` retirés. Pour ~10 : alléger The run (2,0) ou Enterprise (1,9), NON fait (hors validation).
> - **Titres révélés au scroll** : `src/scripts/reveal-titles.ts` (chargé par `BaseLayout`) + CSS `.reveal-words .rw` : chaque mot de `h2.t-h2` s'allume selon `--p` (0 → 1). Amélioration progressive (sans JS = noir), rien si reduced-motion, pages `/legal/` exclues, `[data-no-reveal]` pour exclure, page trop courte / bas de page = révélé. Piège réglé : poser `--n`/`--p`/classe AVANT de découper, sinon flash noir → gris au chargement. Plancher d'opacité **0,55** (3,67:1 sur `--secondary`, conforme AA grand texte) : le 0,22 d'origine donnait 1,59:1, corrigé avant la release.
> - **Typo** : tokens `t-h4` (16 px) / `t-h5` (14 px) dans `global.css`, 48 titres de cartes convertis (script) ; H1 de /pricing passé en `t-h1` + `t-lead`. **Copy** : 7 titres, 43 leads, 20 metas sans tiret de liaison (ponctuation seule, aucun mot changé). **Mobile** : la frise de The run tombait au zoom 0,32 (textes de 4 px) → `StepChainFlow` prend `stackOnMobile` (verticale sous 640 px, remontage si rotation). **SEO** : 30 metas réécrites ≤ 155 car. (max mesuré 155), `metaDescription` dédiée pour les fiches MCP, `/docs` (redirection noindex) retiré du sitemap.
> - **Vérifié** : lint 0 erreur (6 warnings préexistants : AutoAssign, LabsCurtain, StatBand), `tsc` OK, `astro build` vert (94 pages), tokens bien émis dans `dist/_astro/*.css`, 91 URLs du sitemap en 200, table live + onglets testés, reveal testé (`--p` = 0,962 attendu 0,96). ⚠️ Le panneau de preview masqué ne rend pas d'image : rAF / IntersectionObserver / transitions y sont figés, tester l'animé quand il est affiché.
> - **Derniers correctifs (25/09, demande CEO « fais les derniers correctifs et envoie en prod »)** : tableau des 7 checkpoints de The run passé en `<details>` fermé par défaut (contenu conservé et indexé) → home **11,0 écrans**. Code mort supprimé (0 importeur vérifié) : `home/{Hero.astro,Features,Today,RunTimeline,Platform,Narrative,DecisionGraph,Showcase}`, `illustrations/RunTimeline`, `Placeholder` ; `TeamGrid` extrait dans son propre fichier ; export `Integrations` de Proof retiré. Build 94 pages vert, lint = les 6 warnings préexistants.
> - **Versioning (cause du tag raté de #316)** : sur `main`, le stable = dernière rc de `dev` sans suffixe ; les pushs directs sur `dev` ne font que `rc+1` sur la même base, la rc restait à `v1.1.5-rc36` → stable `v1.1.5` déjà existant. Correctif sans toucher au pipeline : suivre le flux documenté, PR `feature/* → dev` labellisée `landing:release:minor` (→ `v1.2.0-rc1`), puis promotion `dev → main` (→ `v1.2.0`).
> - **Livré (25/09)** : 9 commits atomiques → PR #317 vers `dev` (label `landing:release:minor`, → `landing-v1.2.0-rc1`), puis branche de release commit-tree (arbre exact de `dev`) → PR #318 vers `main`, `--admin --squash` (`702eda94`). Le bot a bumpé la version produit 0.38.1 → **0.39.0** sur la branche de release ; release `landing-v1.2.0` taguée + GitHub Release, images landing et frontend construites, backend ignoré. **Vu en prod** sur taskforce-project.fr : table « Delivery runs » + onglets, « Show the seven checkpoints », script de révélation inliné, `.t-h4` / `.reveal-words` dans le CSS, nouvelle meta, footer `v0.39.0`, sitemap 91 URLs sans `/docs`. **Réalignement** : `dev` était resté en 0.38.0 (le bump ne vit que sur la branche de release depuis #316) → commit `chore(version)` sur `dev` pour que la prochaine release parte de la version réellement en prod.
> **▶ MAJ 24/09/2026 (livré, PR #316) - Hero de la home repositionné pour se démarquer de Dust (test des 5 s).** `[landing]`
> - **Constat** (analyse concurrentielle CEO, dust.tt) : Dust = plateforme d'agents HORIZONTALE (40 M$, 3000+ orgs, 70+ connecteurs). Notre hero sonnait générique/Dust-adjacent : eyebrow « The AI delivery operating system » + sous-titre sans nommer les agents de code -> confusion en 5 s pour un visiteur venu de Dust. La copie PROFONDE est déjà nette (/vs, comparisons.ts, « missing layer between tracker and coding agent ») ; seul le hero était faible.
> - **Fait** (`Hero.tsx`, l'actif ; Hero.astro = legacy mort) : eyebrow -> « The governed layer above your coding agents » (dit le wedge : AU-DESSUS des agents, gouverné) ; sous-titre réécrit pour mener par la division du travail et NOMMER les agents : « Your coding agent writes the code. TaskForce turns the outcome into the spec, plan and prompt it runs (Claude Code, Cursor, Copilot) - a human approves every step, and the reasons are remembered. »
> - **Non touché** (volontaire) : H1 « Describe the outcome. TaskForce runs the delivery. », logos transverses (voulus 14/09), CTA register (signup ouvert). **Livré** : commit `888a9160` → `dev`, PR #316 (branche release commit-tree, `landing:release:patch`) mergée `--admin --squash`, vu en prod. ⚠️ Versioning : le calcul a produit `landing-v1.1.5` / `frontend-v0.16.0` déjà existants (stream RC `v1.1.5-rc36` désynchronisé du stable) → aucun nouveau tag ni Release ; image frontend re-poussée à cause de `frontend/product-version.json` (comportement habituel des releases landing).
> **▶ MAJ 23/09/2026 (WIP, non commité) - Adoption du runner : plugin Claude Code interactif + « Connect a runner » libre-service.** `[taskforce-plugin + backend + frontend]`
> - **Plugin Claude Code (`taskforce-plugin/`, nouveau)** : mode INTERACTIF « tire tes tâches TaskForce dans ton Claude Code ». `.claude-plugin/{plugin.json,marketplace.json}` + commande `/taskforce` + README. Le bloc `userConfig` collecte le jeton utilisateur + workspace + URL et les injecte au serveur MCP `taskforce-mcp` SANS en-tête de délégation (donc en tant que l'utilisateur, avec ses droits). `claude plugin validate` OK. Bémol v1 : jeton d'accès collé (court) faute d'endpoint « jeton personnel durable » ; publier `taskforce-mcp` sur npm permettra le `npx`.
> - **« Connect a runner » backend (libre-service, ADR-013)** : `RunnerProvisioningService` (core/service/delivery) porte les 5 étapes de `ops/kc-runner.sh` via le client admin Keycloak existant (`KeycloakConfig.keycloakAdminClient()`) : client `tf-runner-u<id>` en client_credentials, rôle porté par le compte de service, mapper propriétaire = e-mail de l'APPELANT authentifié (jamais une valeur cliente). Endpoint user `GET/POST /api/delivery/runners` (`RunnerProvisioningController`, HORS préfixe machine `/api/delivery/runner/`), gaté par le flag. DTOs `RunnerProvisionResponse`/`RunnerStatusResponse` ; test unitaire des builders (client_credentials-only, mapper propriétaire). **Reste** : l'écran front (panel Agents, `agents-panel.tsx`) + rebuild backend (pas de JDK sur le poste) + tests front (seuils de couverture par fichier).

> **▶ MAJ 23/09/2026 - Runner active en PROD + mode « sans depot » pour les taches non-code (ADR-013).** `[ops + taskforce-runner]`
> - **Active en prod** (le flag etait faux), pour le test E2E du CEO, entierement en SSH depuis le poste. Identite `tf-runner-pierre` (realm `taskforce-prod`, proprietaire `pierre.michel.work@gmail.com`) via `ops/kc-runner.sh`. Flag pose par un override compose HORS depot (`~/taskforce-runner-flag.yml`) : le backend prod plumbe ses env vars explicitement (pas `env_file`), et editer le fichier tracke figerait l'auto-deploy (`git merge --ff-only`) ; l'override s'auto-desactive au prochain rebuild backend. **2 faits de runbook corriges** : le backend n'ecoute PAS sur `100.122.50.25:8080` (pas de nginx sur VM1, sortie par tunnel Cloudflare) -> le runner vise `https://api.taskforce-project.fr/api` ; SSH VM1 depuis le poste marche (Tailscale check-mode). `npm run check` tout vert contre prod.
> - **Mode sans depot** : le 1er vrai test (TF-28 « cree un doc sur le bureau ») a echoue sur le garde-fou « projet sans depot ». Demande CEO : TaskForce n'est pas que du code. Ajoute un chemin parallele dans le runner quand `claim.repoFullName` est nul : dossier temporaire jetable (`<home>/scratch/run-<id>`) au lieu d'un worktree, **aucun outil Bash/git**, brief dedie (`buildReplyPrompt`) qui fait rendre le travail par `taskforce_add_comment` (deja dans le perimetre delegue) et ignore toute consigne « ecris sur la machine ». Jamais de PR, jamais un fichier hors du bac a sable. Flag `acceptRepoless` (defaut true) pour un runner code-only. **Zero modif backend/MCP.** 90 tests verts, tsc propre. Ecrire sur la vraie machine (bureau) = agent computer-use, ecarte volontairement. Refs : `taskforce-runner/src/{index,agent,prompt,config}.ts`, `test/runner.test.ts`.
> **▶ MAJ 22/09/2026 - Outillage pour activer le runner en PROD (ADR-013).** `[ops]`
> - `ops/kc-runner.sh` : equivalent Linux/prod de `scripts/keycloak-runner.ps1`, provisionne le compte de service runner dans le realm `taskforce-prod` (role `delivery-runner`, client `tf-runner-<name>` en client_credentials, mapper `tf_runner_owner`), calque sur `ops/kc-setup.sh`, idempotent, affiche le secret une fois. Activation prod = ce script sur VM1 + `DELIVERY_LOCAL_RUNNER_ENABLED=true` dans `.env.prod` + recreate backend ; runner du poste pointe sur l'API prod (tailnet `100.122.50.25:8080`). Reste desactive par defaut ; usage perso du CEO (aucun user actif).
> **▶ MAJ 22/09/2026 - Release v0.38.0 : CodeQL a arrete la release, corrige sans bypass.** `[backend + ci + securite]`
> - **Bout-en-bout REEL valide d'abord** : issue RUNNER-1 deleguee a Claude Code (vrai `claude` 2.1.278, abonnement du CEO), depot `Miche1-Pierre/runner-sandbox` clone, CONTRIBUTING.md ecrit, **PR #1 ouverte**, issue en « In review by AI », rien de merge. 45 s, 12 tours.
> - **Puis la CI de la PR de release (#313) a bloque sur CodeQL** : 7 nouvelles alertes (1 critique). Je n'ai PAS bypasse. Corrige : (1) **log-injection** dans `DeliverySessionFilter` (code deploye) -> helper `forLog` qui neutralise les sauts de ligne du chemin journalise ; (2) les 6 alertes du **runner** (command-injection sur le lancement `.cmd`, http->file, file->http credential) sortent du perimetre : `taskforce-runner/` est un **outil local non deploye** (paquet npm distinct, jamais dans les images ni sur les VM), mis en `paths-ignore` d'une config CodeQL dediee, avec justification. Le lanceur garde le motif cross-spawn (verbatim + arguments proteges), seul robuste pour un `.cmd` Windows a arguments a espaces (teste : le passage en tableau casse les chemins) ; entrees operateur ou validees (modele, nom de depot).
> - **A tracer aussi (dette)** : le runner reste hors CodeQL -> ne pas y relacher la vigilance ; durcissement du lancement de processus a suivre si le runner devient une vraie feature produit (ADR-013).
> **▶ MAJ 21/09/2026 - Projet + dépôt sans friction : « Connect GitHub » en un clic dans la création de projet, et le runner clone le dépôt tout seul (non releasé).** `[frontend + backend + taskforce-runner]`
> - **Demande CEO** : « tu crées un projet, ça te demande si tu veux créer un dépôt », et Claude a accès à ce dépôt-là. **Constat** : l'essentiel existait (intégration GitHub du workspace en OAuth `repo,read:org`, section « Code repository » du dialogue avec New repo / Link existing, livrées en v0.2x), mais sans GitHub connecté le dialogue renvoyait vers Settings et l'on perdait le formulaire ; et le runner exigeait de déclarer à la main le chemin local de chaque dépôt. **Backend** (`BE-AGENT-DELIVERY-1`) : `GET …/integrations/github/connect?returnTo=` ; le chemin de retour est gardé sur l'état OAuth (migration **V89** `oauth_states.return_to`) seulement s'il est sûr (`shared/security/SafeReturnPath` : relatif à l'application, borné au workspace du flux, sans schéma, `//`, antislash, `@`, `%`, `..` ni caractère de contrôle), et le callback y renvoie au lieu de Settings. **Frontend** (`FE-AGENT-DELIVERY-1`) : bouton « Connect GitHub » dans la section, brouillon (nom, clé, description) mis de côté en `sessionStorage` le temps de l'aller-retour, retour sur `?newProject=repo` qui rouvre le dialogue, « New repo » présélectionné et nom de dépôt proposé depuis le nom du projet. **Runner** : `autoClone` (défaut vrai) ; un dépôt non déclaré est cloné à la demande par `gh repo clone` dans `~/.taskforce-runner/repos/<owner>/<name>` avec les droits GitHub de la personne, branche par défaut détectée, nom `owner/name` validé strictement avant de devenir un chemin et un argument.
> - **Vérifié** : suite backend complète verte (1303 tests, 0 échec), dont 30 cas `SafeReturnPathTest` et 2 cas d'intégration sur vrai Postgres (chemin gardé ou écarté, retour au bon endroit) ; front `tsc` + `eslint` + 890 tests verts, seuils respectés ; runner 83 tests, et clone automatique éprouvé pour de vrai sur un dépôt de l'organisation (clone, branche par défaut, réutilisation). **Non vérifié** : le parcours OAuth GitHub à l'écran (il demande le compte GitHub du CEO) et un vrai run de Claude Code. **MAJ même jour** : CLI Claude Code 2.1.278 installé par le CEO (premier essai « not a valid application » : le paquet natif de ~240 Mo n'était pas arrivé, réinstallation = réparé) ; vérifié hors réseau que le runner le trouve (`npm run check` tout vert) et que son jeu complet d'options est accepté, y compris à travers le lanceur `claude.cmd` de npm (`--max-turns` masquée de l'aide mais acceptée, une option inconnue est refusée). Reste : sa connexion à Claude, puis le premier run réel. **Répondu au CEO** : créer une ORGANISATION GitHub par API est impossible sur github.com ; créer le dépôt dans une organisation existante est faisable (`POST /orgs/{org}/repos`, `read:org` déjà demandé) et reste à faire ; réutiliser le login GitHub pour l'intégration n'est pas souhaitable (le login ne demande que `user:email`, exiger `repo` à toute connexion serait disproportionné).
> **▶ MAJ 21/09/2026 - Historique des délégations dans le panneau « AI Workflows » (non releasé).** `[frontend + backend]`
> - **Constat CEO** après le test du runner local : « pourquoi je ne vois pas les workflows passés ? ». Le panneau ne listait que les analyses ; les délégations n'existaient que dans le canvas, sous la forme « Issue #1080 », sans titre, non cliquables, et chargées une seule fois à l'ouverture. **Backend** (`BE-DELIVERY-RUNS`) : `DeliveryRunResponse` porte désormais `issueKey`, `issueTitle`, `projectId`, `projectName` (le lien profond du board exige le projet) ; `findByProjectIds` charge l'issue et son projet dans la même requête (`JOIN FETCH`), sinon chaque run en coûterait une de plus. **Frontend** (`FE-AGENT-DELIVERY-HISTORY`) : section « Delegations » en tête du panneau (clé + titre de l'issue, logo et nom de l'agent, projet, ancienneté, statut ; dépliée : résumé ou erreur, « Open issue », « View result »), au-dessus des « Analyses » ; le bouton de la topbar charge les délégations au montage, compte les actives dans son badge et poll tant qu'une est active (il relit le run de l'issue, ce qui fait avancer son état côté serveur, puis la liste) ; dans le canvas, les runs affichent clé + titre et ouvrent l'issue au clic (reste à faire noté le 11/09, fait).
> - **Vérifié** : suite backend complète verte (1270 tests, 0 échec), dont un nouveau test d'intégration sur vrai Postgres `DeliveryRunRepositoryIntegrationTest` (5 : liste bornée + ordonnée + issue et projet chargés, plafond de page, file du runner bornée au provider et au délégant, **claim atomique à un seul gagnant**, signe de vie accepté du seul runner qui tient le run) ; front : `tsc` + `eslint` + suite avec couverture vertes (885 tests, 85 % de lignes), dont un test de rendu du panneau `workflow-dock.test.tsx` (8 : panneau vide, délégation lisible, dépliée avec résumé + lien + ouverture de l'issue par le lien profond, échec, active, cas dégradé sans projet, agent inconnu, ordre des sections). **Non vérifié à l'écran** : la session du navigateur de test avait expiré et l'agent ne saisit pas de mot de passe ; l'aspect visuel reste à regarder sur la stack dev. Aucune migration.
> **▶ MAJ 20/09/2026 - Runner local de délégation : « Assign to Claude Code » sur le poste de la personne (ADR-013, prototype, non releasé).** `[backend + taskforce-mcp + taskforce-runner + keycloak]`
> - **Demande CEO** : tester Claude Code en local depuis TaskForce, avec une identité Keycloak propre et un périmètre de droits strict. **Backend** (`BE-AGENT-DELIVERY-RUNNER`) : le provider `claude-code` devient « pull » (`DeliveryAgentProvider.pullBased()`), le run reste QUEUED jusqu'au claim atomique d'un runner ; migration **V88** (`claimed_by`, `claimed_at`, `heartbeat_at`) ; endpoints machine `POST /api/delivery/runner/{claim, runs/{id}/heartbeat, runs/{id}/result}` ; jeton machine relayé par `POST /api/auth/runner/token` (Keycloak n'est pas joignable de l'extérieur en prod) ; runner muet 10 min = run en échec. **Identité** : client confidentiel `tf-runner-<nom>`, `client_credentials` seul, rôle `delivery-runner`, propriétaire signé dans le jeton (`tf_runner_owner`) ; script idempotent `scripts/keycloak-runner.ps1`. **Session déléguée** (`DeliverySessionFilter`, fail-closed) : l'agent agit au nom du délégant, ses droits pour plafond, resserrés à la lecture du workspace du run (liste fermée), l'écriture des issues du projet du run, jamais de suppression. **taskforce-mcp 0.3.0** : `taskforce_get_issue`, `taskforce_add_comment`, en-tête de session. **taskforce-runner/** (nouveau) : claim, worktree git isolé, `claude -p` en mode `dontAsk` avec liste fermée d'outils, puis push + PR par le runner, jamais de merge. **Aucun shell** : les processus sont lancés par tableaux d'arguments (constat Semgrep du premier jet corrigé à la source : sous Windows un lanceur `.cmd` passe par `cmd.exe` avec des arguments protégés ou refusés, les commandes `setup` sont des tableaux, le nom de modèle venu du serveur est validé).
> - **Vérifié sur la stack dev** : suite backend complète verte (`it.ps1 -Test ALL` : 1264 tests, 0 échec) ; 59 tests du runner ; CI verte sur `dev` (tests backend, CodeQL, Semgrep) ; 34 sondes HTTP en direct conformes ; un run complet jusqu'à « In review by AI » et le chemin d'échec jusqu'à « Blocked ». **Non vérifié** : l'agent était un substitut (`test/fixtures/stub-agent`), le CLI Claude Code n'étant pas installé sur le poste ; restent à éprouver les options de `claude -p` et l'ouverture de PR par `gh`. **Garde-fous** : `delivery.local-runner.enabled` vrai en dev, **faux en prod** ; côté Anthropic, chacun fait tourner SON Claude Code non modifié avec son propre compte (abonnement ou clé API, `agent.auth`), TaskForce ne voit jamais ces identifiants : c'est la forme admise par leurs conditions, pour le CEO comme pour tout utilisateur (première rédaction trop stricte, corrigée le 20/09 après relecture de la page officielle ; avant mise sur le marché : Commercial Terms, usage du nom et du logo, confirmation écrite). **Limites connues** dans l'ADR-013 (jeton de session par run à faire, pas d'expiration de file, commentaire de l'agent au nom du délégant).
> **▶ MAJ 20/09/2026 - UI anglaise cohérente : restes français retirés (v0.37.1).** `[frontend + backend]`
> - Repérés en recapturant les visuels du site. **Front** : les cartes du dashboard sont codées en dur (elles ne passent pas par `constants_*`) et gardaient des restes français (« Actives / Ouvertes / À risque », « N ouvertes », « +N autres opérations », légende « sain · à risque · en pause », libellés de séries de `spec-chart`) : traduits sur place. **Back** : `SmartAssignService.buildFallbackReason` composait ses raisons en français alors que le prompt du LLM, donc ses raisons, est en anglais : repli passé en anglais, avec un test qui fige la langue. Reste connu : l'i18n FR/EN complète du dashboard (FE-CORE-004) n'est pas faite.
> **▶ MAJ 20/09/2026 - Grammaire de densité app + site, visuels et image Docker du site (v0.37.0).** `[frontend + landing]`
> - Demande CEO : tester la « grammaire Linear » proposée par ChatGPT (dark + violet). Décision après diagnostic : **clair + bleu conservés**, on joue sur tailles / bordures / densité. **App** (PROD-8.11) : tokens `--control-height*` 28 / 32 / 36 + `--row-height`, utilitaires `h-control` / `h-row`, tailwind-merge étendu, 9 primitives + 41 contrôles de pages, lignes de liste 45 -> 37 px. **Site** (PROD-9.10) : même système en 32 / 36 / 40, formulaire de démo unifié à 40, deux redéclarations CSS silencieuses corrigées (`--radius`, 5 niveaux d'ombre), visuels d'app recapturés (le zoom montrait un panneau Smart Assign retiré du produit), `Dockerfile` de prod en Node 22 (le job Release Services échouait). README racine : description du vrai produit (fin du template ERP) + marqueurs BADGES. Branche `origin/v2` périmée supprimée (intégration = `dev`). Contrat LLM : `.ai/design-grammar.md`.
> **▶ MAJ 14/09/2026 - Fiches detaillees des connecteurs MCP-ready (v0.36.7).** `[landing]`
> - Demande CEO : « enrichis les fiches detaillees des MCP-ready ; sur le site tout doit etre a jour par rapport a l'app, si ca fonctionne faut l'inscrire ». Avant : fiches SEULEMENT pour les 3 natifs (GitHub/Slack/Plane), les 25 MCP-ready = tuiles non-cliquables. **Fait** : chaque MCP-ready a desormais sa **fiche generee HONNETE** (`mcpFiche(c)` dans `connectors.ts`, rendue par la route dynamique `[key].astro`, reutilise le layout `ConnectorDetail.astro`). Contenu genere depuis la VRAIE data du catalogue (rien d'invente) : tous les MCP-ready marchent pareil (serveur MCP officiel hebergé 1-clic OAuth + l'agent appelle leurs outils dans un run, approbation humaine sur toute ecriture), differencies par nom/desc/categorie/URL MCP reels. Matrice Connect=live / Remember=rolling / Act=beta ; section « honest about the edges » (ingestion native pas cablee hors Plane ; outils exacts = ceux du serveur MCP du fournisseur). Les 3 natifs gardent leurs fiches riches (route statique prime sur `[key].astro`). Tuiles catalogue cliquables si `native || mcp` (`hasFiche`). `connectors-data.ts` enrichi (catLabel/mcpUrl/docsUrl/websiteUrl) via `scripts/generate-connectors.mjs`. **28 fiches** au build (3+25), sitemap etend (fiches MCP derivees du catalogue). Verifie en live (DOM) : fiche Linear complete + honnete, tuile cliquable, GitHub garde sa fiche riche. Build Node 22 vert.
> **▶ MAJ 14/09/2026 - Catalogue d'integrations HONNETE, cale sur l'app reelle (v0.36.6).** `[landing]`
> - Retour CEO : « la page integrations est pas a jour du tout, faut reprendre ce que j'ai dans l'app et ce qui fonctionne VRAIMENT ». La page vendait « 138 connecteurs » + 12 outils en avant comme s'ils marchaient tous. **Realite etablie** (catalogue backend + `ConnectorCatalog` + capabilities live) : **3 natifs profonds** (Plane -> ingestion Brain OS ; GitHub/Slack -> actions natives, capability `act`), **25 MCP-ready** (`mcpSuggestedUrl` = serveur MCP hebergé verifié -> 1 clic, utilisable par l'agent), **~104 connectables** (stocke la cle, MCP generique possible), **6 « recommend »** = libs UI (shadcn/radix/magic-ui… `authType NONE`) qui ne sont PAS des connecteurs.
> - **Spec CEO** : montrer TOUTES les integrations ; joignable -> nom+logo + **badge « comment se connecter »** (MCP/OAuth/API key/Token/Config) + description ; **pas de moyen de connexion aujourd'hui -> nom + logo SEULS** (sans badge ni description, on ne survend pas). **Implemente** : nouveau `src/lib/connectors-data.ts` **genere** depuis le catalogue backend (`scripts/generate-connectors.mjs`, 138 connecteurs enrichis auth/mcp/native/desc, fleches `→` et tirets longs assainis) ; `IntegrationCatalogue.tsx` **reecrit** (data externalisee) : vue defaut = « Working today · 3 native + 25 MCP-ready », badge de connexion + description par tuile, natifs marques Memory/Actions + fiche cliquable, non-joignables en nom+logo seuls, « Show all 138 » + recherche + filtre 17 categories. Lead + legende de `integrations.astro` reformules honnetement (plus de « connect them in minutes... into your runs » qui sous-entendait que les 138 marchent). **Verifie en live (DOM)** : tuiles natives/MCP correctes, filtre « UI components » = 6 libs nom+logo seuls. Build Node 22 vert. Fiches detaillees inchangees (GitHub/Slack/Plane).
> **▶ MAJ 14/09/2026 - Logos des nouveaux connecteurs + doc Fern rafraichie (v0.36.5).** `[landing + docs]`
> - **Logos** : fetch cible SVGL des 9 nouveaux connecteurs -> **4 recuperes** (Webflow, shadcn/ui, Radix UI, Magic UI) dans `landing-page/public/logos/` + `frontend/public/logos/` ; **5 absents de SVGL** (Square, Wix, 21st.dev, Aceternity UI, Origin UI) -> restent en initiales (fallback honnete). Cles ajoutees a `frontend/scripts/fetch-logos.mjs` (le script complet fait `rm -rf` + refetch TOUT -> j'ai fait un fetch cible pour eviter le churn). ⚠️ le script derive ses cles d'une **liste hardcodee** (pas du backend) -> a completer a la main quand le catalogue grossit.
> - **Doc Fern** (demande CEO « enchaine sur Fern ») : **decouverte** que Fern etait **deja entierement scaffolde** (24/08) dans `fern/` (docs.yml avec **custom-domain `docs.taskforce-project.fr`**, 30+ pages FR, API ref, scripts `generate-openapi.sh` + `clean-openapi.mjs`). Le seul manque = **spec OpenAPI perimee** (179 chemins vs 203 live). Rafraichie via le pipeline officiel -> **197 chemins** (203 - 6 actuator), 43 sections renommees/ordonnees. `fern check` = **0 erreur** (15 warnings benins : pages template supprimees + contraste du bleu ajuste par Fern). **Reste (cote CEO)** : `fern login` (compte Fern) -> `fern generate --docs` (publie sur `taskforce.docs.buildwithfern.com`) -> configurer le custom domain cote Fern + **CNAME `docs` -> Fern** chez OVH. ⚠️ springdoc est **desactive en prod** -> regenerer le spec depuis le backend DEV (`docker compose ... up backend` puis `./fern/scripts/generate-openapi.sh`).
> **▶ MAJ 14/09/2026 - Landing : hero transverse + resync catalogue integrations 138/17 (v0.36.4).** `[landing]`
> - Demande CEO : diversifier les logos du hero (« montrer qu'on touche a tout ») + lier le mot « tools » vers integrations + « mettre a jour la partie integration ». **(1) Hero** (`Hero.tsx`, le .tsx est l'actif ; Hero.astro = legacy) : `BUILT_WITH` passe de 6 logos tous IA+dev (Claude/Cursor/OpenAI/Ollama/GitHub/Linear) a **8 logos TRANSVERSES, un par categorie** (GitHub repo, Linear tracker, Slack chat, Notion docs, Figma design, AWS cloud, Datadog observabilite, Claude modeles) ; le mot « tools » de « Works with the tools… » devient un lien vers `/product/integrations`. **(2) Resync catalogue** : le backend `ConnectorCatalog` a grossi a **138 connecteurs / 17 categories**, la landing affichait **129 / 16** (perime). Verifie via l'endpoint autoritaire `GET /api/workspaces/{slug}/integrations/catalog` (token admin) -> `total:138`. Ajout des **9 manquants** dans `IntegrationCatalogue.tsx` (TOOLS) : nouvelle categorie **UI components** (shadcn/ui, 21st.dev, Radix UI, Aceternity UI, Magic UI, Origin UI) + Square (Payments) + Webflow/Wix (Design). Compteurs `TOOLS.length`/`CATS.length` = dynamiques (auto 138/17) ; copy codee en dur « 129 … 16 categories » corrigee partout (integrations.astro x3, nav.ts, changelog, roadmap.astro, Proof.tsx). **(3) Bug** : la section Integrations de la home (`Proof.tsx`) liait « Browse all integrations » vers `/integrations` (**route inexistante, lien mort**) -> corrige en `/product/integrations`. Build Node 22 vert ; hero logos rendus, integrations page « 138 connectors across 17 categories », TOOLS=138/CATS=17. ⚠️ **logos absents pour les 9 nouveaux** (fallback initiales, comme les autres manquants) -> option : `npm run logos` (frontend, SVGL) pour les recuperer.
> **▶ MAJ 14/09/2026 - Landing : cadre d'audit Enterprise rempli (v0.36.3).** `[landing]`
> - Retour CEO (capture, cadre entoure en rouge) : « il manque un screen ici ». La maquette « audit » de la section Enterprise (`Proof.tsx` / `Trust`) etait un `AppShot` au **corps VIDE** (placeholder « a remplir avec une capture », jamais posee). Rempli avec une **vraie vue d'audit statique** dans la DA du site (points de statut colores, `divide-y`, horodatages mono, chips modele) : feed de 8 evenements attribuables (approbations, refus « sent back », appels modele, assignation) qui **remplit le cadre** (lignes en `flex-1` -> aucun vide quel que soit le ratio) + barre de resume (« 5 of 7 checkpoints approved · immutable · exportable to your SIEM »), toast « Approved · CTO » conserve au-dessus. Modeles honnetes (claude-sonnet-5/opus-5, gpt-oss-20b self-hosted ; PAS de llama-3.3 decommissionne). Build Node 22 vert. ⚠️ visuel non capture (fenetre derriere -> screenshots blancs cette session), a eyeballer en live.
> **▶ MAJ 14/09/2026 - GEO/LLM landing : llms.txt + BreadcrumbList.** `[CERTIF-SEO / landing]`
> - Suite du volet SEO, cap sur la visibilite LLM (demande CEO « faut pousser le GEO »). **(1) `public/llms.txt`** (convention llmstxt.org) : resume honnete de l'entite « TaskForce = AI Delivery Operating System » + liens produit/ressources, avec mention explicite de la maturite (live/beta/planned -> roadmap) pour ne rien survendre aux LLM. **(2) `BreadcrumbList` JSON-LD** auto sur toutes les pages internes (`breadcrumbLd` dans `structured-data.ts`, branche dans `BaseLayout` via `Astro.url.pathname`) : libelles tires de la vraie nav (`nav.ts` = source unique ; ex. `/product/brain-os` -> Home > Product > **TaskForce Memory**, pas le slug), fallback segment connu puis slug embelli ; rien sur la home. C'est ce type de donnee (pas Organization/SoftwareApplication) qui remplit la section « Ameliorations » de Search Console. **Verifie** : build Node 22 vert ; home = 3 JSON-LD (sans breadcrumb), `/product/brain-os` = Organization + WebSite + BreadcrumbList (3 ListItem) ; `llms.txt` servi. Deploy landing-only -> main (v0.36.1). **Contexte Search Console (CEO)** : propriete Domaine `taskforce-project.fr` verifiee, sitemap soumis (67 URLs, « Operation effectuee »), home+pricing+product en file d'indexation. ⚠️ **redirection Vercel inversee** : l'apex sert desormais le site et `www` -> 308 -> apex (le canonical non-www du code matche enfin le host servi). **FAQPage (fait, v0.36.2)** : section FAQ VISIBLE en bas de home (6 Q/R honnetes alignees sur la maturite reelle : live/beta/planned) via `FaqSection.astro` (deja utilise sur enterprise), qui emet le JSON-LD `FAQPage` depuis les MEMES items -> markup = contenu visible (regle Google) + bon pour le GEO. Home = **4 blocs JSON-LD** (Organization + WebSite + SoftwareApplication + FAQPage). **Reste (option)** : documentation sur **Fern** (prochain chantier, demande CEO).
> **▶ MAJ 14/09/2026 - SEO/GEO landing : donnees structurees JSON-LD + fixes techniques.** `[CERTIF-SEO / landing]`
> - **Demande CEO** « bosser sur le referencement du site ». Audit de la surface SEO reelle (landing Astro). **Deja solides** : canonical dynamique, OG/Twitter, robots.txt + sitemap auto (72 routes derivees de `BUILT_ROUTES`), titres/desc par page ancres sur l'entite « AI Delivery Operating System ». **Trous corriges** :
> - (1) **Aucune donnee structuree JSON-LD** -> nouveau `src/lib/structured-data.ts` (source unique de l'entite) : `Organization` (logo, `founder`=Pierre+LinkedIn, `contactPoint`) + `WebSite` sur **toutes** les pages via `BaseLayout` ; + `SoftwareApplication` (applicationCategory `DeveloperApplication`, subCategory « AI Delivery Operating System », `offers` free 0 EUR) sur la home (prop `software`). **Pas** d'`aggregateRating` (jamais d'avis fabrique). `sameAs` **laisse vide** tant que les profils org reels/possedes (GitHub org, X) ne sont pas confirmes - un mauvais `sameAs` brouille la desambiguisation de l'entite (l'inverse du but).
> - (2) **`/og-image.jpg` inexistante** (chaque partage + preview LLM = image cassee) -> nouvelle `public/og-image.png` 1200x630 (2x retina) generee via **Playwright** (chromium en cache), on-brand : marque bleue, « The AI Delivery Operating System », domaine, fil Plan/Draft/Approve/Ship. (3) `og:image` **relative -> absolue** (`new URL(image, Astro.site)`) + `og:site_name`/`og:locale`/dimensions/`alt`. (4) `manifest.json` **perime** (« beautiful themes », bordeaux `#9b2c2c`, icones `icon-192/512.png` inexistantes) -> refait (bleu `#2563eb`, `favicon.svg`) + `<link rel="manifest">` ajoute au `<head>`. (5) sitemap : `<lastmod>` + `<priority>` (home 1.0, legal 0.4).
> - **Verifie** : build Astro **Node 22** vert (one-shot conteneur `node:22-slim`, source copiee hors node_modules hote). `dist/index.html` = **3** blocs JSON-LD (Organization+WebSite+SoftwareApplication, avec Offer/Person/ContactPoint) ; `/product` = **2** (entite partout, Software reserve a la home) ; `og:image` absolue ; sitemap avec lastmod. ⚠️ **`astro dev` local bloque par une policy Application Control Windows** (binding natif `satteri` refuse) ; le conteneur `taskforce-landing-dev` est sur **Node 20** incompatible Astro 7 (meme cause que le fix CI Lighthouse) -> son `Dockerfile.dev` reste a passer en Node 22 (chantier a part).
> - **Reste** : completer `ORG_SAMEAS` avec les profils org reels (attente CEO) ; release **landing-only -> main** (sans attribution Claude) ; actions Search Console cote CEO (verif propriete, soumettre `sitemap.xml`, Inspection d'URL). Refs : `landing-page/src/lib/structured-data.ts`, `src/layouts/BaseLayout.astro`, `public/og-image.png`, `public/manifest.json`, `src/pages/sitemap.xml.ts`.

> **▶ MAJ 11/09/2026 - Refonte UX assignation façon Linear (A-suite) : reco fondue dans le menu Assignee + multi-assign en plan + heuristique lit le titre.** `[FE + BE-AGENT-DELIVERY]`
> - **Demande CEO** (screenshots du panneau Smart Assign a barres + du dialog multi-assign a cases/scores) : « c'est pas du tout elegant comme multi assign, comment ils auraient fait Linear ? mm le smart assign individuel ? ». Reponse Linear (cf. `linear.app/docs/assigning-issues`, `linear.app/docs/select-issues`) : assigner = **une action rapide, au clavier, dans un petit menu** ; la reco est une **ligne « Suggested » en tete du meme menu**, jamais un panneau de scores ; le lot = plan a valider, pas un tableau a cocher score-par-score.
> - **Individuel** : nouveau **`components/smart-assign/assignee-menu.tsx`** = `Popover` + `Command` (cherchable) qui remplace le `Select` + l'etoile ✨ + le `SmartAssignPanel` dans l'issue-sheet. Sections **Suggested** (la reco IA vit ICI : ligne « Suggest best assignee » qui lance `smartAssignIssue` **a la demande** - pas d'appel LLM metre a chaque ouverture -, puis top + 2 alternatives avec visage/logo + « pourquoi » en sous-titre, personne OU agent), **Members** (+ « No assignee »), **Delegate to an agent** (logos). Choisir une personne = assignee, un agent = delegation. Le gros panneau de barres (Semantic/Workload/Available) est **supprime** : `smart-assign-panel.tsx` ne garde que le helper `MatchReasoning` (encore utilise par le dialog de creation d'issue).
> - **Multi-assign** : **`bulk-assign-dialog.tsx`** refondu en **PLAN a valider** (issue -> qui) au lieu du tableau a cases + badges de score (18-55% affiches = embarrassant + bruite). Chaque ligne = identifiant + titre + -> + suggestion (visage/logo + prenom, « pourquoi » au survol) ; on **ecarte** une ligne d'un clic (skip au survol, restaurable), puis **« Assign N »**. Plus de cases a cocher, plus de scores, resume discret « N of M · P people · K agents ». Logique d'application inchangee (agent->`delegate`, humain->`updateIssue`).
> - **Backend - l'heuristique lit le TITRE** : beaucoup d'issues n'ont pas de label mais un titre explicite (« Bug: ... », « Refactor ... ») donc aucun agent n'etait jamais propose. `agentSuitability(labels, **issueText**, priority, points, hasSpec, repoLinked)` : si aucun label mecanique mais un **token** mecanique dans le titre/description (`bug/fix/refactor/typo/docs/test/lint/bump/...`) -> **+15** ; un token de jugement humain (`design/architecture/strategy/roadmap/...`) -> **-20** (malus, PAS un veto ; seul un **label** humain veto). Match par token (pas substring -> pas de « prefix »->« fix »). Facteur « Routine change » ajoute. `SmartAssignAgentSuitabilityTest` etendu (`@ParameterizedTest` sur le titre, 11 cas). **Backend SmartAssign 46/46 verts** (full `it.ps1 -Test ALL` en cours), front **tsc + eslint verts**, aucun test front ne referencait les composants touches. `on ne touche pas le metier` : Smart Assign etendu, jamais casse.
> - **Reste** : verif live (l'app tourne mais le compte dev n'a pas de workspace seede) ; puis release `v2->dev->main`. Chantiers suivants **B** (onboarding -> Brain OS) et **C** (Brain OS facon Notion + passe UX Linear).
> - **B1 - Projet -> Brain OS (11/09)** : `BE-AGENT-DELIVERY` / chantier B slice 1. La creation d'un projet ecrit desormais une **fiche de contexte** dans le graphe (le trou §0 : « aucun chemin d'ecriture ne partait de l'activite projet »). Reutilise l'idiome Phase 4bis : `ProjectCreatedEvent` publie dans `ProjectService.createProject` (nouveau `ApplicationEventPublisher`, sur pattern Cycle/Issue) -> `BrainIngestionListener.onProjectCreated` (`@Async @TransactionalEventListener(AFTER_COMMIT)`) -> `BrainIngestionService.writeProjectNode` (upsert `refType=PROJECT`, domaine `PROJET`, type `NOTE`, `metadata.projects=[id]`). **AUCUN LLM** (le contexte est la `description` ecrite par l'equipe, pas de paraphrase) -> gratuit/instantane. Bonus : le node `refType=PROJECT` **ancre enfin la region du projet** dans le graphe (Phase 4ter en avait besoin). Tests `BrainIngestionServiceTest` (+5 : create/update/vanished/no-desc/no-llm). Gate cible **72/72 vert** (BrainIngestion + ProjectService integ + ProjectController WebMvc + CreateProjectTool). Le nouveau publisher n'a casse aucun test (ProjectService teste en integration + slice, comme Cycle/Issue).
> - **B2 - Contexte entreprise/activite -> Brain OS (11/09)** : chantier B slice 2 (decisions CEO : **question libre** « que fait ton entreprise / equipe ? », captee **a l'onboarding ET au dialog New workspace**). **Migration V87** `workspaces.activity VARCHAR(500)` (colonne = editable en reglages + coherent avec l'idiome event = ids, le listener relit l'entite). `NodeRefType.WORKSPACE` ajoute (enum varchar, `BrainEnums.valueOf` la parse tel quel). `CreateWorkspaceRequest`/`UpdateWorkspaceRequest`/`WorkspaceResponse` + entite gagnent `activity`. `WorkspaceService.createNewWorkspace`/`updateWorkspace` posent l'activite + publient `WorkspaceContextEvent` (si non vide) -> `BrainIngestionListener.onWorkspaceContext` -> `writeWorkspaceNode` (upsert `refType=WORKSPACE`, domaine `PRODUIT`, node **transverse** sans `metadata.projects` -> « Base commune » au centre du graphe). Sans LLM. **Front** : `activity` dans les payloads + type `Workspace` ; champ dans le dialog `team-switcher` (i18n `shell.workspaceActivity*` FR/EN) + etape « Team » de l'onboarding (`updateWorkspace {name, activity}` a la fin). Le 1er espace (cree au signup) recoit son activite via l'onboarding ; les suivants via le dialog. Tests `BrainIngestionServiceTest` (+4 workspace) ; gate cible **106/106 vert** (dont `IntegrationSocleTest` = V87 validee par le socle, `WorkspaceServiceIntegrationTest`, `AuthServiceTest`). Front **tsc + eslint + vitest workspace 35/35** verts. **Reste** : suite complete + release `v2->dev->main` ; (option) editer l'activite dans les reglages General ; puis **C** (Brain OS facon Notion).
> - **C - Brain OS facon Notion (11/09)** : demande CEO (« la partie brain OS, faudrait que ce soit comme un Notion » + « UI/UX inspire toi de Linear »). Surprise a l'orientation : la vue **page etait deja par defaut**, le graphe deja secondaire. Le vrai manque = un **arbre de pages** hierarchique (`parentNodeId` existait de bout en bout mais **jamais settable** hors seeding ; 0 node parente). **Decision CEO** : les **16 domaines restent les racines**, les pages se nichent dessous (sous-page herite du domaine). **Editeur : reste markdown** (pas de bascule TipTap : le moteur de liens du graphe parse `#tags`/`[[wikilinks]]`, HTML le casserait). **C1 backend** : `parentNodeId` settable a la creation + au deplacement (`Create/UpdateKnowledgeNodeRequest` + `KnowledgeService`), garde meme-workspace (`requireNode`) + **anti-cycle** au reparentage (remonte les ancetres). **Aucune migration** (colonne `parent_node_id` deja la). Test `KnowledgeServiceTest` (5, dont self-parent + cycle refuses). **C2 frontend** : nouveau `components/brain/page-tree.tsx` = arbre recursif (domaines racines -> pages nichees), expand/collapse, `+ page`/`+ sous-page` au survol (dialog pre-rempli domaine+parent), **fil d'Ariane** des ancetres dans l'en-tete, auto-deplie le chemin de la selection, orphelins (parent filtre) remontent en racine -> rien ne disparait. Remplace la liste par-domaine ; recherche/tags/graphe inchanges. **C3 polish Linear** : titre **renommable inline** (Notion, clic -> Entree/blur sauve), titre `text-2xl`, et **pages Brain dans le Cmd+K global** (`command-palette.tsx` lit `useBrainStore.overview`, groupe « Brain pages ») + deep-link `/brain?node=<id>` consomme par la page (nettoie l'URL). Gates : back **KnowledgeService 5/5** + suite complete ; front **tsc + eslint (0 err) + vitest brain 28/28**. Pas de migration -> deploy code-only. Release groupee C1+C2+C3 (demande CEO « enchaine sur C3 puis release ensemble »). **Reste (C-suite, option)** : navigation clavier dans l'arbre, drag-to-nest, editeur par blocs qui emet du markdown.
> - **C-suite + B-suite (11/09)** : demande CEO « fais c suite et b suite ». **B-suite** : l'activite de l'espace est desormais **editable dans Reglages > General** (`settings/page.tsx`, champ Activity via `updateWorkspaceInfo` -> reecrit la fiche de contexte Brain via l'event existant). **C-suite drag-to-nest** : nouveau backend `PATCH .../brain/nodes/{id}/move` (`MoveNodeRequest{parentNodeId,domain}`, `KnowledgeService.moveNode`) - `parentNodeId` **null = racine du domaine** (EXPLICITE, contrairement a updateNode ou null=inchange) ; reutilise la garde anti-cycle ; deplacement structurel (ni re-embed ni re-sync). Front `page-tree.tsx` : lignes **draggables**, drop sur une page = sous-page (aligne le domaine), drop sur un domaine = racine ; garde anti-cycle cote UI (descendants bloques) + highlight de la cible. Store `moveNode` (avale l'erreur -> null) + service + route `NODE_MOVE`. **C-suite nav clavier** : roving focus dans l'ordre DOM (`data-tree-row`), fleches Haut/Bas = deplacer, Droite/Gauche = deplier/replier, Entree = ouvrir. Editeur par blocs = **non fait** (trop gros pour une suite, reste un chantier a part). Gates : back **KnowledgeServiceTest 7/7** (+2 move) + suite complete ; front **tsc + eslint (0 err) + vitest 866/866**. Pas de migration -> code-only. Release groupee (bump mineur).
> - **Fix : modals decales / voile qui ne couvre pas (mode daltonien) (11/09)** : retour CEO screenshot « ca depasse » sur l'Auto-assign - le modal etait decale a gauche et la board transparaissait a droite (non assombrie). Cause : `globals.css` posait le **filtre daltonien sur `<body>`** (`html.cb-* body { filter }`). Un `filter` cree un bloc conteneur pour les descendants `position: fixed` ; `<body>` grandit avec le contenu (`min-height:100svh` + board large) -> les portails Radix (voile + modal, rendus dans `<body>`) se calaient sur `<body>` au lieu du viewport -> decalage + voile incomplet. **Seul** declencheur possible de ce symptome (unique `filter`/`transform` sur body ; la regle dyslexie ligne 338 = font-only, inoffensive). **Fix** : filtre deplace sur un conteneur `#app-root` (nouveau `<div id="app-root">{children}</div>` dans `app/layout.tsx`) ; les portails (voile/modals/menus/toasts/cookie) vivent HORS de ce div -> leur `fixed` reste ancre au viewport. Contrepartie assumee : ces portails ne sont plus daltonises (bonus, le design gere deja le daltonisme par couleur+icone+libelle). tsc + eslint verts. Ne se declenchait qu'en mode daltonien actif.
> - **Fix : menu Assignee non-scrollable (11/09)** : retour CEO « je peux pas scroller mais j'aime l'affichage ». Le `CommandList` (default `max-h-75`=300px) ouvert bas dans le sheet debordait sous le viewport -> le bas de la zone scrollable etait hors ecran, inatteignable. Fix : borne le `CommandList` a `calc(var(--radix-popover-content-available-height,22rem) - 2.75rem)` (hauteur dispo Radix moins la barre de recherche) -> le menu tient toujours a l'ecran et scrolle a l'interieur (twMerge override le max-h par defaut). Durcissement au passage : rows du dialog Auto-assign passees en `max-h-[55vh] overflow-y-auto` (evite le meme debordement avec beaucoup d'issues). tsc + eslint verts.
> - **Fix : delegation a un agent non connecte -> toast ACTIONNABLE (11/09)** : retour CEO « si c'est pas connecte faudrait que je puisse le connecter en direct... un toast avec l'action, le gars clique et boom c'est connecte ». Avant : deleguer a Copilot sans GitHub creait un run en ECHEC avec un message rouge mort (friction). Maintenant : `issue-sheet` **pre-verifie la connexion requise** avant de deleguer (`checkAgentConnection`) et, si absente, montre un toast sonner avec un **bouton Connect** au lieu de creer un run en echec : **github-copilot** -> GitHub pas connecte -> « Connect GitHub » = OAuth **1 clic** (`integration-store.connectGitHub`, redirige) ; **claude-api/cursor** -> cle absente -> « Connect » ouvre le modal **Reglages > Agents** en place (`settings-store.openSettings("agents")`, monte global dans app-shell). Echec generique -> meme action de repli. Plus de run en echec pour le cas « pas connecte ». tsc + eslint + vitest 866/866 verts. (Reste option : meme pre-check dans le bulk Auto-assign.)
> - **Fix : menu Assignee compact + marge + backdrop modal (11/09)** : retour CEO (2 captures) « c'est colle au bord, mets de la marge » + « je veux un dropdown ou je peux scroller, pas un truc enorme » + le multi-assign « deborde ». **(1)** Le menu Assignee prenait toute la hauteur dispo (regression de mon cap `available-height`) et collait au bord de l'ecran. -> `assignee-menu` : cap **compact** `min(20rem, available)` (dropdown ~320px qui scrolle) + `sideOffset=6` + `collisionPadding=12` (marge de l'ecran). **(2)** Le multi-assign « deborde » : investigation -> **aucun** bug de stacking/portail/transform (les dialogs Radix portent dans `<body>`, aucun z-index > l'overlay z-50, modal centre). C'est la board **visible a travers un voile trop clair** (`bg-black/50`, texte clair lisible en dark), pas un vrai debordement (l'overlay `fixed inset-0` couvre tout le viewport ; le scroll horizontal de la board est un conteneur interne `overflow-x-auto`). -> **DialogOverlay `bg-black/50` -> `bg-black/65 backdrop-blur-sm`** : la board est assombrie ET floutee (recule sans ambiguite, facon Linear). Le modal est un frere de l'overlay (meme z-50, peint apres) -> jamais floute, aucun piege de containing-block. Rows du bulk `overflow-x-hidden` (durcissement). ⚠️ non reproduit en live (backend dev KO) -> a confirmer par le CEO (si la board reste a pleine luminosite / modal decale, ce serait un vrai bleed a diagnostiquer via `document.documentElement.className`). tsc + eslint verts.
> - **Chantier Brain OS : vraie hierarchie + graphe cliquable (11/09, EN COURS)** : retour CEO « t'appelles ca comme Notion ? c'est illisible... ca respecte aucune hierarchie... je peux meme pas cliquer sur les notes ». Cartographie faite (agent Explore). **Causes reelles** : (a) deux regroupements qui se contredisent - l'arbre groupe par **domaine**, le graphe groupe par **`metadata.projects`** (regions camembert) ; aucun n'est la containment Brain>Workspace>Projet ; (b) les noeuds workspace/projet ingerES sont **plats** (pas de `parentNodeId`) alors que la primitive existe (`V56`) ; (c) le clic du graphe : `si hasChildren -> ZOOME (n'ouvre pas), sinon -> ouvre` -> les noeuds parents etaient inouvrables (« que 2 ont des liens ») ; (d) les liens ne viennent que des `[[wikilinks]]` + `parentNodeId` + liens manuels (les `#tags` ne creent rien) -> connexions au coeur qui semblent arbitraires. **Decision modele (CEO) : vue AGREGEE au niveau compte** - Brain OS = couche de presentation au-dessus des workspaces, donnees cloisonnees par workspace (securite/tenant intacts, zero migration BDD lourde). **Slices** : (1) graphe cliquable = chaque noeud s'ouvre au clic [FAIT ici] ; (2) containment reelle `parentNodeId` (workspace racine > projets > notes/cycles, backend + backfill) + arbre aligne ; (3) archetypes par type de projet (tech/marketing/finance) + socle commun + vue Brain OS « compte » qui liste les workspaces. **Slice 1** : `brain-graph.tsx` `onClick` -> ouvre toujours la note (`ref` toujours pose) ; le zoom d'exploration reste a la molette + cadrage auto. tsc + eslint verts (composant canvas hors coverage unitaire). Refs : `brain-graph.tsx:673`, `BrainIngestionService.writeProjectNode/writeWorkspaceNode`, `V56__brain_node_hierarchy.sql`, `page-tree.tsx`.
> - **Kanban restructure facon Linear/21st, style TaskForce (11/09)** : demande CEO (capture d'un composant 21st.dev) « reorganise la structure du kanban comme ca, avec NOTRE style, et recode-le chez nous (pas de vendoring) ». -> `projects/[id]/page.tsx` : cartes enrichies (chip **priorite** teinte + labels + **note/description** + footer **identifiant / echeance (ambre si proche) / story points / assigne**), colonnes en **panneaux** (`bg-muted/40` + pastille couleur + compteur en pill + menu), bouton **Add task** par colonne (`CreateIssueDialog defaultStatusId`). Garde @dnd-kit et le raccourci de statut (survol). **Pas de progress ring** (aucun champ progression cote metier -> story points a la place, honnete ; si CEO veut une vraie progression = evol metier). Recode maison avec nos tokens (pas de squircle/Inter, pas de framer-motion ajoute). tsc + eslint + vitest 866/866 verts. **Reste** : page auth split-screen (form a droite, fond noisy maison, screen de l'app) - prochaine slice.
> - **Fix modal Auto-assign qui deborde + settings responsive (11/09, diagnostic EN LIVE)** : le CEO etant connecte, inspection de sa session reelle via l'extension Chrome (fini les suppositions sur captures). **(1) Modal bulk « ca sort du modal » = VRAI bug** (mes fixes precedents backdrop/overflow-x traitaient le symptome) : `DialogContent` est en `display:grid` sans `grid-template-columns` -> la colonne implicite se dimensionne au **max-content** d'un enfant (titre long non-secable) = **563px** et depasse `max-w-lg` (512) de 76px ; le `min-w-0 truncate` du titre ne sert a rien car une colonne `auto` ignore le min. **Fix : `[grid-template-columns:minmax(0,1fr)]` sur `DialogContent`** (global, verifie en live : overflowX 77 -> 0, titre tronque). Ne fait que clamper un debordement. **(2) Dropdown Assignee « toujours pas scrollable » = deja corrige en v0.25.3** : inspecte en live -> `max-h 320px` applique, `scrollHeight 494 > clientHeight 320`, `overflow-y auto`, **scroll a la molette OK** (revele la section « Delegate to an agent »). Le CEO etait sur un **cache HTML perime** (v0.25.2) -> hard refresh. Aucun changement code. **(3) Settings pas responsive** : le `/settings` ouvre un **modal** ; `FormField` etait `grid-cols-[180px_1fr]` sans breakpoint (label fixe ecrase le champ sur mobile) -> `grid-cols-1 sm:grid-cols-[180px_1fr]` (label au-dessus < sm) ; et la nav laterale du modal etait `hidden sm:flex` **sans alternative mobile** -> ajout d'un `<select>` de section (`sm:hidden`). tsc + eslint + vitest 866/866 verts.
> - **Fix scroll MOLETTE du dropdown Assignee (11/09, v0.26.2)** : **CORRECTION** de mon point (2) ci-dessus (« deja OK, cache perime ») - c'etait FAUX. Le CEO precise : « scroll molette KO, seuls le clic-molette et la scrollbar marchent ». Vraie cause : le sheet parent (Radix Dialog / shadcn Sheet) verrouille le scroll via **react-remove-scroll**, qui **preventDefault la molette** sur ce popover (portalise HORS du sheet = « hors zone autorisee »). Mon test live precedent scrollait via `scrollBy()` -> contournait le blocage natif = **faux positif**. Le clic-molette (autoscroll) et le drag de scrollbar ne passent pas par le listener wheel -> marchent, coherent avec le symptome. **Fix** : `onWheel` sur le `CommandList` qui scrolle la liste a la main (`scrollTop += deltaY`) ; react-remove-scroll ne fait que preventDefault (pas de stopPropagation), donc le handler React se declenche ; `AssigneeMenu` ne vit QUE dans le sheet (toujours verrouille) -> scroll natif deja bloque, pas de double-scroll. ⚠️ **non re-verifie en live** (session CEO expiree, login interdit) - diagnostic solide mais a confirmer par le CEO. tsc + eslint verts.
> - **Brain slice 2 (containment via gabarit) + reseed complet du demo (11/09, EN COURS)** : CEO « on termine le brain mtn... et fait le reseed complet du demo : onboarding des gens, du workspace, projets, Brain OS ». **Backend (code fait)** : (1) `BrainTemplateService.taskforceProjectSeed` - le hub « Brain OS » devient la **racine** (`key="hub"`) et chaque **projet est nested dessous** (`parentKey="hub"` au lieu de null) -> vraie containment Brain OS > Projets > notes (avant : projets = racines, relies au hub par simples wikilinks) ; (2) `KnowledgeService.getOverview` - amorcage paresseux **configurable** : `brain.demo-slugs` (defaut `demo`) -> le brain du demo se seed en **TASKFORCE** (riche, derive des 5 projets reels) a sa 1re ouverture, **sans** le reseed owner-only (que je ne peux plus appeler, session CEO expiree). **SQL seed (fait)** : `prod_demo_seed.sql` - workspace `activity` renseignee (onboarding-complet de l'espace) ; skills membres deja seedes (onboarding des gens OK, lignes 162/616). **Reste** : suite backend verte (en cours) -> release (recree les 2 VMs) -> re-appliquer les SQL seeds en prod (reconstruit le demo) -> ouvrir le brain demo (se remplit hierarchiquement) -> frontend arbre/graphe qui rendent la hierarchie (containment `parentNodeId`). NB : pas de migration Flyway (le demo est reseed a neuf via le gabarit). L'ingestion live (`parentNodeId` sur writeProjectNode/writeCycleNode) reste une slice separee (workspaces reels, pas le demo).
> - **Brain slice 2bis : graphe = CONSTELLATION radiale par containment (11/09)** : demo rempli (1011 noeuds TASKFORCE, containment verifie : hub 312 -> 5 projets -> systemes -> notes). Retour CEO « style ok MAIS arborescence illisible, j'arrive pas a retrouver Brain OS > workspace > projet, sous forme d'etoile/constellation ». Prototype constellation valide avec le CEO (« go, le rich, ca apparait au scroll »). **`brain-graph.tsx` reecrit** : le layout metaball-par-projet (`metadata.projects`) -> **arbre radial par `parentNodeId`** (rayon = profondeur, secteur angulaire ∝ feuilles ; Brain OS au centre, projets 1re couronne, systemes/notes en rayons ; 1 teinte par projet, descendants heritent). **Level-of-detail** : les notes (profondeur >=3) s'estompent en vue d'ensemble et se revelent au zoom (« le rich apparait au scroll »). Rendu deja radial-ready (paintNode LOD labels + titres courts dernier segment, drawLink dendrogramme radial) donc REUTILISE ; supprime : metaballContour/traceContour/packOffset/les regions (onFramePre/Post). Clic ouvre toujours la note. **Pas de reseed** (les 1011 noeuds ont deja la containment). tsc + eslint verts ; non verifiable live (session CEO expiree) -> review CEO. **Arbre de gauche (`page-tree.tsx`) refait (v0.28.1)** : retour CEO « l'arborescence est illisible, faut que ce soit regroupe par workspace et projet ». Avant : groupe par **domaine** (04·Engineering melangeait tous les « X › Engineering »). Apres : regroupe par **CONTAINMENT** (`parentNodeId`) comme le graphe - Brain OS (hub) racine -> projets -> systemes -> notes ; titres courts (dernier segment) ; hub deplie par defaut (defaut via `isExpanded` inverse, pas de setState-in-effect), le reste replie ; drag-to-nest + nav clavier conserves ; icone Folder/FolderOpen pour les conteneurs. Plus de dossiers de domaine. tsc + eslint verts.
> - **Brain slice 2ter : niveau ESPACE + fix racine (le cap ejectait l'ossature) (11/09, v0.29.0)** : retour CEO screenshot « c'est pas ca : Architecture transverse puis Engineering/Finance/... a plat et repetes, aucun dossier workspace/projet ; et dans le graphe je vois pas la separation espace/projets, et certaines notes/bulles ne sont pas cliquables ». **Cause racine CONFIRMEE par SQL** (pas une hypothese) : `KnowledgeService.getOverview` tronquait a `OVERVIEW_CAP=1000` une liste triee `domainAscTitleAsc` ; le demo a **1011** noeuds actifs et le domaine **PROJET** (le hub « Brain OS » + les 5 README de projet) trie **13e/17** -> tombe aux positions **1002-1009**, DONC **hors plafond** -> le front recevait 1000 feuilles **sans hub ni projets** : chaque systeme pointait un `parentNodeId` absent -> orphelins -> l'arbre applatit tout sous un pseudo-racine (« Architecture transverse »), et le graphe ne pouvait ni positionner ni cabler ces noeuds (= « certaines pas cliquables »). **Un seul bug expliquait les DEUX symptomes.** **Backend** : (1) `capStructureAware(ordered, cap)` - tronque en preservant le SQUELETTE (tout noeud reference comme `parentNodeId` par un autre : hub/espace/projets/systemes) puis complete avec les feuilles ; l'ossature n'est jamais larguee quel que soit le tri (test `KnowledgeServiceTest` +2) ; `OVERVIEW_CAP` 1000 -> **2000** (le demo passe entier). (2) **Niveau ESPACE** dans le gabarit `taskforceProjectSeed(projects, workspaceName)` : `Brain OS` (racine) -> **1 dossier d'espace** (`key="ws"`, nom = `workspace.getName()`) -> **socle « Transverse »** (`key="ws:transverse"`, contient Charte/Archi/Securite/Infra/Roadmap, reparentes) **+ 1 dossier par projet** (`parentKey` projet passe de `"hub"` a `"ws"`) -> archi (systemes) -> notes. Exactement la demande « root -> 1 dossier/workspace -> 1 dossier/projet -> archi ». **Frontend** : `page-tree.tsx` deplie par defaut la racine ET ses dossiers d'espace (containers de 1er niveau) -> on voit d'emblee Brain OS > espace > projets. `brain-graph.tsx` **relayout « coeur + galaxies »** : le COEUR (refId null : hub, espace, socle transverse) au centre, chaque **PROJET (groupe `refId`)** = un **secteur colore** (galaxie) autour, dispose radialement dedans ; **territoires** = couronnes translucides teintees dessinees derriere les noeuds (`onRenderFramePre`) + nom du projet au bord -> la separation espace/projets devient visible ; palette **categorielle** (hex distincts) au lieu de la rampe HSL (« code couleur moyen ») ; **filet** : tout noeud non place -> couronne externe (reste present + cliquable). La clicabilite universelle vient surtout du fix cap (l'ossature revient). Gates : back `it.ps1 -Test ALL` (en cours) ; front tsc + eslint verts. Non verifiable live (session CEO expiree). **Reste** : release `v2->dev->main` (recree les 2 VMs) -> vider les lignes brain du demo (ws 18) en prod -> le brain se re-seede a la 1re ouverture avec le niveau espace. Refs : `KnowledgeService.capStructureAware`, `BrainTemplateService.taskforceProjectSeed`, `brain-graph.tsx` (data useMemo + drawTerritories), `page-tree.tsx` (defaultOpen). **Deploye + demo reseede** : v0.29.0 sur les 2 VMs (VM1 20:20, VM2 20:21) ; niveau espace pose sur le demo par **restructuration chirurgicale SQL in-place** (pas clear+lazy : logged out, je ne peux pas declencher le lazy-seed) -> `Brain OS(312) > Demo Workspace(1323) > [Socle transverse(1324) > 5 docs] + [5 projets > systemes > notes]`, 1013 noeuds < cap 2000, verifie SQL. Le gabarit reste la source de verite pour tout futur reseed.
> - **Canvas React Flow du workflow IA (unifie) (11/09, v0.30.0 EN COURS)** : demande CEO « utilise React Flow (`@xyflow/react`) pour tout ce qui est workflow IA - delegation de tache a un agent ou plusieurs en mm temps ». Choix CEO = **canvas unifie** (plan d'analyse DAG **ET** delegations vers agents). **Backend** : `GET /api/workspaces/{slug}/delivery/runs` = runs de delegation du workspace (vue workflow), bornes aux **projets visibles** (`ProjectVisibilityGuard.viewableProjectIds`, jamais de fuite d'un projet prive) + plafond 200 ; `DeliveryRunRepository.findByProjectIds` (JPQL `r.issue.project.id IN`), `DeliveryService.listRuns`, `DeliveryController` `@GetMapping("/runs")`. Test `DeliveryServiceRunsTest` (+2 : filtre visibilite + court-circuit si 0 projet). **Frontend** : `@xyflow/react` ^12 ajoute (CSS importee dans `globals.css` APRES tailwindcss - ordre requis Tailwind v4) ; `delivery-service.listWorkspaceRuns` + `delivery-store.fetchWorkspaceRuns`/`workspaceRuns` (+ tests store 90% + service). Nouveau `components/workflows/workflow-canvas.tsx` = canvas React Flow, **layout deterministe** (1 job = 1 bande, taches en colonnes par niveau de dependance topologique anti-cycle ; region delegations issue->agent en dessous), noeuds custom aux tokens (theme auto via `.dark`), statut live (couleur/anim), MiniMap+Controls+Background, clic job -> page Analytics. `workflow-canvas-dialog.tsx` (grand overlay 92vw/86vh) ouvert depuis le dock « AI Workflows » (bouton « Open the workflow canvas »). **Testabilite** : le canvas se peuple des vraies donnees (jobs = launchAnalysis / runs = delegation) ; **un run live d'agent exige un provider connecte** (GitHub Copilot / Cursor / Claude API) - ni le CEO ni moi n'en avons cable un -> la partie delegation reste vide tant qu'aucune cle n'est connectee (pas de mock). Le canvas est **hors scope coverage** (include = lib/hooks/components-auth). Gates : back `it.ps1 -Test ALL` (en cours) ; front tsc + eslint + vitest delivery 23/23 verts. **Reste** : rafraichissement live des runs dans le canvas (polling), navigation issue au clic d'un run (besoin du contexte projet), release v0.30.0. **DEPLOYE v0.30.0 (#287)** sur les 2 VM (verifie live : bundle 0.30.0, containers recrees).
> - **Refonte page auth = ecran scinde (11/09, v0.31.0 - buildee, verifiee live, EN ATTENTE de ship)** : demande CEO (ancienne, deferee apres brain+reseed) « page login/signin en split-screen, FORM A DROITE, fond noisy recree en interne (pas de vendoring 21st.dev), un screen de l'app ». `app/auth/layout.tsx` passe d'une colonne centree a un **ecran scinde** : GAUCHE = panneau de marque sombre (gradient bleu + **grain recree en interne via feTurbulence**, aucune dep) + logo + titre/sous-titre produit + **capture reelle** `dashboard.webp` (copiee de landing-page, fenetre d'app qui deborde en bas) ; DROITE = le formulaire (login/register/forgot heritent via le layout partage), barre (theme + action opposee) + footer app. Responsive : < 1024px le panneau cede la place, formulaire pleine largeur, marque dans la barre. CSS auth reecrite dans `globals.css` (`.auth-shell` 2 colonnes, `.auth-formcol` 3 bandes, `.auth-aside*`, `.auth-brand-mobile`). i18n : `panelTitle`/`panelSubtitle` ajoutes (EN + FR). **Verifie LIVE** (pages auth publiques, `next dev` :3000) : rendu correct login + register + mobile (DOM inspecte : aside flex@1280 / none@375, image chargee avg-brightness 247, form centre) ; tsc + eslint + vitest auth **86/86** verts. ⚠️ **tagline = copie provisoire de moi** -> a valider par le CEO avant ship (page publique = 1re impression). **Reste** : valider la copie, puis release v0.31.0 (frontend-only, bump mineur) + deploy VM2. **DEPLOYE v0.31.0 (#288)** (VM2, verifie 0.31.0). **Iteration retour CEO (v0.31.1)** : logo TaskForce plus gros (h-16, non inverse) ; fond du panneau = la **vague du Lab** (`/assets/tour/labs-wave.jpg`) **floutee** (blur 46px) + **striped noise** (rayures fines + grain, multiply) au lieu du degrade bleu nuit (« pas les couleurs de TaskForce ») ; texte sombre (fond clair). Verifie live (DOM : logo 96x64, ::before = labs-wave floutee, noise multiply). Point CEO clarifie : GitHub OAuth **app deja creee cote GitHub** - il manque juste `GITHUB_CLIENT_ID/SECRET` dans `.env.prod` (backend `integrations.github.client-id` -> vide -> 404) ; et le canvas workflow **a bien des runs possibles** (analyse via Groq + delegation stub `available=true`). **v0.31.2 (retour CEO)** : sur le panneau auth, **rayures retirees -> grain (noise) seul** (`.auth-aside-noise` = feTurbulence desature en multiply, plus de repeating-linear-gradient).
> - **GitHub OAuth prod - clarification + integration (13/09, v0.31.3)** : verif live -> le **login social GitHub/Google MARCHE deja** en prod (Keycloak realm `taskforce-prod` a les IdP github+google `enabled`, l'app `taskforce-oauth` ; clic bouton -> redirige vers GitHub authorize). C'etait l'app **`taskforce-oauth`**. L'**integration** « Connect GitHub » (app **`taskforce`**, Ov23liNqmdjKIKoJ76ML) elle n'etait pas cablee en prod : (1) le backend ne recoit QUE les env **explicitement mappees** dans `docker-compose.prod.yml` (comme Stripe) - **`GITHUB_CLIENT_ID/SECRET` ajoutes** au bloc `environment` du backend ; (2) creds copies de `.env.dev` -> `.env.prod` VM1 ; (3) **reste cote CEO** : ajouter le redirect URI **`https://api.taskforce-project.fr/api/integrations/github/callback`** a l'app `taskforce` (elle n'a que `auth.taskforce-project.fr` ; le backend envoie `api.` = `APP_API_URL`). Release backend (rebuild VM1). NB SLACK_CLIENT idem non mappe (a faire si besoin de l'integration Slack).
> - **RGPD : outil de consentement cookies (a categories) + PostHog gate (13/09, v0.32.0)** : demande CEO « on veut etre a jour sur l'app » + choix PostHog EU. Constat verifie : app/site ne posaient QUE des cookies essentiels (auth `RefreshTokenCookie` + `sidebar_state`), aucun tracking -> la notice etait deja conforme ; login social GitHub/Google marche deja (Keycloak). Pour ajouter de l'analytics il faut un vrai consentement -> **construit** : `lib/analytics/consent.ts` (choix versionne en localStorage, evenements `tf-consent-change`/`tf-consent-open`, `analyticsAllowed()`) + test (6/6) ; `components/common/cookie-consent.tsx` (banniere Accepter tout / Refuser / Personnaliser -> categories **Necessaires** (toujours ON, exemptees) + **Analytics** (opt-in) ; ouverture DERIVEE, pas de setState-in-effect) ; `components/analytics/posthog-provider.tsx` (init PostHog UNIQUEMENT si consentement analytics + cle presente ; host EU `eu.i.posthog.com` ; pageview App Router ; opt-out au retrait) ; lien **« Manage cookies »** au footer (revocation) ; i18n `shell.manageCookies` EN+FR ; `posthog-js` ^1.430 ajoute ; ancien `cookie-banner.tsx` (notice) supprime. Verifie live (banniere + vue Personnaliser OK). Gates tsc+eslint+vitest consent 6/6 verts. **Reste** : (1) CEO cree le projet **PostHog EU** -> me donne la cle publique `phc_...` ; (2) je cable `NEXT_PUBLIC_POSTHOG_KEY/HOST` cote build app (docker-compose.vm2.yml, server-local) + landing (Vercel) ; (3) meme banniere sur le **site Astro** ; (4) rafraichir l'audit RGPD + la spec (gaps perimes). Cle absente = PostHog dormant, la banniere reste conforme.
> - **PostHog EU cable (13/09, v0.32.1)** : le CEO a cree une org **EU** (project 273428, region EU Cloud). Cle `phc_` publique (write-only) posee en **defaut dans le code** (`posthog-provider.tsx`, surchargeable par `NEXT_PUBLIC_POSTHOG_KEY`) -> app analytics live (derriere le consentement) sans plumbing build-arg VM2. NB : sa 1re cle etait sur la **region US** (rejetee en EU) -> il a refait une org EU (option 1 retenue vs Umami cookieless / US). Cle perso EU utilisee pour l'API (config dashboard) ; **a revoquer par le CEO apres**. **Reste** : landing (PostHog + meme bandeau consentement, PUBLIC_POSTHOG_KEY), dashboard PostHog, refresh audit RGPD. Decouverte : la landing avait deja une conception **Umami cookieless auto-heberge** (`AudienceTracking.astro`, CERT-C20) non deployee -> a retirer/retirer au profit de PostHog EU (choix CEO).
> - **Landing : consentement cookies + PostHog EU (13/09, v0.33.0)** : meme dispositif que l'app, porte sur le site Astro. `src/lib/consent.ts` (miroir de `lib/analytics/consent.ts` : `tf-cookie-consent` versionne v2, evenements `tf-consent-change`/`-open`) + `src/lib/analytics.ts` (PostHog `eu.i.posthog.com`, gate sur consentement, `capture_pageview:true` car site multi-pages, cle `phc_` en defaut surchargeable par `PUBLIC_POSTHOG_KEY`) + ilot `src/components/site/CookieConsent.tsx` (`client:load` dans `BaseLayout`, Accepter/Refuser/Personnaliser, Switch, rouvrable). Footer statique (SSR non hydrate) -> le bouton **« Manage cookies »** n'a pas d'`onClick` : il porte `data-cookie-settings` et l'ilot l'intercepte par **delegation** `document.click`. **Stub Umami retire** (`AudienceTracking.astro` supprime, vars `PUBLIC_UMAMI_*` -> `PUBLIC_POSTHOG_*` dans `.env.example`). Verifie live (astro dev) : rien avant consentement (aucun cookie/LS `ph_`), init apres Accepter (cookie `ph_phc_...`, remote-config recupere depuis eu.i.posthog.com = egress EU confirme), persistance au reload, reouverture footer OK, revoke -> `opt_out_capturing` (`__ph_opt_in_out_...=0`). tsc+eslint+astro build (69 pages) verts. Version produit UNIFIEE 0.33.0 (footers app+landing). Release landing-only -> main (Vercel). Fix CI au passage : job Lighthouse repasse en node 22 (Astro 7 exige >=22.12, cassait le build depuis la montee 5->7).
> - **RGPD : audit refresh + dashboard PostHog + coherence legale (13/09, v0.33.1)** : (1) **Audit RGPD rafraichi** (`taskforce-docs/v1/07-securite/`) : `Audit_RGPD_Conformite` (TF-RGPD-001 banniere / 005 mentions legales / 007 suppression Keycloak / 009 procedure violation clos ; tokens d'integration chiffres ; PostHog EU en sous-traitant ; transparence 4/4 ; ~85 %) + `Registre_Traitements` (traitement n°9 reecrit Umami->PostHog EU, base legale = consentement ; PostHog ajoute aux sous-traitants, TF-RGPD-010 = signer le DPA) + fiche `Landing` (LP-ANA-001 done). (2) **Dashboard PostHog EU** configure via l'API (cle perso EU) : projet 273428 renomme « TaskForce », dashboard « TaskForce - Overview » epingle avec 6 insights (pageviews, visiteurs uniques, top pages, sources, appareils, pays). **CEO doit revoquer la cle perso `phx_` apres.** (3) **Coherence legale** (v0.33.1) : les pages `privacy-policy` (app), `legal/cookies` + `legal/subprocessors` (landing) affirmaient « aucun analytics/traceur » -> corrigees pour decrire PostHog EU sous consentement. Release app+landing 0.33.1.
> - **Refonte bandeau cookies (13/09, v0.33.2)** : le bandeau (emoji + cases) faisait « naze » en contexte app (retour CEO). Refonte dans notre DA inspiree du bloc Hirael : icone Cookie, carte d'angle bas-droite, entree animee, categories en `Field`+`Switch` (Necessary « Always on » verrouille, Analytics opt-in), bouton **Manage** qui deplie, **Save choices** a l'ouverture. **Meme logique conservee** (module consentement versionne, gating PostHog via CONSENT_EVENT, reouverture footer par delegation, RGPD inchange). App (`components/common/cookie-consent.tsx`) + landing (`components/site/CookieConsent.tsx`) alignes. Verifie live (landing). Voir [[posthog-consent-rgpd]].
> - **Onboarding responsive + suppression compte 2 options (13/09, v0.34.0)** : retours CEO en test.
>   - **Onboarding responsive** : `onboarding-shell.tsx` centrait l'etape avec `justify-center` dans un conteneur scrollable -> sur petit ecran (mobile), le HAUT d'une etape longue etait rogne (impossible de scroller jusqu'au titre). Corrige avec le motif canonique `my-auto` (centre quand ca rentre, defile depuis le haut sinon) + paddings mobiles (`px-5 py-8 sm:px-6`). Sidebar deja `max-md:hidden`. **A confirmer sur device par le CEO** (flux auth-gated, pas rendu en local sans la stack).
>   - **Suppression de compte : 2 options (RGPD Art. 17)** : le CEO voulait pouvoir supprimer **immediatement** sans attendre les 30 j. C'est conforme (Art. 17 = « sans delai indu » ; la grace = filet anti-erreur, pas une obligation). **Backend** : `GdprService.deleteMyAccountImmediately` (marque puis `purgeAccount` dans la MEME transaction -> reutilise handover workspaces + anonymisation + suppression Keycloak) ; `DELETE /api/gdpr/account?immediate=true` ; test `GdprServiceTest` (+1 = 6). **Front** : `deleteMyAccountImmediately()` (service + test), panneau Account = **RadioGroup 2 choix** (Planifier 30 j *recommande* / Supprimer immediatement *irreversible*), meme dialog de confirmation par ressaisie d'email, immediate -> `logout()` (redirige /auth/login). tsc + eslint + vitest gdpr 5/5 verts. Reste (CEO) : bandeau de suppression en HAUT + bouton Restore quand deja planifie (« le reste »).
> - **Onboarding menu mobile + bandeau suppression en haut + Restore reglages (13/09, v0.34.1)** : suite retours CEO.
>   - **Onboarding mobile** : sur telephone la sidebar (etapes) etait masquee sans moyen de l'ouvrir. Ajout d'un **bouton menu** dans la topbar (libelle = etape courante) qui ouvre les etapes dans un **overlay `Sheet`** (n'occupe pas la place du contenu, contrairement a une sidebar toujours ouverte). Pastille d'etape partagee desktop/mobile. `onboarding-shell.tsx`.
>   - **Bandeau de suppression EN HAUT** : l'ancienne barre fixe basse masquait les cartes du dashboard. Deplace dans le flux **sous la topbar** (`app-shell.tsx`, a cote de `PendingInvitationsBanner`) -> il pousse le contenu au lieu de le recouvrir ; retire de `(protected)/layout.tsx` ; restyle (strip `border-b`, teinte amber).
>   - **Restore dans les reglages** : quand le compte est deja planifie pour suppression (`user.scheduledPurgeAt`), le panneau Account montre l'etat + un bouton **Restore account** au lieu des options de suppression. tsc + eslint verts. A confirmer live (flux auth-gated).
> - **Brain OS : containment reel pour les vrais comptes (14/09, v0.35.0)** : retour CEO en test - un compte neuf voyait une **liste plate** (16 READMEs de domaine + « Contexte » + « Projet » a la racine) au lieu de **Brain OS > espace > projet > notes**. Cause : le signup seedait le scaffold plat `BLANK` (`blankScaffold`, 16 domaines racine) et l'ingestion live (`writeProjectNode`/`writeWorkspaceNode`) creait des nodes **sans `parentNodeId`** ; la containment n'existait que dans le seed demo (`taskforceProjectSeed`). **Fix backend** : `BrainTemplateService.blankContainmentSeed` (nouveau seed BLANK : Brain OS -> dossier d'espace -> socle transverse + notes niveau ESPACE -> un dossier par projet -> notes niveau PROJET) ; `BrainSeedingService` marque le dossier d'espace (cle « ws ») en `refType=WORKSPACE` (ancre pour l'ingestion live) ; `writeProjectNode` niche le dossier projet sous le dossier d'espace (`parentNodeId`) + cree le paquet de notes du projet (`seedProjectNotes` : architecture/backlog/fait/problemes/decisions/runbook) ; `writeWorkspaceNode` enrichit le dossier d'espace au lieu d'une fiche plate. **Notes benefiques a chaque niveau** (espace + projet), comme demande. Le FRONT (`page-tree.tsx`) groupe deja par `parentNodeId` -> l'arbre niche automatiquement, pas de refonte data cote UI. Tests `BrainIngestionServiceTest` MAJ (dossier README/PROJET, titre = nom projet, notes nichees). Suite backend `it.ps1 -Test ALL`. **Fait ensuite** : (1) **compte de test CEO restructure** - suppression du brain plat de « Pierre's Workspace » (id 16, gmail) en SQL prod (lazy-seed `getOverview` le re-seede niche a la prochaine ouverture, template BLANK=containment, projet VMoney niche) ; (2) **polish UI Notion (v0.35.1)** : `page-tree.tsx` - icones par TYPE de page (README/ADR/FINDING/RUNBOOK/SPEC/SOP/DOC/NOTE), guides d'indentation verticaux (wrappers `border-l` par niveau), libelles courts (`smartLabel` retire le prefixe du parent : « VMoney - Architecture » sous « VMoney » -> « Architecture »). tsc + eslint verts. **A valider visuellement par le CEO** (app auth-gated, pas rendu en local).

> **▶ MAJ 08/09/2026 - Prep bêta / post LinkedIn : audit 3 axes + correctifs, tracking Grafana produit, re-seed démo prod.** `[FE + Landing + Infra]`
> - **Audit prêt-au-lancement (3 agents, lecture seule)** : sécu OWASP (0 P0 ; vérifié EN LIVE que l'actuator ne fuit pas : `/actuator/*` = 404, `/api/actuator/*` = 401), gestion d'erreurs/feedback front, flux cassés. Verdict : socle solide ; bloqueurs de conversion/embarras corrigés ci-dessous.
> - **Funnel landing** : les 46 CTA pointaient vers `/waitlist` (« launching soon ») alors que le signup est OUVERT → tous redirigés vers `${APP_URL}/auth/register`. C'était le frein n°1 pour un post LinkedIn.
> - **Bug « faux succès » (classe P0)** : les stores avalent l'erreur et renvoient `null/false` → `catch` morts → `toast.success` même en échec (pire : `issue-sheet` PERDAIT le commentaire en l'annonçant posté). Corrigé en branchant sur le retour : edit-project, bulk-assign, issue-sheet (commentaire ajout/suppr + due-date/titre), settings (workspace), team-switcher (création). `deleteComment` passe `void`→`boolean`.
> - **Cohérence FR/EN + domaine** : `taskforce.dev/.com` périmé (contact RGPD MORT) → `contact@taskforce-project.fr` (légal, confidentialité, aide, billing) ; français qui fuyait dans l'UI EN (bannières board/brain/cycles, `not-found`, fallbacks `getErrorMessage`) → EN.
> - **Robustesse UI** : `global-error.tsx` ajouté ; liens morts `/dashboard` → `/` (error, payment) ; racine protégée 0-workspace = plus de spinner infini (état vide + Reload/Sign out) ; loading/error fiabilisés sur Inbox/Analytics/Issues/Members (+ champs `error`/`membersError` sur 2 stores).
> - **Légal + footer** : mentions légales renseignées (éditeur = Pierre Michel, personne physique, immatriculation en cours ; hébergeur Vercel / OVH+Cloudflare / MNS) ; footer landing = LinkedIn + Instagram + email (GitHub/X génériques retirés) + tag `v0.3.23`.
> - **Tracking produit Grafana** (`monitoring/`) : vues read-only `metrics.*` (utilisateurs, plans, inscriptions dans le temps, activité externe) + rôle `grafana_ro` (SELECT borné, vues definer → pas d'accès direct aux tables) + relais socat 9102 (Tailscale) + datasource + dashboard « TaskForce - Produit ». Exclut Pierre + la démo.
> - **Re-seed démo prod** : `backend/tf-api/seed/prod_demo_seed.sql` = workspace « Demo Workspace » (slug `demo`, remplace `nimbus`), owner = compte réel de Pierre, SANS mutation Stripe/plan (section billing du seed dev retirée), transaction atomique.
> - **Gates + DÉPLOIEMENT (08/09)** : `tsc` + `vitest run --coverage` + build landing verts → release **#252** (merge `main`, auto-deploy VM2 + Vercel). Un test périmé (`client.test.ts` assertait le fallback FR) → CI rouge → corrigé et re-release **#253** (CI reverdie). Grafana + seed **appliqués en SSH** (Tailscale remonté) : dashboard health OK, seed « Demo Workspace » id=17, `grafana_ro` ne lit pas `public.users`. Sécu prod **vérifiée** : actuator ne fuit pas (404/401), **anti-bot ACTIF** (`TF_TURNSTILE_*` + `TF_HUMAN_CHALLENGE_*` SET dans `.env.prod`).
> - **Round 2 (08/09, demandes user)** : hébergeur mentions légales MNS→**OVH** ; **`comparison-table` supprimé** (dead code + pricing périmé) ; **`AiRateGuard.account-rpm`** ajouté (throttle requêtes/minute/compte, prod=1 pour ~5 users bêta ; `BE-IA-005`) ; **Brain OS frère synchronisé** (fiches Infra/Backend/Frontend). ⚠️ Capacité réelle : 8000 TPM Groq ne tient PAS 5 appels *deep* simultanés — le throttle est une protection gracieuse, pas de la capacité (vrai déblocage = Groq Dev / Ollama).
> - **Métriques Grafana nettoyées (08/09)** : le filtre d'exclusion ratait l'ANCIEN démo (`@demo.taskforce.dev` / `demo-%`, seed Nimbus du 26/08), le **compte école** de Pierre (`pierre.michel@stagiairesmns.fr`) et les **comptes RGPD-anonymisés** (`@anonymized.invalid`, ex. `deleted-1`) → dashboard tombé de 20 à **9 users réels**. Exclusion **centralisée** dans `metrics.real_users` (activité + `kpi_workspaces` filtré par `owner_id` en héritent). Vues ré-appliquées en prod (SSH). Les ~8 users orphelins de l'ancien seed Nimbus restent en base (masqués des métriques) - suppression DB optionnelle.
> - **Nettoyage + démo soutenance (08/09)** : 8 users orphelins `@demo.taskforce.dev` **supprimés** de la base (dead data). Nouveau **projet démo « TaskForce » (identifier TF, 27 issues REELLES** : auth OWASP, AiRateGuard, Brain OS GitHub/Slack, MCP, RGPD, monitoring, billing, funnel...) ajouté au workspace `demo` via `backend/tf-api/seed/prod_demo_taskforce_project.sql` (idempotent, a lancer APRES `prod_demo_seed.sql`) - ~2/3 assignees, 7 ouvertes non assignees pour la démo Smart Assign. Appliqué en prod.
> - **Pricing (chantier OUVERT, en attente décisions user 08/09)** : pas de réduction annuelle réelle côté Business. Prod Stripe = **mode TEST** (`sk_test`), price-ids Basic/Business présents MAIS ⚠️ **`STRIPE_WEBHOOK_SECRET` VIDE** en `.env.prod` (VM1) -> synchro de plan portail cassée. `pricing-data.ts` montre un palier « 29€ » annuel -14% sans price-id annuel Stripe -> réduction non facturable. A trancher : montants Basic/Business + % réduction annuelle, puis créer les price-ids annuels (clé rk_test fournie), aligner app + landing, et poser le webhook secret.
> - **Pricing IMPLÉMENTÉ (08/09, décisions user)** : passage en **USD**. **Basic 10/mo (8 annuel)**, **Business 16/mo (14 annuel)** (Free gratuit, Enterprise sur devis) ; réduction annuelle = **-2 USD/mois FIXE** (pas un %). 4 prix USD créés dans Stripe (test) sur les produits TASKFORCE_BASIC/BUSINESS + **webhook endpoint créé** (`we_...`, secret récupéré → à poser en prod). **Backend** : `getPriceIdForPlan(plan, interval)` + surcharge mensuelle, `getPlanForPriceId` matche les 4 ids (annuel → même PlanType), portail Stripe autorise le switch mensuel↔annuel, DTO `CreateCheckoutSessionRequest.billingInterval`, `StripeConfig`/`StripeService` + `application-{dev,prod}.yml` + `docker-compose.prod.yml` + `.env.example` (vars `STRIPE_PRICE_ID_*_YEARLY`). **Front** : toggle annuel réellement transmis au checkout (`stripe-service.createCheckoutSession(..., interval)`), `$` partout, réduction -2$, **quotas IA affichés alignés** sur les vrais plafonds (50k/300k/800k). **Landing** : `PricingSection` USD + Business 16/14 + badge « Save $2/mo » + `legal/subscription` USD. **Gates** : tsc + build landing verts ; gate backend complet en cours. **Reste** : commit + release (labels backend+frontend+landing) → sur VM1 `.env.prod` (price-ids USD + `_YEARLY` + `STRIPE_WEBHOOK_SECRET`) + redéploiement backend + test d'un checkout. ⚠️ **Contrat API changé** (`billingInterval` au checkout) → MAJ `../taskforce-docs` Backend.md à la clôture. Révoquer la clé `rk_test` après.
> - **Pricing DÉPLOYÉ (08/09)** : release **#255** → backend prod recréé (healthy) avec USD/annuel/webhook (`Webhook secret configured: true`), `.env.prod` posé (price-ids USD + `_YEARLY` + `STRIPE_WEBHOOK_SECRET`), endpoint webhook joignable (400 sans signature). Landing USD live (« Save $2/mo », zéro €). Un test périmé (`stripe-service.test` assertait le body checkout sans `billingInterval`) a rougi la CI Frontend → corrigé test-only + re-release **#256** (même piège que `client.test` en #253 : relancer `vitest run` après tout changement de comportement front). Reste : test manuel checkout carte `4242...` + MAJ Backend.md + révoquer `rk_test`.
> - **Démo Smart Assign affinée (08/09)** : les 8 profils de compétences des coéquipiers démo enrichis + DISTINCTS (front→Sarah, back/sécu/RGPD/billing→Marcus, IA→Nina, infra→Tom, UX→Lina, QA→Omar, fullstack→Aïcha) via `backend/tf-api/seed/prod_demo_skills.sql`. Le score sémantique Smart Assign est calculé EN LIVE par Groq (profile_text + skills_json ; colonne `embedding` non peuplée) → un UPDATE SQL suffit, pas de ré-embedding. Recommandations variées au lieu de tout sur Aïcha. Appliqué en prod. Vérifs billing : webhook signé OK (`customer.subscription.updated` userId=29 synchro) ; le sous €19 de Pierre = ANCIEN sous (pré-USD), un nouveau checkout = $16. Portail Stripe = hébergé (brandable via Dashboard Stripe) ; downgrade in-app = chantier optionnel.
> - **Changement de forfait IN-APP (09/09, demande user)** : la rétrogradation (et l'upgrade payant↔payant) se fait désormais **sans quitter l'app**, plus via le portail Stripe (le point de friction signalé). **Backend** `BE-PAY-022` : `POST /api/billing/change-plan` {planType, billingInterval} → `StripeService.changeSubscriptionPlan(customerId, priceId, seats)` liste l'abonnement ACTIF (customer porté par `users`), remplace le price avec **proration** + lève l'annulation programmée ; plan **reflété immédiatement** (user.planType/status) pour éviter la course webhook (`customer.subscription.updated` rejoue idempotemment). BASIC/BUSINESS only ; depuis FREE → 409 (→ `/checkout`) ; retour au gratuit = portail (résiliation fin de période). **Front** `FE-CORE-013` : `stripe-service.changePlan`, `renderCta` clarifié (Manage / Checkout / upgrade immédiat / **rétrogradation confirmée par AlertDialog** / Cancel plan), `refreshUser()` + toast au succès ; disclaimer « placeholders » → note bêta honnête (USD, test mode). **Tests** : `PaymentAndDataControllersWebMvcTest` (change-plan 200 + 409), `stripe-service.test` (changePlan mensuel/annuel/erreur). **Gates** : tsc + `vitest run --coverage` verts (840/840, couverture par-fichier tenue) ; suite backend complète en cours. Reste : release (labels backend:minor + frontend:minor) + déploiement + test user sur `/demo/billing`.
> - **Durcissement USD + beta free-tier (09/09, Round 4)** : DEPLOYE en #258 (backend 5a491b8b live, endpoint change-plan → 401 ; frontend recree). Puis, demandes user : **(1)** **garde-fou devise** dans `StripeService.changeSubscriptionPlan` — Stripe interdit de changer la DEVISE d'un abonnement ; un sub pre-USD (EUR, ex. l'ancien €19 de Pierre) ne peut pas basculer vers un price USD → message actionnable clair (409 « cancel + re-subscribe in USD ») au lieu du 502 opaque ; messages de l'endpoint change-plan passes en anglais (UI app EN). **(2)** **Bandeau beta** sur la page Facturation app : facturation en preview, tout le monde en Free tier, checkout en mode test Stripe (cartes test only, aucun debit reel). **(3)** **Suppression du pricing mort** `frontend/components/pricing/*` + `lib/constants/pricing-data.ts` (ancien pricing Plane.so JAMAIS route : EUR 0€/29€, modele 3 plans, essai 14j, SEPA/PayPal, hebergement France/Allemagne — incoherent avec le vrai USD 4 plans per-seat). **(4)** `legal/terms.astro` EUR → USD. Sweep : plus aucun `€`/EUR user-facing (app + landing). Rappel test : en `sk_test`, seules les cartes de test Stripe marchent (`4242…`) ; une vraie carte est refusee sans jamais debiter. Gates : tsc + vitest --coverage verts ; suite backend complete relancee. A releaser : labels backend:patch + frontend:patch + landing:patch.
> - **v2 spécifiée : Agent Delivery Pipeline (09/09)** : `taskforce-docs/v1/road_to_v2/Agent_Delivery_Pipeline.md` (`TF-AGENT-DELIVERY`) + docs compagnons `UX_Actions_At_Scale` (anti dropdown-hell façon Linear) et `Scalabilite_et_Robustesse`. Décisions : orchestration **Java-native** (pas de Python/LangChain), exécution dans le cloud du provider **sous le compte user** (Claude via Managed Agents), **task-agnostic**, décision humaine 1 clic, colonnes custom/supprimables. Provider-agnostic (Claude Code d'abord, puis Copilot/Cursor).
> - **P1 slice 1 EN COURS (backend fait, 09/09)** : lien Projet ↔ dépôt GitHub (`BE-AGENT-DELIVERY-1`). Migration **V85** (`projects.repo_provider`/`repo_full_name`), `ProjectRepoService` (service dédié) + endpoints `PUT`/`DELETE …/projects/{id}/repo` (CREATE via `GitHubIntegrationService.createRepo` / LINK), exposé dans `ProjectResponse`. Run ciblé **18/18 verts** (ProjectRepoServiceTest + 2 slices web ProjectController). **Front fait (09/09, `FE-AGENT-DELIVERY-1`)** : section « Code repository » au wizard (None/New repo/Link existing, statut GitHub), `useProjectStore.linkRepo`/`unlinkRepo` + `setProjectRepo`/`unsetProjectRepo`, `repoProvider`/`repoFullName` sur `Project`, route `PROJECT_ROUTES.REPO`. tsc + **vitest 847/847** verts (7 nouveaux tests). **Slice 1 RELEASEE + DEPLOYEE** (#260, main `b51da1b2`, V85 + colonnes confirmees en prod, 2 VM healthy).
> - **P1 slice 2 sur branche `v2` (09/09)** : registre de providers d'agents (`BE-AGENT-DELIVERY-2` / `FE-AGENT-DELIVERY-2`). Contrat `DeliveryAgentProvider` + 3 beans (Claude Code/Copilot/Cursor, `available=false`) + `DeliveryAgentProviderRegistry` + endpoint `GET /delivery/providers` ; front `getDeliveryProviders` + `DELIVERY_ROUTES`. Metadonnees only, dispatch = slice 3. **Workflow** : tout sur `v2`, puis `v2->dev->main` avec un gros check a la fin (demande user). tsc + delivery-service test verts ; backend cible en cours.
> - **P1 slice 3 sur `v2` (09/09)** : pipeline de delegation prouve par un stub (`BE/FE-AGENT-DELIVERY-3`). Migration **V86** `delivery_runs` + entite `DeliveryRun` (QUEUED/RUNNING/DONE/FAILED) ; contrat etendu `dispatch`/`poll` (defauts qui throw, seul `StubDeliveryProvider` les implemente et simule un run complet) ; `DeliveryService.delegate` + `DeliveryRunner` (@Async, declenche apres commit) ; endpoints `POST .../delegate` + `GET .../run` ; front `delegateIssue`/`getIssueRun`. Le repo lie au projet (slice 1) alimente le brief. **Decision humaine** (pas d'auto-move statut = slice 4). Front tsc + delivery-service 4/4 verts ; backend cible (runner + slice web + socle V86) re-lance apres fix d'une accolade manquante. **Backend 9/9 vert (V86 validee par le socle).**
> - **P1 slice 5 (UI) sur `v2` (10/09)** : delegation visible dans l'app (`FE-AGENT-DELIVERY-4`). `delivery-store` + composant `DelegateAgentControl` (picker providers+logos, indispo « soon » + etat du run avec polling -> DONE + resume + lien) insere dans l'issue-sheet apres le SmartAssign. tsc + **vitest 857/857** verts. Decision humaine (pas d'auto-move statut = slice 4 restante). **v2 = slices 2+3+5 accumulees**, slice 1 en prod ; gros check + `v2->dev->main` a la fin.
> - **P1 slice 4 sur `v2` (10/09)** : auto-move kanban (`BE-AGENT-DELIVERY-4`). Fin de run -> issue deplacee vers colonne **« In review by AI »** (DONE) ou **« Blocked »** (FAILED), creees a la volee si absentes (custom, supprimables, is_default=false, cat STARTED) via `IssueStatusRepository.findByProjectIdAndName`. Best-effort, pas de migration. Humain peut deplacer a la main ; decision d'accepter = humaine. `DeliveryRunner` + IssueStatusRepository/IssueRepository ; test etendu. **Prochain : gros check + release MINEURE `v2->dev->main`** (labels backend:minor + frontend:minor, patch remis a 0). **FAIT : release #262 mineure DEPLOYEE** (main `ae41ed99`, backend v0.11.0 / frontend v0.17.0, V86 + endpoint verifies en prod, 2 VM healthy).
> - **Version PRODUIT unifiee - footer app + landing (10/09)** : les 2 footers montraient une version stale/incoherente (`v0.3.23` = ancienne somme front+back de `frontend/package.json`+pom, decouplee des vrais tags git back 0.11 / front 0.17 / landing 1.1.5). Choix user : **UN seul numero, affichage unifie = `0.18.0`** (les tags Docker par service restent internes au deploiement). Source de verite `frontend/product-version.json` (`version`, lu par `next.config` -> `NEXT_PUBLIC_APP_SEMVER` -> footer app) + **copie synchronisee** `landing-page/src/product-version.ts` (footer landing, contextes de build Docker isoles). Script `scripts/bump-product-version.mjs <major|minor|patch>` bumpe **LES DEUX** ; ancien script somme `scripts/product-version.mjs` **SUPPRIME** ; la CI `release.yml` cree le tag produit unique `taskforce-v<version>` sur main. Gates : front tsc + landing build verts. (Release #263 avait pose 0.4.0 landing-only ; remplacee par #264 a venir = 0.18.0, labels frontend:patch + landing:patch, les 2 footers.)
> - **Icones agents (Claude/Copilot/Cursor) (10/09)** : dans le menu Assignee les agents tombaient sur les initiales (CL/GI/CU) faute de logo. **Claude** -> `logoKey="anthropic"` (logo deja vendorise). **Cursor** + **GitHub Copilot** : SVG recuperes de SVGL (`cursor_light/dark`, `copilot/copilot_dark`) et vendorises `public/logos/{cursor,githubcopilot}-{light,dark}.svg` + entres `"themed"` dans `brand-logos.generated.ts` ; `scripts/fetch-logos.mjs` recoit `githubcopilot` (cursor y etait deja) pour survivre a un `npm run logos`. `ClaudeApiProviderTest` maj (logoKey anthropic). Part avec la release A3+A4.
> - **A3 - Smart Assign recommande agent-vs-humain sur `v2` (10/09)** : `BE/FE-AGENT-DELIVERY-A3`. Le recommandeur decide desormais si une tache doit aller a un **agent** ou a un **humain**. **Backend** : `SmartAssignService.computeRecommendation` gagne `hasSpec` + `considerAgents` ; heuristique **pure** `agentSuitability(labels, priority, points, hasSpec, repoLinked)` (0-100) : label « humain » (design/decision/recherche...) = **veto** ; sinon base 50 + bonus labels mecaniques (bug/chore/docs/test/refactor), spec presente, petit scope, depot lie ; malus urgent/important, gros scope, pas de spec. `>=65` -> agent **recommande** (l'humain passe en alternative) ; `45-64` -> agent en **alternative** ; sinon reco humaine inchangee. Le provider est choisi par preference (`cursor` si depot lie, sinon `claude-api`>`copilot`>`stub`) via `DeliveryAgentProviderRegistry`. DTO `SmartAssignCandidateResponse` + `kind`/`agentKey`/`agentLogoKey` (userId null pour un agent) ; log `assignment_events`/flywheel **seulement pour un humain**. **Pas d'agent** en preview creation (rien a deleguer), bulk (A4 a venir) ni redistribution (equilibrage de charge entre humains). **Front** : `SmartAssignCandidate` + `kind`/`agentKey` ; `SmartAssignPanel` rend l'agent (logo `BrandLogo`, bouton « Delegate to X », pas de barres charge/dispo) ; `onAssign` bifurque agent->`delegate` / humain->assignee. Heuristique testee (`SmartAssignAgentSuitabilityTest`, 5 cas ; `SmartAssignServiceTest` 35 verts, dep agent mockee vide). **A4 (bulk) enchaine** : `bulkRecommend` passe `considerAgents=true` ; le dialog board « Auto-assign (N) » delegue les taches agent-adaptees (branche `kind==="agent"` -> `delivery.delegate`, sinon `updateIssue`), rendu logo + « N people · M agents », toasts « X assigned / Y delegated ». Front tsc + vitest 866/866 verts. **Reste** B (onboarding->Brain OS), C (Notion/Linear).
> - **Refonte delegation UX - A1+A2 sur `v2` (10/09)** : demande CEO (screenshot « le bourbier, on voit que dalle ») - deleguer a un agent doit se faire **comme assigner une personne** (valide par Linear : l'agent vit dans le champ Assignee). **A1** : les cles/config providers sortent de l'issue -> nouvelle page reglages **« Agents »** du workspace (`components/settings/agents-panel.tsx` + section dans `settings/page.tsx`, groupe Workspace) qui liste les agents + branche leur cle (Anthropic/Cursor) une fois pour toutes (endpoints backend inchanges). **A2** : dans l'issue, les agents dispos apparaissent **dans le menu Assignee** (groupe « Delegate to an agent », logos) - choisir une personne = assignee, choisir un agent = delegation ; le gros bloc « Delegate to an agent » (picker + formulaires de cles inline) est **supprime**, `DelegateAgentControl` reduit a un **statut de run slim** (rien si pas de run, sinon en cours/terminé+lien/echec). tsc + delivery vitest 19/19 verts. Backend inchange. **Reste** : A3 (Smart Assign recommande agent-vs-humain), A4 (bulk auto-assign avec agents), B (onboarding -> Brain OS), C (Brain OS facon Notion + passe UX Linear). Chokepoint identifie pour A3 = `SmartAssignService.computeRecommendation()` (candidats `User` -> + agents avec discriminateur `kind`).
> - **Cloture d'entame - quick wins (10/09)** : (1) **11 PR Dependabot fermees** comme **superseded** par la montee Astro 7 / Next 16.3.4 (#267) - le lockfile est deja a une version >= la cible (10 landing : postcss/nanoid/minimatch/devalue/lodash/defu/smol-toml/h3/rollup/picomatch + 2 frontend : brace-expansion/flatted). Restent ouvertes les vraies (vitest #268/#257, humanfs #225, qs/hono mcp #222/#95) = backlog deps separe. (2) **Workflow `sync-readme-badges.yml` : trigger `dev` seulement** (retire `main`) - il echouait systematiquement (push refuse sur main protegee) = ❌ recurrent trompeur ; le badge remonte sur main par le squash-release (comme la version produit). Prend effet quand ce fichier atteint main (prochaine release). (3) **`API.md` (Brain OS)** : ligne `DeliveryController` ajoutee au registre des controleurs. Note : la session CVE lancee depuis le chip fait doublon (deps deja bumpees) - aucune branche distante creee, no-op si elle ouvre une PR.
> - **Version produit AUTOMATIQUE (footer) sur `v2` (10/09)** : le footer app+landing restait bloque a `v0.18.0` malgre 2 releases - parce que `product-version.json` etait bumpe **a la main** (jamais fait) et que `release.yml` taguait `taskforce-v` en LISANT ce fichier statique. Decision CEO : **automatique + bien calcule = le plus fort bump parmi les services touches** (major > minor > patch), lu des labels de la PR. **Backfill** : les 2 releases manquees (#266 minor delivery + #267 patch deps) portent 0.18.0 -> **0.19.1** (pose maintenant). **Automatisation** : nouveau workflow `.github/workflows/sync-product-version.yml` (`on: pull_request -> main`) qui calcule la cible (max des labels vs version de `main`) et l'ecrit dans les 2 fichiers source du footer **sur la branche de la PR** -> portee sur main par le squash (pas de push sur main protegee, contrairement au badge README qui echoue) -> le footer, build depuis ce fichier sur la VM, reflete la version des la release. **Jamais de regression** (bump seulement si cible > courant : backfill/re-run = skip). `bump-product-version.mjs` gagne un mode `set <x.y.z>`. Logique validee sur 6 cas (max-bump, no-label skip, bootstrap skip, idempotence). Contrainte : le footer se construit depuis le fichier commite sur la VM (pas touche a l'infra Docker/VM, non testable) -> la valeur doit etre dans le fichier au release, d'ou l'ecriture sur la branche de PR. A releaser : labels `frontend:release:patch` + `landing:release:patch`.
> - **Securite : montee des deps CRITICAL (Next + Astro) sur `v2` (10/09)** : le job CI **Security Scan** etait rouge **depuis des jours** (chronique, pas une regression) - Trivy remontait des **CVE de dependances** (pas de secret ; mes fixtures de test `sk-ant-`/`gho_` n ont PAS ete flaggees). **Next** `16.2.11 -> 16.3.4` (frontend) corrige 2 CVE CRITICAL : `CVE-2026-75604` (RCE serveurs Windows) + `GHSA-2xp9-vwfh-vxw4` (RCE Image Optimization API via AVIF) - montee mineure, build prod verifie. **Astro** `5.18.2 -> 7.3.2` (landing) corrige `GHSA-26w7-cxv4-gfx2` (RCE optimisation image AVIF) ; **5.x n a pas de correctif** (5.18.2 = dernier 5.x) donc **montee majeure 5->7** + `@astrojs/react 5` + `@astrojs/mdx 5` (React 19 deja en place -> **aucun changement de code**), build **69 pages** + typecheck verts. Risque pratique de la CVE Astro faible (landing **SSG**, pas d usage `astro:assets`/endpoint image live), mais on aligne quand meme. A releaser avec labels `frontend:release:patch` + `landing:release:patch`.
> - **P1 - Cursor + Copilot cables (providers reels) sur `v2` (10/09)** : `BE-AGENT-DELIVERY-B2`. Demande user : « fais cursor et copilot directement ». **Cursor** = provider **asynchrone** (`CursorProvider`, Background Agents API `api.cursor.com/v0/agents`) : `dispatch` lance un agent sur le **depot lie** (slice 1), `poll` suit jusqu'a la PR ; cle Cursor chiffree (`IntegrationProvider.CURSOR`, sans migration). **Copilot** = provider **synchrone** (`CopilotProvider`, inference hebergee GitHub `models.github.ai/inference`, OpenAI-compatible) **reutilisant le token GitHub deja connecte** (zero cle en plus). Contrat elargi : `poll(externalRef, workspaceId)` (les async retrouvent leur cle) + **re-poll sur lecture** (`DeliveryRunner.refresh`, appele par `GET /run` quand RUNNING -> le polling front fait progresser l'async jusqu'au terminal). Endpoints cle generalises `GET/POST/DELETE .../delivery/{anthropic|cursor}` (DTOs generiques `ConnectKeyRequest`/`DeliveryKeyStatus`, OWNER/ADMIN). **Front** : service/store **generiques** par provider (`getDeliveryKey`/`connectDeliveryKey`/`disconnectDeliveryKey`, `keys: Record<provider,status>`), panneau connect **une ligne par provider a cle** (Anthropic, Cursor), boutons desactives tant que la cle manque. **Honnetete** : ni Pierre ni moi n'avons de compte Cursor/Copilot -> **live NON verifie** ; cable sur les contrats publics, **prouve par `MockRestServiceServer`** (`CursorProviderTest` 8, `CopilotProviderTest` 4) ; opt-in (cle requise) + echec **gracieux** (FAILED -> « Blocked » + message). Claude(API) + stub restent les chemins prouves. Le coding-agent Copilot qui ouvre une PR (assignation d'issue) reste la suite, comme `claude-code`. Gates : **front tsc + delivery vitest 19/19** ; suite backend en cours.
> - **P1 B1 - Delegation Claude via l'API Anthropic sur `v2` (10/09)** : `BE-AGENT-DELIVERY-B1` / `FE-AGENT-DELIVERY-B1`. Modele **« zero install »** demande par le user : l'utilisateur **colle sa cle API Anthropic**, TaskForce **execute dans le cloud d'Anthropic, facture sur SON compte**. **Backend** : `IntegrationProvider.ANTHROPIC` (VARCHAR len 32 -> **aucune migration**), cle stockee **chiffree** (`Integration.accessToken` @Convert `EncryptedStringConverter`, unique par workspace) ; `ClaudeApiProvider` (`key=claude-api`, `available=true`, models opus-5/sonnet-5) = provider **synchrone** : `dispatch` fait **un appel Messages API** (`POST api.anthropic.com/v1/messages`, en-tetes `x-api-key` + `anthropic-version`), la sortie texte de Claude = resume livrable (task-agnostic). Contrat elargi : `DeliveryDispatch.immediateResult` (providers sync) + `AgentBrief.workspaceId` (retrouver la cle) ; `DeliveryRunner` utilise `immediateResult` si present, sinon `poll`. Endpoints `GET/POST/DELETE .../delivery/anthropic` (connect/status/disconnect, **OWNER/ADMIN**, cle jamais renvoyee en clair -> seulement un `keyHint` des 4 derniers car.). **Front** : `getAnthropicStatus`/`connectAnthropicKey`/`disconnectAnthropicKey` + store, panneau **inline dans `DelegateAgentControl`** (colle la cle -> Connect ; « Claude (API) » desactive tant que non connectee) ; note « billed to your account, stored encrypted ». **Preuve** : le user **ne peut pas tester en live** (pas de credits API, ni Cursor/Copilot) -> **6 tests `ClaudeApiProviderTest` via `MockRestServiceServer`** (vrai wire + parsing + erreurs cle absente/401) + `DeliveryServiceAnthropicTest` (4) + runner sync. Gates : **backend 18/18** (`ClaudeApiProviderTest,DeliveryServiceAnthropicTest,DeliveryRunnerTest,DeliveryAgentProviderRegistryTest,DeliveryControllerWebMvcTest`) + **front tsc + delivery vitest 19/19** verts. Le **stub reste le chemin demo** (zero cout). Provider `claude-code` reste « soon » (= la suite : clone repo + edits agentiques + PR via Managed Agents). **v2 = slices 2+3+4+5+B1** ; gros check + `v2->dev->main` a la fin (demande user).

> **▶ MAJ 07/09/2026 - Gating IA : plafonds recalibrés sur Groq + garde-débit par minute (protège le 8K TPM partagé).** `[BE-agent + Certif]`
> - **Découverte** : le gating par plan (quota mensuel de tokens PAR COMPTE) **existait déjà et était câblé** (table `ai_token_usage`, `AiUsageService.assertWithinQuota` → 409 upsell sur les 9 chemins IA, endpoint `/ai/usage` + UI). Le chantier = **calibrer + compléter**, pas construire.
> - **Recalibrage plafonds** (`limitFor`) sur le budget Groq réel (gpt-oss-120b tier gratuit : 200K tokens/JOUR = ~6M/mois PARTAGÉS org-wide) découpé ~10 comptes : FREE 100K→**50K** / BASIC 500K→**300K** / BUSINESS 2M→**800K** / ENT ∞. L'ancien barème (non dérivé de Groq) laissait 3 comptes BUSINESS vider le mois entier.
> - **Garde-débit par minute** (NOUVEAU, le vrai manque) : `AiRateGuard` = fenêtres-minute Redis GLOBALE (8K TPM = limite Groq) + PAR COMPTE (6K, équité), greffé dans `assertWithinQuota`/`record` → couvre les 9 chemins IA **sans toucher un site d'appel**. Dépassement → `AiRateLimitedException` → **429 propre + Retry-After** (au lieu de l'échec Groq opaque qui tuait les outils MCP). Fail-open sans Redis (dev/test). Config `ai.rate.global-tpm`/`account-tpm`.
> - **Vérité dure** : 8K TPM partagés ne tiennent PAS ~10 users IA simultanés (1 appel « deep » ≈ 3-5K tokens). Le gating protège + structure le pool (429 gracieux) mais le vrai déblocage = **Groq Dev tier** ou **Ollama local** (illimité, déjà le provider embeddings + défaut dev). À décider à part.
> - **Tests** : `AiRateGuardTest` (7 : fenêtres global/compte + fail-open Redis) + `AiUsageServiceTest` étendu (plafonds recalibrés + garde consulté). **Version** : back patch `0.0.14`→`0.0.15` → produit **v0.3.23**.
> - **Labs : RIEN À FAIRE** (clarif user 07/09). Labs est **fini et gardé VOLONTAIREMENT en expérimental** — le badge « lab » **RESTE** (ne jamais le retirer, c'est intentionnel). Ce n'était pas un chantier à terminer : mon topo l'avait à tort listé « à finir ». Des 3 chantiers de la revue, GitHub/Slack + gating faits ; Labs = déjà OK tel quel.

> **▶ MAJ 07/09/2026 - CI backend reverdie : `billing_portal_200` réparée. Vraie cause = test obsolète, PAS les price-ids.** `[BE-billing + Certif/CI]`
> - **Contexte** : le job « Run Backend Tests with Coverage » (`mvn verify` de `backend-tests.yml`) était rouge en permanence depuis le 05/09 (>= 6 runs `dev`), unique échec `PaymentAndDataControllersWebMvcTest.billing_portal_200` (attend 200, reçoit 409). Ce rouge masquait les vraies régressions (il a failli masquer la casse GitHub/Slack du bloc ci-dessous) et forçait les merges de release en `--admin`.
> - **Diagnostic corrigé** : l'hypothèse « `STRIPE_PRICE_ID_*` absents de l'env CI » (notée le 07/09 dans le bloc ci-dessous et dans la mémoire) est FAUSSE pour ce test. En `@WebMvcTest`, `StripeService` est mocké (`@MockitoBean`) et `StripeConfig` n'est pas chargé, donc les price-ids n'ont aucun effet. Et `/api/billing/portal` n'utilise aucun price-id : depuis le refactor « client Stripe porté par la table `users` », `BillingController.createPortalSession` lit `user.getStripeCustomerId()`. Le test était resté sur l'ancien contrat : `User` mocké SANS `stripeCustomerId` + stub `subscriptionRepository` (que plus aucun des 4 contrôleurs testés n'injecte) -> `IllegalStateException("Aucun abonnement à gérer...")` -> 409 via `GlobalExceptionHandler`. Prouvé par un run ciblé AVANT correctif : `Body {"status":409,...,"path":"/api/billing/portal"}`.
> - **Correctif (test uniquement, ZÉRO changement prod)** : le `User` mocké porte un `stripeCustomerId` réel (`cus_123`) ; suppression du stub `subscriptionRepository`, du `@MockitoBean SubscriptionRepository` et des imports `Subscription`/`SubscriptionRepository` devenus morts. `createBillingPortalSession` reste mocké. Le « 409 clair sans price-ids » de la mémoire est réel mais concerne `getPriceIdForPlan` / le chemin `/checkout`, pas `/portal` : les deux avaient été confondus.
> - **Vérification** : `it.ps1 -Test PaymentAndDataControllersWebMvcTest` = 7/7 vert ; `it.ps1 -Verify` (= `mvn clean verify`, commande exacte de la CI, tests + gate JaCoCo) = **1060 tests, 0 échec, 1 skip, « All coverage checks have been met », BUILD SUCCESS**. La CI backend redevient significative ; les releases n'ont plus besoin d'`--admin` pour ce motif. L'option 1 du ticket (injecter des price-ids factices en CI) n'aurait rien corrigé.

> **▶ MAJ 07/09/2026 - Intégrations : GitHub + Slack branchés sur l'ingestion Brain OS (patron Plane).** `[BE-integrations + FE-integrations]`
> - **Contexte (revue d'état des lieux, « terminer GitHub/Slack »)** : cartographie du code réel. GitHub = OAuth + lecture repos/issues + liens PR/commit manuels ; Slack = OAuth + config canaux + **notifications sortantes déjà câblées**, mais son **mirror entrant était du code mort** (`fetchHistory`/`resolveUserName` sans appelant, colonnes V58 non mappées) dont le scaffolding visait le **module chat RETIRÉ** ([[chat-not-in-cdc-but-delivered]]). **Aucune des deux n'alimentait le Brain OS** (seul Plane le faisait). cf. [[integration-connect-vs-ingest]].
> - **Décision** : brancher les deux sur le Brain OS via le **patron `PlaneIntegrationService.sync`** (lecture seule → `KnowledgeNode` dédupliqué par `metadata.externalId` + `embedNode`). Recycle les lecteurs existants (GitHub `listRepoIssues` enrichi du body ; Slack `fetchHistory`+`resolveUserName` : le code mort retrouve un usage vivant). **Push / webhooks / Events API écartés** (gros, non testables → roadmap).
> - **GitHub** (`GitHubIntegrationService.sync(slug,userId,repo)`) : ingère issues + PR d'un dépôt → nodes `NOTE`/`PROJET`, tags `[github,external]`, `externalId = repo#numéro`, corps borné à 6000 car. Endpoint `POST /integrations/github/sync?repo=`. UI : bouton « Sync → Brain OS » sur le dépôt sélectionné (`GitHubRepoBrowser`).
> - **Slack** (`SlackIntegrationService.sync(slug,userId,channel)`) : ingère l'historique d'un canal → nodes `NOTE`/`HISTORIQUE` (log de conversation, pas `PROJET`), tags `[slack,external]`, `externalId = canal:ts`, auteur résolu, filtre le bruit (< 4 car.). Endpoint `POST /integrations/slack/sync?channel=`. UI : bouton « Sync » par canal. ⚠️ **non testable côté user** (pas de workspace Slack) → validé par contract test.
> - **Tests** : `GitHubIntegrationContractTest` + `SlackIntegrationContractTest` étendus (constructeurs + 1 test de sync chacun : Brain OS mocké, `MockRestServiceServer` sur l'API réelle, dédup vide → `createNode` vérifié 2×). Front : `syncGitHub`/`syncSlack` + tests service. Les deux services deviennent des sources Brain OS de 1re classe, au même rang que Plane.
> - **Version** : front patch `0.3.7`→`0.3.8` + back patch `0.0.13`→`0.0.14` → produit **v0.3.22**.
> - **Piège attrapé par la CI complète** (que le gate local ciblé avait manqué) : ajouter des collaborateurs **@Service** (`KnowledgeService`, `BrainSearchService`) au constructeur des 2 services **casse leurs tests d'intégration slicés** (`@DataJpaTest` + `@Import` ne câble pas les @Service). Fix : `@MockitoBean KnowledgeService` + `BrainSearchService` dans `GitHubIntegrationServiceIntegrationTest` + `SlackIntegrationServiceIntegrationTest` (les repos JPA + JdbcTemplate sont déjà fournis par la slice ; les WebMvcTest/autres slices mockent déjà les services). Leçon : lancer la suite backend COMPLÈTE avant push, pas seulement les classes touchées.
> - ⚠️ **CI backend rouge de façon chronique et indépendante** (depuis le 05/09, avant ce lot) : `PaymentAndDataControllersWebMvcTest.billing_portal_200` attend 200 mais reçoit **409** car `STRIPE_PRICE_ID_*` absents de l'env CI (cf. [[billing-stripe-price-ids-prod]]). Artefact d'env CI, pas un bug prod (les price-ids sont posés en `.env.prod`). À traiter à part (poser les price-ids en CI ou rendre le test tolérant) : ce rouge permanent masque les vraies régressions (il a failli masquer celle ci-dessus). **[CORRIGÉ le 07/09 - voir le bloc du 07/09 en tête : la vraie cause était un test obsolète (customerId manquant sur le user mocké), pas les price-ids. CI reverdie.]**
...
>
> **▶ MAJ 06/09/2026 - Revue d'état des lieux : décisions de priorisation.** `[Planning]`
> - **À FAIRE maintenant** (validé user) : finir **Labs** (Intelligence/Analytics + Brain OS), finir **GitHub** (sync write/bidir) + **Slack** (bidir — ⚠️ non testable côté user), **cache des insights IA**, **couvrir les 3 classes IA ~0 %** (`AnalysisJobService`/`IssueAiService`/`DecisionService`), **vrai gating par plan** (métrique = tokens IA/mois/compte dimensionnée sur la capacité Groq ÷ ~10 comptes + garde-débit par workspace pour protéger le 8000 TPM partagé), **réparer l'écran `/{ws}/issues`** (actions « Edit »/« Assign to me » mortes + statut `in_review` fantôme → décision : RÉPARER, c'est une vue cross-projets utile).
> - **TODO (plus tard, PAS maintenant)** : nettoyer l'endpoint mort `ai-service /v1/smart-assign/*` (16-dim placeholder, 0 appelant) — **Smart Assign RESTE en Java** (logique data+domaine+autz) ; ne pas toucher avant la soutenance (impacte le dossier). **RBAC granulaire via Keycloak** (permettre à un OWNER/ADMIN d'éditer les rôles par workspace/projet). **Workflows/agents** (assignation à des agents + réponses temps réel + automatisations → ramènera le `workflow-store` orphelin).
> - **On ne touche PAS** : cap Groq (free tier assumé), landing, migrations orphelines (V27 discussions / V28 chat), `workflow-store` (viendra avec les workflows).
> - **Corrigé** : le **login social GitHub/Google est FAIT** (backend `OAuthLoginController` + IdP Keycloak ; les boutons n'affichent « bientôt » que si le provider manque du flag env `NEXT_PUBLIC_AUTH_SOCIAL_READY`, set en prod). Mon topo était stale. Docs de planif (`TODO.md`/`.ai/technical-debt.md`/`known-issues.md`) toujours périmées → à purger.
>
...
>
> **▶ MAJ 04/09/2026 - Footer : TAG de version PRODUIT (somme front+back) plutôt que le SHA opaque.** `[FE-footer]`
> - **Demande (user)** : « je veux le tag de version » (pas une date, pas le SHA `68203471` illisible), et **version produit = travail complet front + back**. Décision produit : **major reste 0** (pas encore de v1), et on **additionne les composantes des deux tags** → `produit = 0.(minor_front + minor_back).(patch_front + patch_back)`. Front `0.1.0` + back `0.0.1` → **v0.1.1**.
> - **Impl** : `scripts/product-version.mjs` lit `frontend/package.json` + la version projet de `backend/tf-api/pom.xml`, calcule la somme, écrit `frontend/product-version.json` (committé). `next.config.ts` le lit au build (`env.NEXT_PUBLIC_APP_SEMVER`, repli sur package.json). Calcul dans un script car le pom backend **n'est pas** dans le contexte de build Docker du frontend (`COPY . .` de `frontend/` seul). Le SHA (`NEXT_PUBLIC_APP_VERSION`, posé par l'auto-deploy VM) reste en tooltip pour la traçabilité. **À relancer** (`node scripts/product-version.mjs`) quand on bumpe un tag front ou back. `app-footer.tsx` + `next.config.ts` + script + json.
> - **Bump 04/09** : pour compter le boulot du jour, front **minor** (`0.1.0`→`0.2.0`) + back **patch** (`0.0.1`→`0.0.2`) → produit **v0.2.2** (0.(2+0).(0+2)). Le jar backend est référencé par wildcard (`COPY target/*.jar`) → bump du pom sans casse. Convention posée : chaque release bumpe le tag du côté concerné, puis relance du script.
>
...
> - **Phase 2 front (FAITE, 04/09)** : `/users/me` expose désormais `deletionScheduledAt` (ajouté à `UserResponse` + les 2 mappings `UserService`/`AuthService`, et au type `AuthUser`). **Bandeau** de restauration (`AccountDeletionBanner`, barre fixe basse montée dans le layout protégé) visible tant que le compte est planifié : rappelle la date de purge + bouton **Restore** (`restoreMyAccount` → `POST /account/restore` → `refreshUser`). **Dialog** de suppression réécrit : slot `details` (liste des conséquences - grâce 30 j récupérable, workspaces solo supprimés, partagés **transférés au membre le plus ancien**), libellé « Schedule deletion ». Le flux ne **déconnecte plus** (compte actif pendant la grâce) et affiche la date. `deleteMyAccount` renvoie la date de purge. Tests `gdpr-service` (delete-renvoie-date + restore) verts, `tsc` OK. Corrige la copie qui mentait (« irréversible »). Répond à « si je me reconnecte, est-ce que je peux récupérer mon compte ? ».
> - **Version** : back **patch** (`0.0.2`→`0.0.3`) + front **minor** (`0.2.0`→`0.3.0`) → produit **v0.3.3**.
>
...
> - **`[TF-MCP-05]` (lots 1+2)** : **25 connecteurs MCP-ready 1-clic** (URLs **vérifiées par sonde**, jamais inventées). Lot 1 (8) : notion, stripe, paypal, intercom, neon, canva, attio, hubspot. Lot 2 (+10) : monday, clickup, airtable, supabase, figma, zapier, grafana + **square/webflow/wix** (nouvellement déclarés au catalogue). GitHub garde son flux **natif** (deep sync). Test `ConnectorCatalogTest`. Toute autre URL MCP reste collable (bring-your-own).
> - **`[TF-MCP-04]` (moteur backend fait)** : `McpImportService.importProject` = fetch MCP (`list_issues` du connecteur) → **normalisation LLM générique** (`{issues:[{title,description,priority}]}`) → `createProject` + `createIssue` en masse (borne 100). Endpoint `POST /api/workspaces/{slug}/mcp/import` (BUSINESS+ + manager). + outil interne `create_project` (import **agentique** aussi possible via Cortex). Réutilise `WorkspaceMcpService.execute` (token/session gérés) → zéro code par service. Test `McpImportServiceTest`.
> - **`[TF-MCP-04]` (wizard front fait)** : mode **« Import from a tool »** dans `create-project-dialog.tsx` (bascule vierge/import) → liste les sources MCP connectées (`getMcpServers`, joignables) + nom du projet → `importMcpProject` → toast + atterrissage sur le nouveau projet. Route `MCP_IMPORT`, service + test `integration-service.test`. `tsc` OK.
> - **`[TF-MCP-04]` (wizard UX)** : le mode Import affiche une **grille de logos** (`BrandLogo`) des outils PM MCP-ready (Linear, Asana, Jira, ClickUp…) au lieu d'un select ; les non-connectés ont un bouton **« Connect » 1-clic inline** (OAuth depuis le wizard, plus de détour par Settings). **Retour OAuth fluide** : nouveau champ `mcp_oauth_states.return_to` (V84, chemin relatif sanitizé anti open-redirect) → le callback renvoie sur `…?import=<outil>&mcp=connected` et `CreateProjectModal` rouvre le wizard en mode import, outil présélectionné. **Reste (optionnel)** : import bulk >100 issues, mapping des statuts externes.
>
...
> - **Flux** (`McpOAuthService`) : `start` (découverte → DCR RFC 7591 → PKCE S256 + `state` → URL d'autorisation, le front y redirige) ; `callback` (valide le `state`, échange le code, stocke les tokens, **redirige TOUJOURS vers l'UI** succès/erreur, jamais une erreur brute). Table `mcp_oauth_states` (V82, état éphémère : PKCE/endpoints/client DCR, expiry 10 min, secrets chiffrés via `EncryptedStringConverter`).
> - **Tokens** (`McpTokenService`) : stockés **chiffrés** dans le `config` de la `ConnectorConnection` (aucune colonne), **refresh auto** avant expiration. `WorkspaceMcpService.toServerRef` utilise le token OAuth géré (sinon le token collé, bring-your-own conservé en fallback).
> - **Sécurité** : **`SsrfGuard`** (garde webhook réutilisé) sur CHAQUE fetch de découverte/DCR/token (bloque loopback/privé/link-local/Tailscale, même via un nom qui résout en interne) ; PKCE S256 ; `state` anti-CSRF résolvant le workspace au callback (jamais l'URL) ; callback public permis dans `SecurityConfig` ; `start` gaté BUSINESS+ + manager.
> - **Endpoints** : `POST .../mcp/servers/{key}/oauth/start` (authed) + `GET /api/mcp/oauth/callback` (public). **Frontend** : bouton « Connect with OAuth (1-click) » + fallback URL+token + reprise du retour `?mcp=connected|error`.
> - **Tests** : `McpOAuthServiceTest` (start + callback + erreurs), `McpTokenServiceTest` (refresh + stockage), `McpOAuthDiscoveryTest` (chaîne + fallbacks). **[statut : ✅ mergé sur dev (#215) - 988 tests verts]**
> - **Live + revue** : `it.ps1 -Test ALL` vert (988). **Start prouvé live contre Linear** (découverte + DCR + URL d'autorisation ; `redirect_uri` localhost accepté). **Revue OWASP Top 10 faite** (conforme) : durcissement **A08/A10** = `authorization_endpoint` (metadata d'un serveur semi-fiable) validé `SsrfGuard` (public http(s)) avant la redirection navigateur - fix **mergé (#217)**. Note mineure non bloquante : suppression du `state` sur erreur dans la même transaction (survit jusqu'à l'expiry sur rollback ; single-use + code AS consommé).
> - **✅ Déployé en prod le 02/09/2026 (#218, `--admin --squash`, auto-deploy VM1+VM2).** Config prod `integrations.mcp.oauth.*` câblée sur les URLs publiques existantes (`APP_API_URL`/`APP_URL`/`FRONTEND_URL`) - aucune nouvelle var d'env, `redirect_uri` = vraie URL publique du backend. **V82 appliquée en prod**, backend sain (boot sans erreur de binding). Reste (non bloquant) : le tour complet consentement + callback contre un vrai compte (Linear) ; caveat DCR pas universel (fallback : coller un token).

> **▶ MAJ 02/09/2026 - Couche de capture data flywheel (`ai_generations`), amorce V2.** `[TF-FLYWHEEL-01]` (branche `feat/ai-generations-capture`)
> - **Objectif (spéc `taskforce-docs/v1/road_to_v2/Data_Flywheel_et_Apprentissage.md`)** : capturer, à CHAQUE point human-in-the-loop où l'IA propose et l'humain tranche, le quintuplet {requête, contexte Brain OS servi, draft, final retenu, signal}. Corpus de préférences PAR WORKSPACE = observabilité aujourd'hui + carburant d'un LoRA/DPO plus tard (on n'entraîne rien maintenant, on collecte le bon corpus). Le plus haut levier : sans ça, chaque interaction IA est perdue à jamais.
> - **Table** `ai_generations` (Flyway `V81`) : `(id, workspace_id FK ON DELETE CASCADE, kind [SPEC|DECISION|SMART_ASSIGN], request_ref, context_refs jsonb, draft jsonb, final jsonb, signal [ACCEPTED|EDITED|REJECTED], edit_distance, model, latency_ms, created_by, created_at)` + index `(workspace_id, kind, created_at)` et index partiel `WHERE final IS NULL` (finalisation rapide). **Extensible V2** : `kind` en VARCHAR (aucun CHECK) et `final` en jsonb → accueillir un outcome d'exécution d'agent plus tard SANS migration. Entité `AiGeneration extends AuditableEntity` (jsonb via `@JdbcTypeCode(SqlTypes.JSON)` + `Map`), enums `AiGenerationKind`/`AiGenerationSignal`, repo dédié. Flag `workspaces.ai_learning_enabled` ajouté (même migration).
> - **`AiGenerationService.record(Capture)`** : UNE méthode, idempotente, **transaction SÉPARÉE** (`REQUIRES_NEW` : un échec SQL de capture ne rollback JAMAIS la transaction métier appelante) + try/catch → **ne jette jamais**. Court-circuitée si `ai_learning_enabled = false` (**opt-in RGPD, défaut OFF**, validé avec le user). Sans final ni signal → ligne ouverte (draft) ; sinon → finalise la dernière ligne ouverte de l'objet (sinon insère une ligne complète). Signal fourni par l'appelant (DECISION/SMART_ASSIGN) ou **dérivé** (SPEC : EDITED si le texte a changé sinon ACCEPTED ; `edit_distance` = Levenshtein borné). `finalizeOnly` = ne pas créer de ligne parasite (affectation hors smart-assign). Purge `purgeWorkspace`/`deleteByWorkspaceId` (la suppression de workspace purge déjà via CASCADE).
> - **Câblage aux 3 gates HITL existants** : (SPEC) `IssueAiService.generateSpec` capture le draft + le contexte RAG (ids des nodes), `approveSpec` le finalise (signal + edit_distance sur la spec). (DECISION) `AnalysisJobService.accept` → ligne complète ACCEPTED (priorité IA → issue), `dismissPriority` → REJECTED. (SMART_ASSIGN) `SmartAssignService.recommend` capture la reco top-1 (draft), `IssueService.updateIssue` la finalise à l'affectation réelle (ACCEPTED si == top-1, REJECTED sinon ; `finalizeOnly`). Corpus DISTINCT de `assignment_events`/`ai_runs` (historique opérationnel du scoring) : aucune duplication d'intention.
> - **Tests** : `AiGenerationServiceTest` (insertion draft, finalisation ACCEPTED/EDITED/REJECTED, dérivation signal SPEC + SMART_ASSIGN, `finalizeOnly` no-op, opt-in coupé, **no-throw** sur repo KO / entrée dégénérée, Levenshtein `@ParameterizedTest`, purge) + verify de `record` dans `SmartAssignServiceTest` (gate SMART_ASSIGN). Gates SPEC/DECISION prouvés end-to-end par le critère de fin (3 lignes en base). **[statut : ✅ DÉPLOYÉ EN PROD]**
> - **✅ Livré + déployé (02/09)** : `it.ps1 -Test ALL` vert (973 tests) ; critère prouvé EN LIVE (3 lignes : SPEC EDITED edit_distance 33 / SMART_ASSIGN ACCEPTED / DECISION ACCEPTED) ; Brain OS frère tracé (`IA-FLYW-001`). **Mergé dev** (#213), **release prod** #214 (`--admin --squash`, main `77fc93c9a`) → auto-deploy VM1+VM2 healthy, **V81 appliquée sur prod** (Flyway `success=true`), opt-in OFF sur les 13 workspaces prod. Le lot **catalogue** (#212 : MCP-ready + UI honnête + traduction anglaise) est parti dans la même release #214. Suite : V2 capture le downstream d'exécution sur le MÊME `ai_generations` (kind/final déjà extensibles).

> **▶ MAJ 02/09/2026 — Lot de correctifs v1 (onboarding, paiement, suppression de compte) + suite MCP.** `[V1-FIXES]` (branche `feat/v1-onboarding-billing-account`)
> - **Onboarding (4 correctifs UX, `app/onboarding/page.tsx`)** : (a) l'**intro de marque** (loader « matière » `LoginIntro`) ne jouait PAS à l'arrivée sur `/onboarding` au 1er login — la page vit HORS du `ProtectedLayout` qui la porte ailleurs ; on rejoue la révélation ici (drapeau `tf.intro` lu en SYNCHRONE au 1er rendu, overlay à index STABLE pour ne pas relancer l'anim à la bascule `loading→wizard`). (b) **Hint clavier `↵ Enter`** ajouté à droite de l'input « ajouter une compétence ». (c) **Feedback auto-invitation** : s'inviter soi-même → `toast.error` clair (+ toast info sur doublon). (d) **Identifiant projet retiré** de la dernière étape (on s'en moque ; `deriveIdentifier` reste utilisé côté `createProject`).
> - **Paiement (`/payment/success` « Verification error » sur compte de test)** : la cause = `AuthService.completeRegistrationAfterPayment` **exigeait un OTP en attente** (ancien flux « inscription PUIS paiement ») ; or l'upgrade in-app n'en crée pas → `getLatestOtp == null` → 500. **Fix** : pour un utilisateur DÉJÀ en base (cas normal), on applique le forfait depuis les **métadonnées de session** (`planType`, posé par `BillingController` au checkout), **sans OTP** — idempotent avec le webhook `checkout.session.completed`. L'OTP ne sert plus qu'à CRÉER un utilisateur inexistant (legacy). Front : le `fetch` brut (sans header d'auth, `NEXT_PUBLIC_API_URL` sans fallback) migré vers `stripeService.verifySession` (apiClient : base URL centralisée + enveloppe `ApiResponse`).
> - **DA des pages de retour paiement** : `success`/`cancel` utilisaient des dégradés vert/rouge/orange **hors charte** → refondues sur la **DA des pages d'erreur** (`not-found.tsx`/`error.tsx`) via un composant partagé `components/payment/payment-shell.tsx` (`bg-background`, `StripedPattern` teinté, fondu radial, lockup de marque, jetons). Vérifié live (Brave) : erreur + cancel on-brand, 0 erreur console.
> - **Suppression de compte (`GdprService.deleteMyAccount`)** : « ne supprimait pas tout » = seul le row `User` était anonymisé (workspaces possédés, appartenances, profils, 2FA laissés). Désormais on **efface tout le footprint** en **SQL brut** (`DELETE FROM workspaces WHERE owner_id` → cascade DB projets/issues/membres/invitations ; puis `workspace_members`/`member_skill_profiles`/`user_two_factor` par `user_id`) **puis** on anonymise le row résiduel (tombstone d'intégrité référentielle) + suppression Keycloak après commit (inchangé). **SQL brut volontaire** : un `delete` d'entité JPA sur un `Workspace` encore référencé par un `WorkspaceMember` managé lève `TransientPropertyValueException` à l'auto-flush. UI : la confirmation « bouton + toggle » (jugée « naze ») remplacée par un **`DeleteConfirmDialog` type GitHub** (ressaisir son email pour armer). Tests : `GdprServiceIntegrationTest` + test « footprint supprimé » ; `AuthServiceTest` aligné (plus de stub OTP sur le chemin utilisateur-existant).
> - **Suite MCP (`[TF-MCP-01]` ci-dessous)** : les **outils MCP live** sont désormais **surfacés dans Ask AI** (empty-state Cortex, `agent-chat.tsx`) — « It can also act on your connected tools » + chips, best-effort (409 non-BUSINESS / aucun serveur → rien). Réutilise `getMcpServers`.
> - **Feedback utilisateur — couverture (demande user « feedback partout ? »)** : mécanisme central = intercepteur axios (`lib/api/client.ts`) qui toast AUTO seulement les erreurs **systémiques** (réseau / 5xx / 429) ; **4xx + succès = à la charge de l'appelant** (par design, commenté). **Trou trouvé + corrigé** : le `<Toaster>` n'était monté QUE dans `(protected)` et `auth` → les routes hors de ces groupes (`/onboarding`, `/payment/*`, `error.tsx`, `not-found`) n'avaient AUCUN Toaster → leurs `toast.*` étaient **silencieux** (y compris le feedback auto-invitation d'onboarding). **Fix** : `<Toaster>` UNIQUE au **layout racine** (`app/layout.tsx`, `bottom-right`), retiré des deux sous-layouts (sinon sonner rend chaque toast en double). ⚠️ Conséquence (validée user) : les toasts `auth` passent de `top-center` à `bottom-right` (Toaster unique = une seule position, choix « unifié »). Vérifié live : toast visible sur `/payment/success`.
>   - **Remédiation couverture par-action (~30 trous, choix user « tout »)** : cause racine = les stores Zustand AVALENT les échecs (retour `null`/`void`) et l'intercepteur ne toaste pas les 4xx → échecs muets + 2 « faux succès » destructifs (delete projet/cycle toastaient « supprimé » inconditionnellement). Patron appliqué : les méthodes qui avalaient passent en `Promise<boolean>` (ou renvoient déjà `null`/throw) → l'appelant teste et `toast.error` ; toasts succès rendus conditionnels ; `catch {}` mensongers (« déjà notifié ») corrigés. Fait par 3 agents à stores/fichiers disjoints (A Issues+board `deleteIssue`/`deleteStatus`→bool ; B Projets/Cycles/Labels `deleteProject`/`deleteCycle`/add/remove→bool ; C Brain/Pages/Membres/Notifs `markAllAsRead`/`dismissJob`→bool). Domaines déjà OK (membres/intégrations/billing/issue-sheet/dashboard) non touchés. **Validé : `tsc` EXIT 0 · `vitest` 831 tests verts · eslint 0 erreur sur les 26 fichiers touchés** (2 erreurs `no-unescaped-entities` PRÉ-EXISTANTES échappées au passage). Bug de sémantique pré-existant corrigé : le delete projet toastait en ROUGE (`toast.error`) sur SUCCÈS → `toast.success`. Toaster vérifié live sur `/payment/success`.
>   - **Toasts « riches » à variations (demande user, insight)** : nouveau module `lib/notify.tsx` au-dessus de sonner (le simple `toast.success/error` reste) - `notifyUndo` (action réversible → bouton Undo), `notifyProgress` (barre de progression déterminée : `setProgress/success/error/dismiss`), `notifyRich` (carte de marque : pastille d'icône + titre + description + bouton fermer + jusqu'à 2 actions, façon shadcn « sonner-05 » mais jetons TaskForce, sans dép `Separator`). **Garde-fou sécurité documenté : jamais de donnée sensible dans un toast.** 1ᵉʳ usage concret : `components/security/two-factor-nudge.tsx` (monté dans le layout `(protected)`) - à l'arrivée dans l'app, si le 2FA (TOTP, `getTwoFactorStatus`) est off, propose « Enable 2FA » (→ `settings?section=security`) / « Not now » ; au plus 1×/session (sessionStorage) + « Not now » = définitif (localStorage). nudge **vérifié live** (carte affichée, « Enable 2FA » → `settings?section=security`, dismiss OK) après 2 bugs corrigés au test (minuteur annulé au remontage → garde module ; slug vide car monté au-dessus de `[workspace]` → `usePathname`).
>   - **Câblage des variations (choix user)** : (1) **Undo sur suppression d'issue** - `deleteIssueWithUndo` (issue-store) : retrait optimiste + toast Undo ~6 s, DELETE différé, minuteur porté par le store (survit à la navigation) ; câblé liste + détail. (2) **Progress sur l'export projet** - `downloadProjectExport(onProgress)` via `onDownloadProgress` axios (progression RÉELLE) → `notifyProgress`. (3) **IA** - les loaders IA cyclaient DÉJÀ des phases (`SPEC_PHASES`+shimmer / `ThinkingBar`), pas de bare spinner ; ajout d'un **compteur de temps écoulé** sur le mode long « Approfondir » de la spec (pas de barre déterminée factice : la génération LLM n'a pas de % réel). tsc+eslint verts ; live du câblage à faire avec de vraies données (workspace admin vide).
>   - **PR #209 → dev** (labels `backend:release:patch` + `frontend:release:minor`). CI : tout vert SAUF « Frontend Tests » au 1er run → **seuil de couverture par-fichier** de `stripe-service.ts` (68.85% < 78%) car `verifySession` (ajouté au fix paiement) n'était pas testé. Mon `vitest run` local (sans `--coverage`) ne le voyait pas ; la CI lance `test:coverage`. Corrigé par 2 tests (`verifySession` succès+erreur) → 85.24% stmts / 80% funcs. Leçon : vérifier `npm run test:coverage` (pas seulement `vitest run`) quand on ajoute des fonctions à un service à seuil. **Mergé (squash) sur `dev`** après CI 100% verte. Contrôle main/dev (demande user) : l'écart de commits = merge commits dev→main + PR parallèles (contenu identique, objets distincts), AUCUN contenu app n'a bypassé dev ; dev est en avance sur main de #208 (MCP) + #209. Dernier `—` résiduel (commentaire de #208) retiré sur dev → **0 tiret cadratin**. **✅ DÉPLOYÉ EN PROD** : `dev→main` fait via une branche release basée sur main (le dev→main direct #210 était en conflit de squash → résolu en prenant la version de dev, branche == dev vérifié), **PR #211 mergée `--admin --squash` le 02/09** (CI 100% verte), auto-deploy VM1 (backend) + VM2 (frontend) déclenché. main == dev. Post-deploy : smoke-test login (cookie refresh cross-sous-domaine) à faire.
> - **Validation** : front vérifié live (Brave) sur les 4 zones + **`tsc --noEmit` et eslint verts** (le prop `confirmTextLabel` du `DeleteConfirmDialog` élargi `string`→`ReactNode` pour accepter l'email en gras). **Backend : `it.ps1 -Test ALL` ✅ VERT — 954 tests, 0 échec, 0 erreur, 1 skipped, BUILD SUCCESS** (1er run ciblé avait révélé 3 échecs — `TransientPropertyValueException` ×2 + `UnnecessaryStubbing` — tous corrigés). Pas encore commité (attente validation user).

> **▶ MAJ 31/08/2026 - Pont MCP-catalogue : les connecteurs génériques deviennent branchables via MCP.** `[TF-MCP-01]`
> - **Contexte** : audit intégrations (demande user) → 3 réelles (GitHub/Slack/Plane) + 126 stubs qui ne faisaient que stocker des identifiants opaques (mort). Or le moteur MCP hôte existe déjà et est complet (`WorkspaceMcpService` + `McpClient` + `McpActionController` : `/mcp/servers`, `/actions/execute`) : toute `ConnectorConnection` portant un `mcpUrl` expose ses outils à Cortex (lecture directe, écritures **validées humainement**, gaté BUSINESS+). Il n'était juste pas relié au catalogue. Décision user : brancher le catalogue dessus (plus gros levier, amorce v2).
> - **Fait (PR 1, le pont)** : capability `"mcp"` ajoutée aux connecteurs génériques du `ConnectorCatalog` (SoT). Le dialog Connect (`integrations-catalog.tsx`) propose un champ **URL serveur MCP** (+ token optionnel) pour ces connecteurs → `POST /mcp/servers` → affiche le statut réel (joignable + **N outils live dans Cortex**, ou erreur) + déconnexion. Routes + service front (`getMcpServers`/`connectMcpServer`/`disconnectMcpServer`). Bannière catalogue reformulée (honnête). Aucune migration (config JSON existante). Design **bring-your-own MCP URL** (zéro URL vendeur en dur). Test `ConnectorCatalogTest` aligné (`observe`+`mcp`, jamais `act`).
> - **Reste (backlog v1)** : ✅ **« MCP-ready » 1-clic = FAIT** (02/09, branche `feat/mcp-ready-suggested-url`) - champ `mcpSuggestedUrl` sur `ConnectorDescriptor`/`ConnectorView` + helper `suggestMcp(key,url)` ; **7 endpoints DISTANTS hébergés VÉRIFIÉS SUR LE WEB** pré-remplis (éditables) : Sentry `mcp.sentry.dev/mcp`, Linear `mcp.linear.app/mcp`, Cloudflare `mcp.cloudflare.com/mcp`, Asana `mcp.asana.com/v2/mcp`, Atlassian/Jira `mcp.atlassian.com/v2/mcp`, GitLab `gitlab.com/api/v4/mcp`, Vercel `mcp.vercel.com`. Dialog Connect : URL pré-remplie + indice (endpoint officiel / guidage bring-your-own), badge **« MCP-ready »** sur la carte. Zéro URL inventée ; les autres restent bring-your-own. Test `ConnectorCatalogTest` étendu. Le registry MCP de l'env (`mcp__mcp-registry__*`) = connecteurs du compte Claude, PAS un annuaire d'URLs pour l'app → non utilisé. ✅ **Surfacer les outils MCP dans Ask AI = FAIT** (02/09, empty-state Cortex, cf. `[V1-FIXES]`). Puis v2 (orchestrateur, assignation aux agents).
> - **✅ UI honnête (choix user « tout miser sur MCP + UI honnête », 02/09, même branche `feat/mcp-ready-suggested-url`)** : le catalogue laissait croire que les génériques faisaient plus qu'ils ne font (chip « Observe - Feeds the Brain OS », badge coin « OAUTH »/« API KEY », indice « Colle la clé API » dans le dialog) alors que **connecter un générique = flux MCP only** (`isMcp` → on colle une URL MCP, jamais une clé morte ; le Brain n'ingère que l'interne). Passe **100% frontend** (aucun churn backend/test, la capability reste `observe`+`mcp`) : (1) sous-titre catalogue + bannière reformulés en **3 niveaux** (deep natif GitHub/Slack/Plane / MCP pour le reste / Soon=roadmap) ; (2) **badge coin** = « MCP » pour un connecteur MCP au lieu du faux « OAUTH/API KEY » natif (auth native gardée sur les profonds) ; (3) **chips caps** : « Observe » masqué sur les génériques (il ne se fait que via MCP), chip « MCP » redondant retiré (le coin le porte), « MCP-ready » conservé, « Observe/Act » gardés sur les profonds ; (4) **vue détail** : « What it can do » ne montre que « MCP - Plug... » sur un générique, section « Connection » reframe « MCP server - Connect by pasting... » au lieu de « API Key » ; (5) **dialog** : indice natif « Colle la clé API » masqué en mode MCP. Fichiers : `components/integrations/integrations-catalog.tsx` + `app/(protected)/[workspace]/settings/page.tsx`. tsc+eslint verts, **vérifié live (Brave)** : Plane=« API KEY »/« Observe », Linear/Asana=« MCP »/« MCP-ready », dialog + détail Linear 100% MCP. Committé sur `feat/mcp-ready-suggested-url` (5bddb3ed).
> - **✅ Catalogue de connecteurs en anglais (choix user « écris tout en anglais aussi », 02/09, même branche)** : le catalogue backend portait du FR qui fuyait dans l'UI anglaise (17 labels de catégorie via l'enum `ConnectorCategory`, ~120 descriptions, les 4 `defaultHint` + 3 `setupHint` inline, les libellés de champs). Traduit EN (sous-agent + revue de diff : **0 em-dash, 0 URL / 0 clé de champ touchée**, commentaires laissés en FR, `→` conservés). Rebuild backend exit 0 + **vérifié live (Brave)** (catégories + descriptions EN). Fichiers : `ConnectorCatalog.java`, `ConnectorCategory.java`.

> **▶ MAJ 31/08/2026 — Cache Redis des agrégats de dashboard (déployé sur main / prod).** `[PERF-CACHE-01]`
> - **Contexte** : le pill « Redis » du deck/dossier était suspecté décoratif. Vérif : **Redis EST déjà utilisé** — rate-limiting distribué (TF-SEC-011, `RedisRateLimitConfig` + `LettuceBasedProxyManager`, `ratelimit.distributed.enabled=true` par défaut en prod, bean eager → le backend ne boote pas sans Redis). Mon premier grep cherchait des patterns de *cache* (`@Cacheable`), d'où la fausse impression « inutilisé ». Décision user : lui donner **aussi** un rôle cache à vraie utilité.
> - **Fait** : `@Cacheable` sur `AnalyticsService.getKpis` (plusieurs count/aggregate coûteux, lu sur le dashboard, tolère 5 min de péremption). Clé = `slug:userId:projectId` car les projets visibles dépendent du rôle (`resolveProjectIds`) ; **l'autorisation reste en amont** (`WorkspaceAccessInterceptor` sur `/api/workspaces/{slug}/**`) donc un cache hit ne contourne aucun contrôle d'accès. `@EnableCaching` dans `shared/config/CacheConfig`. Deps : `spring-boot-starter-cache` + `spring-boot-starter-data-redis`. Config : `spring.cache.type=simple` en dev (mémoire, **0 Redis requis** localement) / `redis` en prod (`time-to-live=300000`, `key-prefix=tf:cache:`). **Health-check Redis désactivé** (`management.health.redis.enabled=false`) pour NE PAS coupler la liveness du backend à Redis (service support : un incident Redis ne doit pas marquer le backend DOWN).
> - **✅ Statut : DÉPLOYÉ** — 953 tests OK (`it.ps1 -Test ALL`) + **CI verte** (Backend Tests + porte de couverture JaCoCo passées, la config-classe vide ne fait pas tomber la couverture). **PR #196→dev, #197→main** (merge admin : backend identique à #196 déjà validé en CI complète ; auto-deploy VM1 = rebuild du rôle backend). Diff net `main..dev` = **les 5 fichiers cache uniquement** (les commits #191/#193 sont dans l'historique mais leur contenu était déjà dans main → pas de sur-release). **Soutenance inchangée** (aucune slide/§ ne dit « Redis inutilisé » ; le cache est un détail backend). **Brain OS MAJ** : `v1/02-produit/Backend.md` + `Infra.md`. Cible étendable aux autres endpoints analytics (throughput/burndown/capacity/workload) si besoin.

> **▶ MAJ 30/08/2026 — Alerting monitoring → Discord (déployé) + migration domaine landing.** `[OPS-ALERT + LANDING-DOMAIN]`
> - **Alerting (déployé VM2 + testé en prod)** : les 9 règles Prometheus n'étaient ni chargées ni routées (§9 dossier l'assumait). Ajouté `monitoring/vm2/prometheus/rules/taskforce-alerts.yml` (chargé via `rule_files:`) + section `alerting:` dans `prometheus.yml` → nouveau service **Alertmanager** (`prom/alertmanager:v0.27.0`, **`user: root`** pour lire le secret en chmod 600, sinon crash-loop `permission denied`) → **webhook Discord** (`alertmanager/alertmanager.yml`, **gitignoré** ; exemple versionné). Vérifié en prod : `alertmanager_notifications_total{integration="discord"}=1`, `_failed_total=0` (alerte de test reçue). Déployé par **scp sur VM2 `~/monitoring/vm2` + `docker compose up -d`** (monitoring manuel, hors auto-deploy). **PR #183→dev, #184→main.** Dossier §8.4/§9 basculés en « en service ».
> - **Domaine landing migré `taskforce.dev` → `taskforce-project.fr`** : `astro.config.mjs` + `sitemap.xml.ts` dérivent de `PUBLIC_BASE_DOMAIN` (comme `nav.ts`, plus de domaine en dur → corrige canonical/OG/sitemap) ; e-mail de contact `hello@`/`security@taskforce.dev` (jamais détenus) → **`contact@taskforce-project.fr`** (redirection OVH → Gmail, offre redirect-only) dans pages contact/légales/blog/`DemoForm` ; `robots.txt`/`security.txt`/`Proof.tsx` alignés. **Vérifié en prod : 0 `taskforce.dev`** sur les pages clés, sitemap 67/67. **PR #187→#188, #189→#190 (main).** Cf. [[landing-domain-email]].
> - **ZAP re-scan (30/08, baseline local front:3000/back:8080, post-HttpOnly)** : **backend 0H/0M/0L** (API 100 % auth → 401), **frontend 0H/4M (tous CSP `unsafe-inline`/`eval`, assumés)/4L** — inchangé vs 27/08 → rapports d'annexe régénérés + case ZAP de la grille C26 alignée sur « à la demande » (gaté `E2E_ENABLED`).
> - **Note traçabilité** : le hook a signalé `AnimatedNavIcon.tsx`/`nav.ts`/`site-icons.ts`/`global.css`/`login-intro.tsx` comme récents → ce sont des fichiers entrés dans l'arbre par les **merges de PR** (#185 méga-menu + autres, promus dev→main sur demande user), **pas des édits de ce lot**.

> **▶ MAJ 28/08/2026 — Audit doc↔code du Brain OS (`taskforce-docs`), demande user.** `[DOC-AUDIT]`
> - 5 agents (lecture seule) ont confronté au code réel : sécu (`07-securite`), API (`05-api`), fiches produit (`02-produit`), diagramme d'auth (`03-architecture`), registres (`09-audits`).
> - **Constat MAJEUR** : docs fortement périmées, qui **SOUS-VENDENT le travail fait**. De nombreux items marqués « ouverts » sont corrigés (`DT-001→006/009/013/014/017` ; `PC-003/005/006/008/011/013` ; la ligne P0 `PC-021bis`) ; les docs sécu **ignorent tout le durcissement OWASP** (SSRF/XSS/refresh-cookie/H1/H2/M3-M7/L8/L12 + CodeQL) ; **erreur grave** : `Auth_Autorisation` affirmait « aucune 2FA implémentée, ne pas la promettre » alors que la **2FA TOTP est livrée** (V79).
> - **FAIT — audit appliqué en ENTIER (5 commits `taskforce-docs`, 28/08)** : les 5 agents ont appliqué leurs correctifs (relus : diff stat + contrôle marqueurs périmés → 0), committés par domaine. `a134346` auth (Architecture cookie/modèle sans RefreshToken/migrations→V80, diagramme login 2FA+cookie, Frontend, Auth_Autorisation 2FA) ; `90996f9` sécu (5 docs 07-securite : cookie/2FA/OAuth/SSRF/CodeQL/H1/H2, CORS PC-027, résiduels STRIDE) ; `d7cd112` registres (DT-001→017 résolus + DT-023→033 sécu ; PC-003→013 résolus, PC-021bis retiré, PC-036→046 sécu) ; `7f04552` API (sections fantômes Chat/Discussions retirées, flux cookie, ~15 endpoints ajoutés, 42 ctrl/~235 endpoints) ; `2271e0e` produit (quotas/plans/V80/features livrées). Fichiers `16-memoire-rncp/Dossier_*` NON touchés (autre process).
> - Corrige aussi le bloc `[QA-51]` ci-dessous : SSRF/XSS/refresh **SONT commités + déployés sur main** (vérifié par l'agent), plus « local non commité ».

> **▶ MAJ 28/08/2026 — Revue OWASP Top 10 + 2 correctifs (SSRF webhook A10, XSS markdown A03).** `[QA-51]`
> - **Contexte** : demande user (revue auth → élargie OWASP Top 10, avec verdict). Bilan : **globalement bon (~7/10)** — A01 (autz service-level + H1/H2/M3 QA-49), A02 (TLS moderne, JWT RS256, mdp dans Keycloak), **A03 SQLi = nulle** (JPA + natives 100 % paramétrées `:param`), A05 (CORS 1 origine, actuator/swagger fermés, en-têtes forts), A06 (Trivy+Semgrep CI), A08, A09 : OK. **Point faible = A07** : tokens access+refresh en `localStorage` + CSP `script-src 'unsafe-inline'` (`next.config.ts:56`) → XSS = vol de session borné ≤3h. Cf. [[auth-security-posture]].
> - **A10 SSRF (corrigé)** : `WebhookService.fireOne` POSTait vers l'URL utilisateur **sans validation** → réseau Docker/Tailscale/localhost/métadonnées joignable (gated OWNER/ADMIN mais = client malveillant de son propre workspace). Fix : nouvelle util `shared/security/SsrfGuard` (rejette schéma non-http(s) + résout le DNS et bloque loopback/privé/link-local `169.254`/**CGNAT `100.64/10` = Tailscale**/ULA `fc00::/7` ; noms Docker → 172.16/12 bloqués). Câblée à `create`/`update` (→ **400** via `IllegalArgumentException`) + `fireOne` (défense en profondeur). Tests : `SsrfGuardTest` (@ParameterizedTest, IP littérales, sans DNS) + IT « OWNER refusé sur 169.254.169.254 » ; le test CRUD bascule sur IP TEST-NET `203.0.113.10` (hermétique).
> - **A03 XSS markdown (corrigé)** : `lightweight-markdown.tsx` (chat IA + notes Brain) rendait `<a href={url}>` sans valider le schéma → un `[x](javascript:…)` s'exécutait au clic (React ne neutralise PAS `javascript:`). Fix : `safeLinkUrl`/`safeImageUrl` → schéma dangereux rendu en **texte inerte**.
> - **⚠️ Statut : écrit en LOCAL, NON validé et NON commité** — Docker Desktop éteint → `it.ps1` (Postgres sibling) et `tsc` (volume Docker) non lançables. À valider soit en local (rallumer Docker) soit via la **CI d'une PR** (Backend + Frontend Tests se déclenchent, les chemins `backend/tf-api/**` + `frontend/**` étant touchés).
> - **Single-flight du refresh (FAIT + validé, 28/08)** : `client.ts` — un seul `/api/auth/refresh-token` en vol partagé par les 401 concurrents (`refreshInFlight` + `refreshAccessToken`), corrige les déconnexions parasites dues à la rotation Keycloak.
> - **Refresh token → cookie HttpOnly (A07) — FAIT + validé live, 28/08)** : nouveau `RefreshTokenCookie` (HttpOnly, `Path=/api/auth`, `SameSite=Lax`, `Secure` piloté par `auth.refresh-cookie.secure` = true prod / false dev, hôte-only, Max-Age 10800). Le refresh **part en cookie** (posé par `AuthController.login`/`refresh-token`, `OAuthLoginController.callback`) et **quitte le corps JSON** (`refreshToken: null`) ; `refresh-token` lit le cookie (repli corps pour transition), `logout` purge le cookie. `RefreshTokenRequest` non-obligatoire. Front : `auth-service.ts` ne stocke plus le refresh en localStorage, `client.ts` refresh via cookie (`withCredentials`, repli legacy + purge). Access token reste en localStorage (30 min, faible valeur — « access en mémoire » = étape ultérieure). Tests : `RefreshTokenCookieTest` 3/3 + `auth-service.test.ts` 36/36 + `tsc` clean + **curl live** (login→`Set-Cookie tf_refresh HttpOnly`/`refreshToken:null`→refresh via cookie seul + rotation→logout `Max-Age=0`). ⚠️ **À revérifier en prod** : cookie cross-sous-domaine `app.*`↔`api.*` (même site → Lax OK par raisonnement, non exercé en dev mono-hôte) → smoke-test login juste après le déploiement.
> - **CI PR #179 (28/08) — 2 régressions de tests, corrigées** (les Backend Tests n'avaient plus tourné depuis un moment, les PR récentes étant docs/ops filtrées par chemin) : **(1)** front `client.test.ts` (3 tests) assertait l'ANCIEN flux refresh (2 args axios sans `withCredentials` + refresh en localStorage) → alignés (27/27) ; **(2)** back **`AuthControllerWebMvcTest` (26) + `AuthUserControllersWebMvcTest`** : la nouvelle dépendance `RefreshTokenCookie` du constructeur n'était pas fournie dans la tranche `@WebMvcTest` → contexte KO → tous en erreur (le commentaire du test prévenait de ce piège) ; fix : `@Import({SecurityConfig, RefreshTokenCookie})` + assertions `refresh-token` alignées (cookie au lieu du corps ; corps vide → **401** au lieu de 400 puisque le champ n'est plus `@NotBlank`). `RetentionSchedulerTest` = **fausse piste** (passe 6/6 ; ses logs d'erreur *attendus* voisinaient l'échec dans le log CI).
> - **Reste proposé (NON fait)** : **CSP en nonce** (retirer `script-src 'unsafe-inline'` via `middleware.ts`) — priorité BAISSÉE depuis que le refresh est hors-JS (un XSS ne vole plus que l'access token ≤30 min) ; risque de casser Turnstile/hydratation → à valider en préprod.
> - **CI durcie (28/08, demande user)** : ajout de **CodeQL** (`.github/workflows/codeql.yml`) — analyse statique native GitHub avec suivi de flux (taint), **gratuite sur repo public**, Java + TS, `security-extended`, cron hebdo. Complète Semgrep (SAST) + Trivy (deps/secrets/misconfig) + ZAP (DAST) déjà en place. Résultats dans l'onglet Security.

> **▶ MAJ 27/08/2026 — Récap pré-bêta « au clair » (docs) + garde DAST.** `[DOC-QA-RECAP]`
> - Demande user : « c'est rangé où tous les trucs de la QA ? dans un dossier au clair j'espère ». Constat : tracé mais éclaté (QA-46→50 uniquement dans ce roadmap de 3600+ lignes ; scans dans `security-reports/`). → **`docs/qa/recap-prebeta.md`** : un seul doc indexant *trouvé → corrigé → déployé* (QA-46→50 + durcissement H1/H2/M3/M4/M5/M7/L8/L12 + M6/L10 vérifiés + L9/L11 acceptés + P1a-c), avec le tableau des scans ZAP (backend 0H/0M/0L/1I ; frontend 0H/4M/4L/4I → aucune High, Med/Low front à qualifier) et les liens artifacts. Sert aussi le dossier RNCP (E9/C11).
> - **Garde `.gitignore`** : rapports DAST complets (`*.html`/`*.json`/`*.png` sous `security-reports/`) exclus du repo **public** (l'user avait supprimé les HTML/JSON ; images recap gardées local) — seule la config `zap.yaml` reste versionnée.

> **▶ MAJ 27/08/2026 — P1c (bêta) : métriques via runbook SQL (option B, pas Grafana).** `[P1c]`
> - **Décision user = option B** : au lieu de brancher Grafana (VM2) sur la DB VM1 (datasource + user read-only + accès réseau à monter), un **runbook SQL prêt à lancer** — zéro infra, adapté à ~10-15 testeurs. Fichier versionné : `ops/analytics/beta-funnel.sql`.
> - **11 requêtes** bâties sur le **schéma RÉEL de prod** (colonnes + enums introspectés le 27/08, rien d'inventé) : Q1 population par cohorte/contexte · Q2 funnel global (n + % du signup) : signup→compte actif→onboardé→workspace→projet→issue→IA→feedback · Q2b/Q3 funnel par contexte / par cohorte · Q4 **fiche de suivi par testeur** (projets/issues créées·finies/commentaires/smart-assign/msgs IA/jours actifs/dernier actif) · Q5 rétention (revenu après J0, actif à J7+) · Q6 Smart Assign adoption+qualité (`assignment_events.decision_source='SMART_ASSIGN'`, `accepted`/`resolved_successfully`) · Q7 chat IA · Q8 fiabilité IA (`ai_runs`, status/fallback, 30j, santé infra) · Q9 conso IA vs plafond plan (FREE 100k/BASIC 500k/BUSINESS 2M ; `ai_token_usage.account_id`=**workspace_id**) · Q10 journal feedback · Q11 conversion (`subscription_history`).
> - **Périmètre `:POP`** (psql var) : défaut = `u.beta_cohort IS NOT NULL` (la bêta) ; `-v POP=TRUE` pour un dry-run sur tous les users ; `\set POP u.beta_cohort = 'beta_2026_09'` pour une cohorte. Vues temporaires session-scoped (`beta_pop`/`beta_flags`/`beta_activity`).
> - **Validé end-to-end contre la prod** (`-v POP=TRUE`, 17 users) : **0 erreur psql**, sorties cohérentes (Smart Assign 27 suggestions/6 acceptées/6 résolues ; chat 1 user/5288 tokens ; rétention capte Pierre ; Q9 % plafond OK). Lancement : `scp` le fichier sur VM1 → `docker cp` dans `taskforce-postgres-prod` → `psql -f` (méthode sans pipe pour ne pas corrompre l'UTF-8).
> - **Reste bêta (hors code, user)** : formulaire d'intake + tag `beta_cohort`/`beta_context` par SQL, fiche par testeur, emails d'onboarding. Webhook Stripe test (sync plan post-paiement).

> **▶ MAJ 27/08/2026 — P1a+P1b (bêta) : cohorte (V80) + export projet serveur (JSON/CSV).** `[P1a-b]`
> - **Fait (branche `feat/beta-p1-cohort`, PAS encore déployé)** : `V80__user_beta_cohort.sql` ajoute `beta_cohort VARCHAR(50)` + `beta_context VARCHAR(20)` (CHECK `PROFESSIONAL|PERSONAL|FREE`) + index sur `users` ; champs `betaCohort`/`betaContext` sur l'entité `User`. La cohorte est sur la PERSONNE (un user ∈ plusieurs workspaces). **Peuplement manuel par testeur** (formulaire d'intake) : `UPDATE users SET beta_cohort='beta_2026_09', beta_context='PROFESSIONAL' WHERE email='…';` — pas de tag auto au signup (app publique → éviter de taguer les non-testeurs ; précis pour ~10-15 pers.).
> - **Keycloak M7 — VÉRIFIÉ (déjà sûr, aucun changement)** : realm `verifyEmail=true`, `duplicateEmailsAllowed=false` ; IdP Google+GitHub `trustEmail=true` (légitime — ils ne renvoient que du vérifié). Couvre la prise de contrôle par e-mail, en plus du code M7.
> - **P1b (fait, même branche)** : `ProjectExportController` (`GET /api/workspaces/{slug}/projects/{projectId}/export?format=json|csv`, **téléchargement** `Content-Disposition`) + `ProjectExportService` (réutilise `IssueService.listIssues/listComments/listActivity` — déjà autorisé par visibilité projet) → export **COMPLET** (issues + descriptions + commentaires + activité en JSON) et **CSV à plat enrichi** (description + nb commentaires vs l'export client existant, superficiel). Records `ProjectExport`/`IssueExport`. Tests `ProjectExportServiceTest` (assemblage + toCsv/échappement RFC 4180).
> - **P1b front (fait, branche `feat/beta-p1-export-ui`)** : bouton « Export » (dropdown **CSV / JSON complet**) sur le board projet → `downloadProjectExport` (blob authentifié, `PROJECT_ROUTES.EXPORT`) ; remplace l'ancien export CSV client superficiel. **P1c FAIT** (cf. bloc daté ci-dessus) — option **B** (runbook SQL), pas Grafana.
...
> - **H2 (critique — fuite temps réel cross-tenant)** : `StompAuthInterceptor` n'autorisait QUE `/topic/notifications.{userId}` ; `projects.{id}`/`analysis.{ws}` n'exigeaient qu'une session authentifiée → streamer l'activité/analyses d'un autre compte. **Fix** : nouveau `RealtimeAuthorizationService` (core, lit la base — ce que le commentaire du code prescrivait) ; l'intercepteur autorise chaque canal (visibilité projet / appartenance workspace).
> - **M3 (projets privés)** : 6 sous-lectures d'issue (`listComments/listActivity/listRelations/listChildren/listTypes/resolveChecklistScope`) vérifiaient l'appartenance workspace mais pas `assertCanView`. **Fix** : garde ajoutée aux 6.
> - **M5 (bypass rate-limit)** : `RateLimitFilter.resolveClientIp` faisait confiance à la 1re valeur de `X-Forwarded-For` (spoofable). **Fix** : `CF-Connecting-IP` (Cloudflare, non spoofable) → `X-Real-IP` → **dernier** hop XFF → remoteAddr.
> - **M7 (OAuth)** : `completeOAuthLogin` liait par email sans vérifier `email_verified`. **Fix** : refus si `email_verified` explicitement `false` (Google/GitHub ne renvoient que du vérifié → logins légitimes intacts ; durcissement complet = realm Keycloak).
> - **L12** : `application-prod.yml` `include-message: always` → `never` (erreurs métier via `ApiResponse.error` inchangées).
> - **Tests** : `StompAuthInterceptorTest` (+ mock `realtimeAuth` + 3 tests canal), `GitHubIntegrationServiceIntegrationTest` (membership OWNER + signatures + H1 cross-tenant→404), `IntegrationControllerWebMvcTest` (stubs + user), `GitHubIntegrationContractTest` (7e arg constructeur `visibilityGuard`), `RateLimitFilterTest` (dernier hop + CF-Connecting-IP). `it.ps1 -Test ALL` (2e passe après fix de compilation du ContractTest).
> - **Décisions user** : signup **reste ouvert** (veut tester manuel+GitHub+Google avec les mails autorisés console Google ; supprimera Alizée/Yseult/cevival avec accord pour repartir de 0) ; **corriger tout** (critique+moyen+reste) ; export **serveur JSON+CSV** (option A) ; pas de dashboard bêta in-app → **Grafana** ; vague **10→24 sept 2026** (deux jeudis).
> - **Reste (prochains lots)** : M4 (proxy fichiers Brain public — gestion front `<img>`), M6 (secrets Turnstile/challenge en prod ?), L8–L11 ; puis **P1** : cohorte (V80 `beta_cohort`/`beta_context`), export projet serveur, métriques bêta via Grafana. **Repo → privé** (action user).

> **▶ MAJ 27/08/2026 — QA-48 : Stripe prod activé en mode TEST (clés du user) + plumbing compose des price-ids.** `[QA-48]`
> - **Constat** : en prod, TOUTES les variables Stripe de `.env.prod` étaient **vides** (secret + public + price-ids) → checkout impossible. Le user a fourni ses clés **TEST** dans `.env.dev` (compte `51TuaJN`).
> - **Fait** : secret + public + price-ids Basic/Business copiés `.env.dev` → `.env.prod` (VM1) via canal SSH chiffré (CR Windows strippés, secret jamais échoté ; backup `.env.prod.bak` gardé). **Validé** : 2 sessions checkout test créées via l'API Stripe (BASIC + BUSINESS → HTTP 200, `cs_test_…`, 3 sièges).
> - **Bug plumbing (le vrai blocage app)** : `docker-compose.prod.yml` ne passait au conteneur backend QUE `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` (`--env-file` ne sert qu'à l'interpolation compose, pas à peupler le conteneur) → les `STRIPE_PRICE_ID_*` n'atteignaient JAMAIS l'app → `getPriceIdForPlan` lisait `""`. **Fix** : ajout de `STRIPE_PUBLIC_KEY` + `STRIPE_PRICE_ID_BASIC` + `STRIPE_PRICE_ID_BUSINESS` au service `backend`. `scripts/auto-deploy.sh` rebuild le backend sur tout diff de `docker-compose.prod.yml`.
> - **Reste (action user, hors code)** : le **webhook** prod n'est pas configuré (`STRIPE_WEBHOOK_SECRET` vide) → après un paiement test le **redirect checkout marche** mais le **plan ne se sync pas auto** (MAJ via webhook). Créer un endpoint webhook (dashboard Stripe test → `…/api/stripe/webhook`) + poser le secret dans `.env.prod`. Cf. [[billing-stripe-price-ids-prod]].

> **▶ MAJ 27/08/2026 — QA-47 : smart-assign 500 au plafond (repli déterministe cassé), affichage quota > 100 %, avatars filtre Assigné, popover Cortex rogné, billing (paiement KO) — feu vert deploy + reset quota.** `[QA-47]`
> - **Smart-assign 500 au plafond (LE vrai bug, retour user « j'ai pas de repli déterministe si plus de model »)** : logs prod = `UnexpectedRollbackException: Transaction ... marked as rollback-only` (`SmartAssignService.recommend` → `IssueController:419`). Cause : `assertWithinQuota` (`@Transactional(readOnly)`) tourne DANS la tx de `recommend()` ; sa levée au plafond marque la tx partagée **rollback-only** → le repli Java s'exécute bien mais le **commit final** casse en 500. (Le cas LLM 429/503 tombe en repli propre — SEUL le plafond 500ait.) **Fix** : `@Transactional(readOnly = true, noRollbackFor = IllegalStateException.class)` sur le gate — pur signal de contrôle, ne doit jamais forcer un rollback ; le chat (hors tx) garde son 409 (propagation inchangée).
> - **Affichage plafonné à 100 % (retour user : « faut pas afficher plus de 100 %, on garde le dépassement en DB pour le coût »)** : `AiUsageService.getUsage` borne le total exposé à `min(réel, plafond)` + prorata `prompt↑`/`completion↓` ; le RÉEL reste en base (`ai_token_usage`). Corrige popover Cortex + Settings usage + bandeau Billing (« 105k/100k » → « 100k/100k »). Le `%` était déjà borné (`pctOf`), pas les tokens.
> - **Chat bloqué + CTA (QA-46) = voulu** ; smart-assign reste DISPONIBLE (repli Java). Distinction validée user.
> - **Avatars dans le filtre Assigné (retour user)** : `deriveAssigneeOptions` porte `email`+`avatarUrl` ; `issue-filters.tsx` rend `UserAvatar` (variantes inline + popover) ; « Non assigné » (FR résiduel) → « Unassigned ».
> - **Popover Cortex rogné (retour user « qu'il soit par-dessus, z-index »)** : `cortex-usage.tsx` — le popover `absolute bottom-full` (s'ouvre vers le haut) était **rogné par l'`overflow`** du panneau flottant Cortex et passait sous le sheet. **Porté sur `<body>`** (`createPortal`, `position:fixed` ancré au trigger, `z-[120]`, reposition scroll/resize, ferme au clic hors trigger+popover) → flotte par-dessus tout, jamais clippé.
> - **Billing / paiement KO (retour user)** : env prod = `STRIPE_SECRET_KEY`/`WEBHOOK`/`SUCCESS`/`CANCEL` présents mais **`STRIPE_PRICE_ID_BASIC` / `STRIPE_PRICE_ID_BUSINESS` ABSENTS** → `getPriceIdForPlan` renvoyait `""` → checkout Stripe avec prix vide = échec. **Fix code** : `StripeService.getPriceIdForPlan` lève un message clair (→ 409) si le price-id n'est pas configuré (au lieu d'un 502 Stripe opaque) ; `billing/page.tsx` remonte le message serveur sur 409. **Action USER (hors code)** : créer les prix récurrents Basic/Business dans le dashboard Stripe **TEST** + poser `STRIPE_PRICE_ID_BASIC` / `STRIPE_PRICE_ID_BUSINESS` dans `.env.prod` + recréer le backend. Portail (rétrogradation/factures) OK une fois sur un plan payant.
> - **Décisions user** : (1) `pierre` reste FREE/bloqué — le blocage marche, c'est voulu ; (2) quotas gardés (QA-46) ; (3) **reset quota pierre à 0** en fin de tour ; (4) **deploy dev→main**.
> - **Tests** : back `it.ps1 -Test ALL` (lancé) ; front = revue statique (node_modules = volume, stack dev éteinte) → filet CI. **Déploiement** : back (VM1) = noRollbackFor + cap + stripe guard + gate small-talk (QA-46) ; front (VM2) = avatars + popover portal + billing msg + QuotaNotice/CTA (QA-46) + taille texte + largeur dropdown.

> **▶ MAJ 27/08/2026 — QA-46 : blocage du quota IA (« j'ai pas de blocage niveau IA ? Faudrait bloquer ça ») + quotas cohérents par plan + taille texte Smart Assign + largeur dropdown Labels.** `[QA-46]`
> - **Diagnostic (prod)** : image backend **rebuild 26/08 17:36** → le gate EST déployé. Compte `pierre` (id=1) = **FREE, 105 027/100 000 tokens** (85 req) → au plafond. Le gate `AiUsageService.assertWithinQuota` (→ **409** via `GlobalExceptionHandler`) est **correct de bout en bout** ET appelé (`AgentService.run:122` + les 6 autres chemins via `AiMeter.metered`). Le ressenti « ça ne bloque pas » venait de **3 trous d'étanchéité / visibilité**, pas du gate.
> - **Trou 1 (étanchéité, back)** : `AgentService.runConversational` (small-talk) **n'était PAS gated** → un compte au plafond contournait en enchaînant « salut/merci ». **Fix** : `assertWithinQuota(workspaceId)` ajouté avant l'appel LLM (→ 409, repli côté front).
> - **Trou 2 (visibilité, front)** : le `catch` de `agent-chat.tsx` aplatissait **toute** erreur (dont le 409) en « Sorry, I couldn't answer. Please try again. » → le blocage passait pour un bug transitoire. **Fix** : helper `getErrorStatus()` (`client.ts`) → sur **409**, message d'upsell clair (« You've reached this workspace's monthly AI limit. Upgrade your plan… »).
> - **Trou 3 (smart-assign)** : déjà OK — repli **Java** côté serveur + bandeau « AI fallback active » (dégradation gracieuse par design, pas un 409).
> - **(d) Modèles par feature — DÉJÀ câblé** : `resolve_tier` (ai-service `ollama_gateway.py`) route `fast`→petit modèle (smart-assign, chat rapide) / `deep`→gros modèle **+ raisonnement** (analyse) / `standard`→gros. Quota **par tokens** (uniforme) → une feature lourde coûte **déjà** plus de quota (elle génère plus de tokens). **Aucune pondération par feature nécessaire.** Plafonds actuels FREE 100k / BASIC 500k / BUSINESS 2M / ENTERPRISE illimité — mesuré **~1,2k tokens/action** → FREE ≈ 80 actions/mois. **Valeurs = décision pricing → à confirmer user** (`AiUsageService.limitFor`, encore commenté « placeholder »).
> - **(a) Taille texte** : `smart-assign-panel.tsx` — échelle typo montée d'un cran (base lisible `text-sm`/`text-xs`), **garde les rem** → suit toujours le réglage a11y de QA-45.
> - **(b) Largeur dropdown** : `issue-sheet.tsx` Labels (DropdownMenu) — contenu `min-w-40 align=end` → `w-[var(--radix-dropdown-menu-trigger-width)]` = largeur du trigger/input.
> - **⚠️ DÉMO (bloquant avant deploy back)** : quota **par compte** (owner) agrégé sur tous ses workspaces → `pierre` (id=1) possède AUSSI le workspace démo **Nimbus** → activer le blocage **coupe la démo**. Avant deploy : **bump `pierre` en ENTERPRISE** (illimité, compte fondateur) ou reset usage août. Blast radius = pierre seul (comptes id=6/id=9 très en-dessous).
> - **Décisions user (27/08)** : (1) **quotas gardés** 100k / 500k / 2M / ∞ (commentaire `limitFor` **finalisé**, plus « placeholder ») ; (2) **`pierre` NON touché** — « si le blocage est fait, c'est good » (reste FREE au plafond → en démo, Cortex chat affiche la carte d'info, assumé) ; (3) le 409 doit être une **carte d'INFO + CTA**, pas un message d'erreur (« au lieu de mettre une erreur, un truc d'info avec un CTA ») ; (4) **rien déployé, tout en local**.
> - **Implémenté** : `QuotaNotice` (carte de tour assistant, bord/fond `primary`, **CTA « Upgrade plan » → `/{slug}/billing`**) rendue via un tour `kind:"quota"` (helper `getErrorStatus` sur 409, `agent-chat.tsx`) ; gate small-talk (`runConversational`) ; commentaire `limitFor` arrêté (routing modèle par feature = déjà via le tier, aucune pondération) ; test back `run_conversational_blocked_when_quota_reached` (409 + aucun appel LLM).
> - **Non fait (assumé, à faire avant un futur déploiement)** : `tsc` front **non lançable sur l'hôte** (node_modules = volume Docker, stack dev éteinte) → **revue statique seulement** ; suite back `it.ps1 -Test ALL` **non lancée** (lourde). **Aucun commit, aucun push, aucun changement prod.** `pierre` reste bloqué en prod (état actuel, voulu).
...
> **▶ MAJ 26/08/2026 — QA-42 : landing — icônes des dropdowns ré-animées avec la VRAIE lib (framer-motion, comme la webapp) + vraies captures produit (hero + section Smart Assign) (PR #156→dev + #157→main, déployé Vercel prod `cdda7731`).** `[QA-42]`
> - **Icônes des méga-menus (retour user : « les mm animations que la webapp, la lib d'animation »)** : approche CSS de QA-41 **remplacée** par le mécanisme framer-motion de la webapp — `AnimatedNavIcon.tsx` porté (`LazyMotion`+`domMin`+`m`, variant « wobble » rotation+échelle, `useAnimation`, déclenché au survol/focus de la carte via le plus proche `a`/`button`). framer-motion **déjà dans les deps du site** (^12.40) → aucun ajout. Classes CSS `nav-ic`/keyframes retirées de `global.css`. `reduced-motion` respecté.
> - **Vraies captures produit (retour user)** : chrome navigateur **cropé** (sharp, top 58 px) + **webp ~59 Ko** (depuis PNG 180-240 Ko) dans `public/screens/`. **Hero** (`Hero.tsx`) : `SpecPanel` (fausse UI) → **dashboard réel** dans `ScreenFrame` (lift au survol, 2 chips reformulés « Runs on your models » / « You approve »). **Nouvelle section `SmartAssign.astro`** (après Before/After) : capture du dialogue Smart Assign + **détail zoom révélé au survol**. Composant réutilisable `ScreenFrame.astro` (coins/bord/ombre, hover CSS, reduced-motion).
> - **Vérifié en live (dev astro)** : 0 erreur compile, hero `dashboard.webp` chargé (1600×862), section « Smart Assign · live » au bon endroit, frame+zoom câblés, 3 images servies 200 webp. **Reste** : committer + déployer via Vercel/main après feu vert user (les icônes CSS de QA-41 sont en prod le temps du remplacement).

> **▶ MAJ 26/08/2026 — QA-41 : toasts 100 % anglais + icônes animées des dropdowns du site (PR #154→dev + #155→main ; toasts déployés prod VM2 `34cf4375` ; icônes CSS remplacées par QA-42).** `[QA-41]`
> - **Toasts (webapp)** : les derniers toasts **français en dur** passés en anglais (le reste l'était déjà ; les toasts d'auth passent par l'i18n `t.*` → déjà EN quand la langue de l'app = EN, non touchés). `lib/api/client.ts` (réseau « Couldn't reach the server » / serveur / 429 + fallback `message`), `dashboard-card.tsx` (resize/remove). Les tests `lib/api/client.test.ts` encodaient les anciennes chaînes FR → **4 assertions réalignées EN** (`Couldn't reach the server` / `Server error` / `Too many requests`, `42s`, `a few seconds`). **Bonus** : la **page 500 `app/error.tsx`** était **entièrement en français** (toast + h1 « Erreur inattendue » + corps + bouton « Tableau de bord ») dans une app sinon anglaise → passée en anglais (toast + page).
> - **Icônes des méga-menus (site Astro)** : **pop élastique au survol/focus** des cartes Product + Resources (`SiteHeader.tsx` → classes `nav-card`/`nav-ic` ; keyframes `nav-ic-pop` + règle dans `global.css`). **100 % CSS** — aucun JS, aucune dépendance (pas de framer-motion comme la webapp) → **scores Lighthouse 100/100/100 du site préservés** ; `prefers-reduced-motion` respecté. **Vérifié en live** (dev server astro) : 8 cartes/8 icônes câblées, keyframes chargées, règle `.nav-card:hover .nav-ic { animation: nav-ic-pop .42s cubic-bezier(.34,1.56,.64,1); transform: scale(1.12) }` présente.
> - **Déploiement** (à valider) : webapp (toasts) via **dev→main → VM2** ; site (icônes) via Vercel (**main**). **Reste** : committer (2 concerns atomiques) + déployer après feu vert user.

> **▶ MAJ 26/08/2026 — QA-40 : LCP dashboard — imports dynamiques des cartes Recharts (PR #152→dev + #153→main, déployé prod `0d5aa2ec`, VM2).** `[QA-40]`
...
> - **Reste** : pass **LCP** (imports dynamiques recharts/force-graph) — 2ᵉ moitié du « oui les deux ».

> **▶ MAJ 26/08/2026 — QA-38 : intro visible en dark + lot Lighthouse #1 (Best Practices 77→~96, SEO 92→100) (branche `perf/lighthouse-bp-seo`, PR #149→dev + #150→main, déployé prod `fde16148`).** `[QA-38]`
...
> - **Best Practices — `deprecations` (weight 5) corrigé** : l'écouteur `unload` déprécié venait de **SockJS** (`sockjs-client`), importé en STATIQUE dans les 2 hooks temps réel → son module s'exécutait au chargement et enregistrait l'`unload` MÊME sans être utilisé (les hooks préfèrent déjà le **WS natif** `/ws`, SockJS = fallback rare). **Import rendu paresseux** (`await import("sockjs-client")` seulement dans la branche fallback) : chemin nominal sans `unload`, fallback intact, bundle initial allégé. (Backend `WebSocketConfig` confirmé : `/ws` natif + `/ws-sockjs` SockJS.) → 77 **~96** attendu.
> - **SEO — `robots-txt` (weight 1) corrigé** : `/robots.txt` était capté par le catch-all `[workspace]` → HTML du SPA (« Unknown directive »). Ajout `app/robots.ts` (route metadata) → `Disallow: /` **valide ET approprié** (app privée). → 92 **100** attendu.
> - **Perf — config** : `experimental.optimizePackageImports` (lucide/recharts/framer-motion/date-fns) — gain modeste. Le VRAI levier Perf = **LCP 6,5 s (weight 25 @ 0.09)**, gated par l'hydratation (le H1 rend après le bundle client) → **imports dynamiques** des gros composants sous la ligne de flottaison (recharts, force-graph Brain) : pass Perf dédié à venir (itératif, mesuré).
> - **Bandeau Labs (correctif du QA-37)** : le liseré `px-1.5 pb-1.5` faisait **déborder le bg sur les côtés/bas** et les coins ne matchaient pas le bandeau (crop différent). Refait : **UN seul calque** de dégradé couvre bandeau + coins (même crop continu, `center 78%`), app en **plein cadre** (`rounded-t-2xl`, côtés/bas = l'app). `lab-shell.tsx`.
> - **Décisions user (« oui les deux, vas-y »)** : avatar resize + pass LCP dynamique **validés** → à faire.
> - **Reste (décisions)** : (a) **avatar 1,4 Mo** plein format = ~60 % du poids page → redimensionner à l'upload (backend) ; (b) `errors-in-console` = beacon Cloudflare bloqué par le **bloqueur du navigateur testeur** (`ERR_BLOCKED_BY_CLIENT`, weight 1) → passe sur profil propre, sinon couper l'auto-injection CF Analytics ; (c) pass LCP dynamique. `tsc`/ESLint verts.

> **▶ MAJ 26/08/2026 — QA-37 : icônes animées « clés » au survol + cadre image du bandeau Labs (branche `feat/animated-icons-labs-frame`, PR #147→dev + #148→main, déployé prod `bba9df02`).** `[QA-37]`
> - **Icônes animées au survol** (retour user : effet façon **animateicons.in**, « sur les icônes »). **Portée resserrée aux spots clés** (nav sidebar + bouton *créer* + cloche topbar), **décidée avec le user contre « toutes »** (155 fichiers importent lucide → tout animer alourdirait le JS d'hydratation, **à rebours de l'objectif perf Lighthouse** demandé en parallèle). **9 composants bespoke** tirés du registry animateicons.in (`components/ui/icons/*-icon.tsx`), **réécrits sur `framer-motion`** (déjà dans le bundle) au lieu du paquet `motion` → pas de 2ᵉ moteur d'anim ; `LazyMotion`+`domMin` (mini), **`prefers-reduced-motion` respecté**. 2 glyphes absents du registry (**Settings2**, **HelpCircle**) **conservés à l'identique** et animés au niveau conteneur (`animated-lucide.tsx`). Déclenchement sur le **survol de la LIGNE entière** (pas du petit glyphe) via un binder `AnimatedNavIcon` qui lie l'écouteur au plus proche `a`/`button` (ref impérative `start/stopAnimation`, `isAnimated={false}` coupe l'auto-survol interne). **Vitrine (Astro) intouchée** → scores 100/100/100 préservés (décision user). `tsc` + ESLint **verts** ; vendored icons exemptés du lint (`as any`).
> - **Cadre bandeau Labs** (retour user « les border aussi avec le bg image ») : l'app est désormais **encadrée** par un liseré du dégradé Labs sur **côtés + bas** (coins `rounded-2xl`, `px-1.5 pb-1.5`) — plus seulement le bandeau du haut (`lab-shell.tsx`).
> - **Reste** : **Lighthouse app** (Best Practices : CSP **nonce** vs `script-src 'unsafe-inline'`, erreurs console, dimensions images ; Perf : imports dynamiques, préchargement police ; SEO : meta/title par page) — rapport avant/après à **mesurer par le user** (session prod authentifiée, non lançable d'ici). Déploiement icônes+bandeau **à valider**.

> **▶ MAJ 26/08/2026 — QA-36 : avatar 401 (fix sécu), intro plus longue, bandeau Labs (bas de l'image) (branche `fix/avatar-intro-labs`, déployé).** `[QA-36]`
> - **Avatar « ne change pas du tout »** : l'upload marchait (`POST /api/users/me/avatar` authentifié → MinIO), mais l'endpoint qui **sert** l'avatar (`GET /api/files/avatars/{id}`, `FileController`, commenté « public ») n'était **PAS dans `PUBLIC_MATCHERS`** de `SecurityConfig` (seul `/api/files/brain/**` l'était) → un `<img>` (sans header Bearer) prenait **401** → image cassée. Ajouté **`/api/files/avatars/*`** aux endpoints publics. Test `FileControllerWebMvcTest.getAvatar_unauthenticated` mis à jour (il **encodait le bug** : attendait 401 → maintenant assert **200 sans JWT**). Vérifié en prod : `avatars/1000` **401 → 404** après le fix (public, pas encore d'objet Minio → un upload donnera 200).
> - **Intro de connexion allongée** (retour user « laisser le temps d'apprécier ») : reveal **~2,9 s → ~4,4 s** (halo/vague/logo au prorata), `components/layout/login-intro.tsx`.
> - **Bandeau Labs** : montre désormais le **BAS** de `labs-wave.jpg` (`background-position: center 78%` sur le bandeau) — formes + couleurs — avec **texte blanc + ombre**, au lieu du haut clair. Position exacte à affiner en live (page Labs auth-gated).

> **▶ MAJ 26/08/2026 — QA-35 : 2FA (TOTP) géré par l'app — plus de page Keycloak (branche `feat/2fa-app-managed`, EN COURS : suite backend en vérif, pas encore déployé).** `[QA-35]`
> - **Pourquoi** : « Enable 2FA » renvoyait vers la **page hébergée Keycloak** (`executeActionsEmail(CONFIGURE_TOTP)`) — Keycloak n'offre pas d'API propre pour enregistrer un secret TOTP → on gère le 2FA **dans l'app** (même logique que le reset mot de passe).
> - **Backend** : migration **V79** (`user_two_factor` : `user_id` unique, `secret`, `enabled`, `confirmed_at`) ; `TotpService` (RFC 6238 HmacSHA1, 6 chiffres/30 s, Base32 RFC 4648, **zéro dépendance**, tolérance ±1 fenêtre) ; `TwoFactorService` (`setup` → secret + URI otpauth *inactif* ; `confirm` → 1er code valide active ; `isEnabled` ; `verifyOrThrow` ; `disable`). Endpoints : `POST /api/users/me/2fa/{setup,confirm}`, `GET`/`DELETE /api/users/me/2fa` (les 3 délégations Keycloak retirées du `UserController`). **Login adapté** (`AuthService.login`) : après le mot de passe, si 2FA actif → **`twoFactorRequired:true`** (aucun token) tant qu'un code TOTP valide n'est pas fourni ; `LoginRequest.totp` + `AuthResponse.twoFactorRequired`. **Sécurité** : le backend (client confidentiel) est le **seul chemin d'auth** → l'imposer au login suffit. **Sans impact pour les comptes sans 2FA** (branche inactive) → pas de lockout global ; recouvrement d'un compte verrouillé = supprimer la ligne `user_two_factor`.
> - **Front** : `setupTwoFactor`/`confirmTwoFactor` + **`TwoFactorSetupDialog`** (QR via lib **`qrcode`** ajoutée + secret copiable) dans *Réglages › Sécurité* ; `useAuth().login` renvoie désormais `AuthResponse` (pour remonter `twoFactorRequired`) ; **`LoginForm`** gagne une **étape « code »**. `enableTwoFactor` (Keycloak) retiré.
> - **Tests** : `TotpServiceTest` (round-trip, fenêtre ±1, rejets), `TwoFactorServiceTest` (setup/confirm/verify/disable), `AuthServiceTest` (+3 cas : sans code → `twoFactorRequired`, code valide → tokens, code invalide → exception), +3 cas front `login-form.test`. Front **tsc/ESLint/coverage verts** (login-form 91,8 %, 831 tests).
> - **Livré + déployé (26/08)** : backend + front CI verts, **migration V79 appliquée VM1**, QR dans notre dialog, étape code au login. Compte user vérifié = **aucun TOTP Keycloak résiduel** (que `password`).
> - **Polish (2ᵉ lot déployé)** : **i18n** de l'étape code + du dialog (clés `twoFactor*` FR+EN) ; **rate-limit TOTP** (backend en mémoire `TwoFactorService` : 6 codes/2 min → blocage, + garde client `login-2fa`) ; **intro de marque aussi après un login par mot de passe** (le `LoginForm` pose `tf.intro`, plus seulement le callback OAuth).
> - **Reste** : ⚠️ le **login OAuth (GitHub/Google) ne passe PAS par notre 2FA** — le check est seulement dans `AuthService.login` (ROPC), `oauthCallback` l'ignore. À décider : soit « le fournisseur a déjà authentifié » (design courant), soit ajouter un flux challenge-token pour l'imposer. Rate-limit TOTP en mémoire → à porter sur **Redis** si scale horizontal. MAJ `taskforce-docs` (API + modèle).

> **▶ MAJ 26/08/2026 — QA-32 : SMTP Keycloak prod (reset/2FA), membres dans les réglages, login premium, bandeau Labs image (branche `fix/qa-smtp-members-login-labs`, PR #133→dev + #134→main, déployé VM2).** `[QA-32]`
> - **① reset MDP + 2FA RÉPARÉS (le vrai bug, clôt le follow-up de QA-24)** : ces emails partent de **Keycloak** (`executeActionsEmail` UPDATE_PASSWORD / CONFIGURE_TOTP), dont le realm prod avait `smtpServer: {}` — Brevo n'était branché que sur Spring. **Configuré en live sur VM1 via `kcadm`** (relais Brevo `smtp-relay.brevo.com:587`, STARTTLS, from `noreply@taskforce-project.fr`) + realm JSON source à jour (creds en placeholders `${MAIL_*}`, zéro secret committé). ⚠️ **Gotchas kcadm** : `-s 'smtpServer.x=…'` ne fusionne PAS (le dernier `-s` écrase → `{}`) et `-s 'smtpServer={json}'` est pris comme **string** (ignoré, exit 0 trompeur) → **utiliser `update realms/<r> -f fichier.json`** ; et `get --fields smtpServer` **ment** (map projetée → `{}`), vérifier sur le realm **complet**.
> - **② membres consolidés dans Réglages** : « Members » était en double (onglet projet **+** section réglages). Toute la gestion (inviter/rôles/retirer) vit désormais dans **Réglages › Membres** via `ProjectMembersManager` (composant partagé), avec **recherche + pagination** (8/page). **Onglet « Members » retiré** de la nav projet (Teams reste) ; route `/members` conservée en relais (liens profonds). **Avatars corrigés** : la liste réglages n'utilisait pas `UserAvatar` (initiales « maison ») → tous « PM » identiques ; désormais photo OAuth ou identicon **unique par email**.
> - **③ login OAuth premium** : `AuthTransition` refait — fond aurora animé (nappes floues qui dérivent), anneau dégradé rotatif + halo, **jalons qui se cochent** (Verifying → Loading → Setting up), « **Welcome back, {prénom}** » (câblé depuis le callback via `userName`). Framer Motion + Tailwind, CSP-safe.
> - **④ bandeau Labs** : fond bleu plat (`bg-blue-600` de `LabShell`) → image `labs-wave.jpg` (dégradé pêche→rose→bleu clair), texte du bandeau passé en foncé pour rester lisible.
> - Typecheck `tsc` + ESLint + CI (Build/Tests/Semgrep/Trivy/Labels) **verts**, déployé VM2 (conteneur healthy, `app`+`/auth/login` 200). Email reset **confirmé reçu** (user).
> - **Suite (QA-33, déployée dans le même lot)** :
>   - **(a) Reset password sans Keycloak visible** — le bouton *Réglages › Sécurité › Reset password* tapait l'action Keycloak (`executeActionsEmail` → **page hébergée Keycloak** + email par défaut **non brandé**). **Recâblé vers le parcours natif `/auth/forgot-password`** (déjà existant : code OTP via template Thymeleaf `reset-password-email` → `KeycloakService.resetPassword` par l'**API admin**) → l'utilisateur ne voit plus jamais Keycloak, email **brandé**.
>   - **(b) Intro de connexion** — itérée : écran premium violet ③ → cinématique blanche abstraite (Canvas) → **direction finale « ElevenLabs »** : le **logo TaskForce s'illumine**, une **vague (blow-up)** ouvre l'app, puis **dissolution/zoom qui révèle l'app** (`components/layout/login-intro.tsx`, framer-motion). Jouée **en OVERLAY sur l'`AppShell`** pendant que l'app charge dessous (pas de loader, pas de coupure) : le callback OAuth pose `sessionStorage['tf.intro']` + joue le `hold` (logo) pendant l'échange, l'`AppShell` consomme le drapeau et joue le `reveal`. Fond = `--background` (thème-aware → dissolution sans flash). `cortex-awakening.tsx` + `AuthTransition` supprimés. **Vague refaite en ONDULATION** (retour user « vraiment une vague ») : le dégradé **Labs** (`labs-wave.jpg`) déformé par un filtre SVG `feTurbulence`+`feDisplacementMap` (baseFrequency animée en SMIL) au lieu d'anneaux géométriques. **Loader du début supprimé** : l'overlay est remonté au `ProtectedLayout` (au-dessus de l'AppShell) et lu **en synchrone au 1er paint** (`useState` init depuis `sessionStorage`) → il couvre le spinner d'auth ET l'hydratation. **Reste** (si voulu) : faire durer le `reveal` jusqu'à ce que le dashboard soit réellement prêt (là : durée fixe ~2,9 s).
>   - **RBAC confirmée** : entièrement côté **service Java** (`WorkspaceMember`/`ProjectMember` + enums `WorkspaceRole`/`ProjectRole`, checks manuels dans les services), Keycloak = **authentification seule** (`SecurityConfig` valide le JWT, `authenticated()`, aucun `@PreAuthorize`/rôle KC).
>   - **Reste** : 2FA (TOTP) encore hébergé Keycloak (à internaliser : secret + QR côté app) ; morph v2 « intro en overlay sur le 1er paint du dashboard » (v1 actuelle : intro → navigation dure).

> **▶ MAJ 25/08/2026 — QA-31b : CORRECTIF — le FRONT tapait le mauvais endpoint (ajout direct au lieu de PENDING).** `[QA-31b]`
> - **Bug** : fix B (QA-31) avait modifié le bon backend (`createInvitation` = `POST /invitations` → PENDING), MAIS les dialogs front appelaient **`inviteMember` = `POST /members/invite`** (ajout **DIRECT**, WorkspaceController). En plus, le bouton **« Add member » de la page Membres workspace** faisait pareil (store `invite` → `/members/invite`). → personne n'avait à accepter, malgré fix B.
> - **Correctif (front only)** : les **deux dialogs** (workspace `members/page.tsx` + projet `project-invite-dialog.tsx`) tapent désormais **`createInvitation`** (`/invitations`) → invitation **PENDING** (email + bannière à accepter) ; l'onboarding l'utilisait déjà. `CreateInvitationPayload` porte le contexte projet ; `InviteMemberPayload` nettoyé (contexte projet retiré, il visait le mauvais endpoint). Plus aucun ajout direct dans l'UI (hors équipes). `tsc` 0. **Backend inchangé** (déjà correct via fix B).

> **▶ MAJ 25/08/2026 — QA-31 : rejoindre un PROJET exige une acceptation (+ diag emails prod) (branche `fix/project-invitation-acceptance`).** `[QA-31]`
> - **Diagnostic (retour user « rien ne marche en prod »)** : deux causes. (1) **Emails prod ÉTEINTS** — `MAIL_HOST` vide → `EmailService.isEnabled()` false → feedback/invitation/notif jamais envoyés (config Brevo `.env.prod` + realm Keycloak, **déjà sur les VM** selon le user → résolu par le déploiement). (2) **Ajout à un projet DIRECT** — `ProjectInviteDialog` faisait `inviteMember` (PENDING) + `addProjectMember` (ajout direct sans acceptation) → membre du projet sans accepter.
> - **Fix (code B)** : l'invitation porte un **contexte projet optionnel** (`project_id`/`project_role`, migration **V78**). Le dialog projet crée désormais une **invitation PENDING** au lieu de l'ajout direct ; à l'acceptation (lien email OU bannière in-app), la personne rejoint le workspace (si besoin) **ET** le projet, avec le rôle choisi. Garde adaptée (déjà membre du projet → refus ; un membre du workspace PEUT être invité à un projet). Réutilise tout le flux accept/email/bannière existant.
> - **Tests** : `WorkspaceInvitationServiceIntegrationTest` — create projet PENDING, accept → membre ws+projet, refus si déjà membre projet.
> - **⚠️ Reste (follow-up)** : le plafond de sièges Free (projet privé, 5 collab) n'est plus enforced à l'invitation (il vivait dans `addProjectMember`) — à ré-ajouter au point d'acceptation.

> **▶ MAJ 25/08/2026 — QA-30 : acceptation / refus d'assignation de tâche (branche `hot-fix`).** `[QA-30]`
> - **Feature demandée** : un membre assigné par un tiers est **notifié** (déjà OK) ET peut désormais **accepter ou refuser** l'assignation. Cohérent avec Smart Assign (le manager propose, l'assigné valide/refuse).
> - **Back** : statut sur l'issue (`assignment_status` PENDING|ACCEPTED + `assignment_assigned_by_id`, migration **V77**, enum `AssignmentStatus`). Assignation par un tiers → **PENDING** + notif ; **self-assign → ACCEPTED** d'office. Endpoints `POST /api/me/assignments/{id}/(accept|decline)` (`AssignmentController` + `IssueService.acceptAssignment/declineAssignment`, **403** si pas l'assigné). **Refus (décision user)** : désassignation + **notif à l'assigneur** (`notifyAssignmentDeclined`, type `assignmentDeclined` mappé sur la préférence ASSIGNED) → il réassigne (Smart Assign s'il veut). `IssueResponse.assignmentStatus` exposé. Tests : `NotificationServiceTest` (notif refus) + `IssueServiceIntegrationTest` (self-assign ACCEPTED **sans notif** — le test attendait l'ancien comportement ; accept → ACCEPTED ; decline → désassigne + notifie ; 403 non-assigné).
> - **Front** : `assignment-service.ts` + `ASSIGNMENT_ROUTES` + `Issue.assignmentStatus`. **My Queue** : colonne **Accept / Decline** sur les assignations **PENDING** (MAJ optimiste : accept → ACCEPTED, decline → sort de la file). **Pas de backfill** : les assignations existantes n'affichent pas de prompt.
> - **Reste : la doc** (dernier chantier).
...
> - **Fix débordement 36px (retour user « ça fait remonter le contenu à l'ouverture d'un panneau »)** : le `SidebarProvider` (`[data-slot="sidebar-wrapper"]`) impose `min-h-svh` (100svh) → sur une page Labs (conteneur `svh − bandeau`) l'app débordait de **36px** (bas rogné) et le reflow d'ouverture d'un panneau « faisait remonter » le contenu. Scoped `.tf-lab-shell [data-slot="sidebar-wrapper"] { min-height: 0 }` → l'app fait exactement 684px (svh − bandeau). **Vérifié en live** : `main` inchangé (684px, top 36) à l'ouverture d'un panneau, zéro débordement.
...
| PROD-5.7 | **Console d'actions MCP générique** `[TF-MCP-06]` — écran pour parcourir les outils d'un serveur MCP connecté et en **exécuter un directement**, **hors Cortex/LLM** (donc **non impacté par le cap Groq**). ✅ **livré (05/09)** : back = endpoint `GET …/mcp/servers/{key}/tools` (schéma d'args + `readOnly`, via `WorkspaceMcpService.serverTools`, tests) ; front = `mcp-action-console.tsx` (liste outils + **formulaire généré depuis le JSON Schema** + exécution `POST …/mcp/actions/execute` + **confirmation sur écriture** + résultat), dépliée sous un serveur MCP joignable dans `ConnectorDialog`. **Générique** : marche pour Stripe **et les 25 MCP-ready**. Gate : 13 tests back + tsc front verts. **Déployé v0.3.17 (#245) + vérifié LIVE** le 05/09 : `list_teams` sur Linear via la console → renvoie l'équipe réelle « VMoney », **hors Cortex** (preuve directe que ça contourne le cap Groq). cf. [[integration-connect-vs-ingest]] |  ✅   |  P2  |  2–4   |
...
| PROD-8.11 | **Densité « grammaire Linear »** (20/09) : tokens `--control-height-sm / --control-height / --control-height-lg` (28 / 32 / 36 px) + `--row-height` (36) dans `globals.css`, exposés en utilitaires Tailwind v4 (`h-control`, `size-control`, `min-w-control`, `h-row`) et déclarés dans l'échelle `spacing` de tailwind-merge (`lib/utils.ts`, sinon les overrides de page cassent). Primitives ui/ basculées : button, input, select, input-group, date-picker, tabs, toggle, table (en-tête 40 -> 36, lignes 32), sidebar (13px). **Clair + bleu conservés** (décision CEO 20/09) : la passe joue sur tailles / bordures / densité, pas sur la couleur. **Validé CEO le 20/09** ; **2e passe faite le 20/09** : 41 contrôles de pages/composants ramenés sur les tokens (`h-9 -> h-control`, les `h-10` volontairement grands -> `h-control-lg`), 23 occurrences non-contrôles (icônes, avatars, logos, bandeau Labs) laissées à dessein, lignes de liste `py-2.5 -> py-1.5` (issues 45 -> 37 px mesurés, backlog, issues projet, fiche membre). Mesuré sur la stack dev : input/select 32, en-tête de table 36, plus aucun `h-9`/`h-10` visible sur Members. Login gardé à 32. Reste : `projects/[id]/list` (WIP CEO, même pattern `py-2.5`) + landing. Grammaire : `.ai/design-grammar.md`. Front-only. |  ✅   |  P2  |  1,0   |
...
| PROD-9.10 | **Grammaire de densité côté site (20/09)** : même système que l'app (PROD-8.11), adapté au marketing. Tokens `--control-height-sm / --control-height / --control-height-lg` = 32 / 36 / 40 (app : 28 / 32 / 36), exposés en `h-control*` / `size-control*` / `min-w-control*`, tailwind-merge étendu (`src/lib/utils.ts`), kit ui + header + pricing tokenisés à valeur constante ; formulaire de démo unifié à 40 (inputs 36 à côté de selects 40 avant). **Deux redéclarations silencieuses corrigées dans `global.css`** : `--radius` 10 px écrasé par 8 px (on garde 8 : avec le mapping du site, sm 6 / md 8 / lg 10 = l'échelle de l'app) ; ombres « flat de l'app » écrasées par les défauts shadcn sur 5 niveaux sur 8 (flat restauré : `--shadow-lg` = 0 4px 12px). Boutons pill (42 / 48) inchangés. **Visuels d'app du site recapturés** à la densité et aux données actuelles : dashboard du hero, plan Auto-assign, et le zoom (il montrait l'ancien panneau Smart Assign, retiré du produit : remplacé par le menu Assignee actuel). Outil de capture réparé (`design-assets` : consentement v2, visite guidée, attente du plan). Image Docker de prod du site passée en Node 22 (le job Release Services échouait sur chaque push touchant le site). Vérifié : mesure DOM, lint 0 erreur, build prod 94 pages, utilitaires présents dans le CSS de prod. Sur `dev` ; release `main` après validation visuelle du CEO. | 🟡 | P2 | 0,5 |

**Sous-total : ~9,5 j·h** (recoupe SEO C20).
