...
> - **`[TF-MCP-04]` (fondation posée)** : outil interne `create_project` → Cortex peut créer un projet natif, donc **import agentique** possible (« importe mon projet Linear ici » = create_project + boucle create_issue depuis `linear__list_issues`). ⚠️ limité par `MAX_TOOL_ITERS` pour les gros projets. **Reste** : wizard « Importer depuis X » + import **bulk déterministe** (fetch MCP → normalisation → création en masse).
>
