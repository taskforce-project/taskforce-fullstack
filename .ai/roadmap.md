...
> **▶ MAJ 27/08/2026 — P1a (bêta) : tag de cohorte des bêta-testeurs (V80).** `[P1a]`
...
> - **Reste P1** : **P1b** export projet serveur (JSON+CSV : issues + descriptions + commentaires + activité — réutilise `IssueService.listIssues/listComments/listActivity`), **P1c** métriques bêta via Grafana (datasource Postgres → `users`/`issue_activity`, filtré par `beta_cohort`/`beta_context`). À bundler (P1a+P1b) pour un seul déploiement backend.
