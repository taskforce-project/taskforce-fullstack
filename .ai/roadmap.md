...
>   - **(b) Intro « Cortex awakening »** — l'écran premium violet ③ (jugé « naze ») remplacé par une **cinématique blanche minimale** (`components/auth/cortex-awakening.tsx`, Canvas 2D + projection 3D maison) : fond **blanc**, forme abstraite qui **se construit** (un trait dessine la structure → éléments mats à ombres douces → respiration → dissolution vers des panneaux d'UI), *white product visualization* pas *AI animation* (zéro néon/particule/fond sombre). 1ʳᵉ connexion = full, ensuite **micro** (localStorage `tf.cortex.intro.seen`), **Skip** discret. Branché sur le callback OAuth (`AuthTransition` supprimé).
