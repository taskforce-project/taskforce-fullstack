...
> - **Reste** : **bouton front** (self-serve testeur — le user/admin ne peut exporter que les workspaces dont il est membre) ; **P1c** métriques bêta via Grafana (datasource Postgres → `users`/`issue_activity`, filtré `beta_cohort`/`beta_context`).
