...
> - **P1b front (fait, branche `feat/beta-p1-export-ui`)** : bouton « Export » (dropdown **CSV / JSON complet**) sur le board projet → `downloadProjectExport` (blob authentifié, `PROJECT_ROUTES.EXPORT`) ; remplace l'ancien export CSV client superficiel. **Reste = P1c** : métriques bêta via Grafana (datasource Postgres → `users`/`issue_activity`, filtré `beta_cohort`/`beta_context`).
