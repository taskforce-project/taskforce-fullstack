...
> - **Tests** : `AiGenerationServiceTest` (insertion draft, finalisation ACCEPTED/EDITED/REJECTED, dérivation signal SPEC + SMART_ASSIGN, `finalizeOnly` no-op, opt-in coupé, **no-throw** sur repo KO / entrée dégénérée, Levenshtein `@ParameterizedTest`, purge) + verify de `record` dans `SmartAssignServiceTest` (gate SMART_ASSIGN). Gates SPEC/DECISION prouvés end-to-end par le critère de fin (3 lignes en base). **[statut : en validation - `it.ps1 -Test ALL` + critère live à faire]**
> - **Reste avant merge** : `it.ps1 -Test ALL` vert ; flag activé sur le workspace démo pour démontrer le critère (3 lignes) ; trace Brain OS frère (`v1/02-produit/IA.md` + `road_to_v2`). Suite : V2 capture le downstream d'exécution sur le MÊME `ai_generations` (kind/final déjà extensibles).
