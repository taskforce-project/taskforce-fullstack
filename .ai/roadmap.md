...
>   - ~~**Status** : lit `/actuator/health`~~ ✅ **fait** : `StatusController` (`GET /api/status`) lit `HealthEndpoint` (DB, cache, disque… déjà cachés par Spring → **sans overload/ping externe**) ; `StatusPanel` affiche overall UP/DOWN/**DEGRADED** + par composant (fini les valeurs codées en dur). Reste optionnel : état « dégradé » depuis métriques Grafana/Prometheus (seuil) — à cadrer.
>   - **Topbar** : **bandeau bleu pleine largeur** (type « sandbox » Stripe) qui **englobe/wrap** les zones **expérimentales** (Intelligence/Lab) — avec **bg image** — pour donner l'impression d'« être ailleurs » ; le reste de l'app inchangé. Règle aussi le non-responsive de la topbar actuelle.
>   - **Signals** : vraie **table shadcn complète** (colonnes triables, en-têtes) comme My Queue, **mais colonnes/données propres aux signaux**.
>   - **Notifications** : **vrais réglages** par type d'event **+ vérifier le plumbing** — une notif est bien générée à l'**assignation d'une tâche** (in-app), arrive en **email** si opté, etc.
