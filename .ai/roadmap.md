...
> - **Reste** : suite backend `it.ps1 -Test ALL` à confirmer verte → commit + deploy fix→dev→main ; i18n de l'étape « code » (FR en dur pour l'instant) ; éventuel rate-limit dédié sur la vérif TOTP ; MAJ `taskforce-docs` (API + modèle de données) au déploiement.
