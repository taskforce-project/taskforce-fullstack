...
> - **Réglages Security (fonctionnel).** UI custom **adossée au métier Keycloak** (décision : pas la console KC) : *Reset password* (email `UPDATE_PASSWORD`) + toggle **2FA** (statut lu des credentials, activation par email `CONFIGURE_TOTP`, désactivation `removeTotp`). *Active sessions* **retiré**. Backend `KeycloakService` (sendPasswordResetEmail/isTotpEnabled/sendConfigureTotpEmail/disableTotp) + `UserController` (`POST /me/password/reset`, `GET|POST /me/2fa[/enable]`, `DELETE /me/2fa`). Cf. [[settings-security-keycloak-backed]].
...
> - Réglages restants : **Account** vide · **Notifications** sans réglage actionnable · **Privacy & Data** (bouton export/anonymisation ; *delete account* → Account, *delete data* → Data). *(Security ✅ ci-dessus. **Usage Cortex/factures** : laissé tel quel pour l'instant — décision. **Sessions actives** : abandonné.)*
