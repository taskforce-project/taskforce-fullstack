...
> - **Vérifié sur la stack dev** : suite backend complète verte (`it.ps1 -Test ALL` : 1264 tests, 0 échec) ; 59 tests du runner ; CI verte sur `dev` (tests backend, CodeQL, Semgrep) ; 34 sondes HTTP en direct conformes ; un run complet jusqu'à « In review by AI » et le chemin d'échec jusqu'à « Blocked ». **Non vérifié** : l'agent était un substitut (`test/fixtures/stub-agent`), le CLI Claude Code n'étant pas installé sur le poste ; restent à éprouver les options de `claude -p` et l'ouverture de PR par `gh`. **Garde-fous** : `delivery.local-runner.enabled` vrai en dev, **faux en prod** ; l'abonnement Claude ne vaut que pour un usage personnel, un usage produit suppose une clé API (`agent.auth`). **Limites connues** dans l'ADR-013 (jeton de session par run à faire, pas d'expiration de file, commentaire de l'agent au nom du délégant).
