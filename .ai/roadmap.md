...
> - **`[TF-MCP-04]` (wizard front fait)** : mode **« Import from a tool »** dans `create-project-dialog.tsx` (bascule vierge/import) → liste les sources MCP connectées (`getMcpServers`, joignables) + nom du projet → `importMcpProject` → toast + atterrissage sur le nouveau projet. Route `MCP_IMPORT`, service + test `integration-service.test`. `tsc` OK. **Reste (optionnel)** : import bulk >100 issues, mapping des statuts externes.
>
