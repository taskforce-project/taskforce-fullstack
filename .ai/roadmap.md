...
> - Typecheck `tsc` + ESLint + CI (Build/Tests/Semgrep/Trivy/Labels) **verts**, déployé VM2 (conteneur healthy, `app`+`/auth/login` 200). **⚠️ Reste à vérifier par l'usage** : réception réelle d'un email de reset (cliquer « Reset password » en prod).
