...
> - **Analytics 409 « ADVANCED_ANALYTICS ».** `AnalyticsService.{getThroughput,getBurndown,getCapacity,getWorkload}` gatent derrière le plan (409). **Décision produit en attente** (gating voulu ? statut 409→402/403 ? UX front « upgrade » plutôt qu'un log d'erreur) — non corrigé.
