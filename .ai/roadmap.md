...
> - **P1 slice 1 EN COURS (backend fait, 09/09)** : lien Projet ↔ dépôt GitHub (`BE-AGENT-DELIVERY-1`). Migration **V85** (`projects.repo_provider`/`repo_full_name`), `ProjectRepoService` (service dédié) + endpoints `PUT`/`DELETE …/projects/{id}/repo` (CREATE via `GitHubIntegrationService.createRepo` / LINK), exposé dans `ProjectResponse`. Run ciblé **18/18 verts** (ProjectRepoServiceTest + 2 slices web ProjectController). **Reste** : front (option repo au wizard de création + lien en cours de route façon Linear), suite backend complète, puis release/deploy sur confirmation.
