...
> - **Tests** : `McpOAuthServiceTest` (start + callback + erreurs), `McpTokenServiceTest` (refresh + stockage), `McpOAuthDiscoveryTest` (chaîne + fallbacks). **[statut : backend compile + `WorkspaceMcpServiceTest` vert ; 3 tests OAuth en cours de run]**
> - **Reste** : `it.ps1 -Test ALL` ; config prod (`redirect-base`/`app-base` = URLs publiques du backend/front) ; live E2E - le `start` (découverte + DCR) testable contre Linear, mais le consentement + callback nécessitent le compte Linear du user. Caveat DCR : pas universel (fallback : coller un token).
