...
> - **Version PRODUIT unique (footer) (10/09)** : le footer landing montrait `v0.3.23` en dur, incoherent avec les tags par service. Fix : **source de verite** `landing-page/src/product-version.ts` (`PRODUCT_VERSION`, seed **0.4.0**) que le footer lit (`nav.ts` -> APP_VERSION) ; script `scripts/bump-product-version.mjs <major|minor|patch>` (le plus fort bump des services touches, a lancer au release) ; la CI `release.yml` cree en plus le tag **`taskforce-v<version>`** sur main (lu depuis le fichier). Change landing-page/+scripts/+.github/ only -> la release ne bump QUE la landing (file-diff) + tague le produit. typecheck + build landing verts. A releaser (landing:release:patch) pour que le footer passe a 0.4.0 en prod.
