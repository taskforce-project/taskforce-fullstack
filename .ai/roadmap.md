...
> - **Avatar « ne change pas du tout »** : l'upload marchait (`POST /api/users/me/avatar` authentifié → MinIO), mais l'endpoint qui **sert** l'avatar (`GET /api/files/avatars/{id}`, `FileController`, commenté « public ») n'était **PAS dans `PUBLIC_MATCHERS`** de `SecurityConfig` (seul `/api/files/brain/**` l'était) → un `<img>` (sans header Bearer) prenait **401** → image cassée. Ajouté **`/api/files/avatars/*`** aux endpoints publics. Confirmé en prod : `curl avatars/1000` → 401 avant le fix.
